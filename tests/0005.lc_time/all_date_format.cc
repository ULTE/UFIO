...
int main()
{
	...
	for (std::uint8_t i{}; i < 50; ++i)
	{
		...
		for (auto fnm : files)
		{
			println(f, u8"Current Locale:", ufio::mnp::code_cvt(ufio::mnp::os_c_str(fnm)));
			println(f, u8"Current Timestamp:", ts);
			char8_t format_buffer[3]{u8'%', u8'%', u8'%'};
			ufio::basic_io_scatter_t<char8_t> test_format{format_buffer, 2};
			ufio::native_l10n loc(ufio::mnp::os_c_str(fnm));
			auto test = [&](char8_t i) {
				std::size_t reserved{::ufio::details::lc_print_reserve_size_time_format_common_impl(loc.loc.u8all->time, ts, test_format)};
				u8s.resize(reserved);
				auto itr = ::ufio::details::lc_print_reserve_define_time_fmt_common_impl(loc.loc.u8all->time, u8s.data(), ts, test_format);
				std::size_t actual{static_cast<std::size_t>(itr - u8s.data())};
				print(f, ufio::mnp::chvw(i), u8" reserved:", reserved, u8"\tactually use:", actual);
				if (reserved < actual)
				{
					print(f, u8"\twrong!");
				}
