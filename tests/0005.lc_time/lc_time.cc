...
{
	ufio::posix_tzset();
	ufio::obuf_file obf("lc_time.txt");
	...
	for (auto fnm : files)
	{
		ufio::native_l10n loc(os_c_str(fnm));
		auto v{posix_clock_gettime(ufio::posix_clock_id::realtime)};
		ufio::iso8601_timestamp utc_tsp{utc(v)};
		ufio::iso8601_timestamp tsp{local(v)};
