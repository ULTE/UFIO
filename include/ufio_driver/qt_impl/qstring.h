...
{
	return ::ufio::basic_general_concat<false, char16_t, QString>(::std::forward<Args>(args)...);
}

template <typename... Args>
inline QString u16concatln_qt_qstring(Args &&...args)
{
	return ::ufio::basic_general_concat<true, char16_t, QString>(::std::forward<Args>(args)...);
}

inline constexpr io_strlike_reference_wrapper<char16_t, QString> io_strlike_ref(::ufio::io_alias_t,
...
inline auto qt_as_filename(T const &hstr) noexcept
{
	...
	{
		return ::ufio::manipulators::basic_os_c_str_with_known_size<char16_t>{
			reinterpret_cast<char16_may_alias_const_ptr>(hstr.data()), static_cast<::std::size_t>(hstr.size())};
	}
	else
	{
		return ::ufio::manipulators::basic_os_str_known_size_without_null_terminated<char16_t>{
			reinterpret_cast<char16_may_alias_const_ptr>(hstr.data()), static_cast<::std::size_t>(hstr.size())};
	}
}
