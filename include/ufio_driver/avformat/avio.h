...
inline void write(basic_avio_context_io_observer<char_type> baciob, char_type const *first, char_type const *last)
{
	...
	if constexpr (INT_MAX < SIZE_MAX)
	{
		for (; first != last;)
		{
			...
			::ufio::noexcept_call(::avio_write, baciob.avios, first, last);
			first += this_round;
		}
	}
	else
	{
		::ufio::noexcept_call(::avio_write, baciob.avios, first, static_cast<int>(last - first));
	}
...
inline char_type *read(basic_avio_context_io_observer<char_type> baciob, char_type *first, char_type *last)
{
	...
	if constexpr (INT_MAX < SIZE_MAX)
	{
		...
		ret = ::ufio::noexcept_call(::avio_read, baciob.avios, first, this_round);
	}
	else
	{
		ret = ::ufio::noexcept_call(::avio_read, baciob.avios, first, static_cast<int>(last - first));
	}
	if (baciob.avios->error)
	{
		throw_posix_error(EINVAL);
...
public:
	...
	void close() noexcept
	{
		...
		ufio::noexcept_call(::av_free, this->avios);
		this->avios = nullptr;
	}
...
struct avio_buffer
{
	...
	void close() noexcept
	{
		...
		::ufio::noexcept_call(::av_free, buffer);
		this->buffer = nullptr;
	}
...
concept has_file_status_impl = requires(T t) {
	{ status(t) } -> ::std::same_as<::ufio::posix_file_status>;
};
...
inline basic_avio_buffer_context<typename rftype::char_type> create_avio_context_impl(rftype rf)
{
	...
	auto bfptr{::ufio::noexcept_call(::av_malloc, buffersize)};
	if (bfptr == nullptr)
	{
		::ufio::fast_terminate();
	}
	...
	ctx = ::avio_alloc_context(
		...
		[](void *opaque, int64_t offset, int whence) noexcept -> ::std::int64_t {
			...
			try
			{
				...
				return seek(rft, offset, static_cast<::ufio::seekdir>(whence));
			}
			catch (...)
			{
				return -1;
...
{
	return ::ufio::details::create_avio_context_impl(io_ref(sm));
}
...
struct avformat_context_guard
{
	...
	void close() noexcept
	{
		...
		::ufio::noexcept_call(avformat_free_context, pFormatCtx);
		this->pFormatCtx = nullptr;
	}
...
struct avformat_input_guard
{
	...
	void close() noexcept
	{
		...
		::ufio::noexcept_call(avformat_close_input, ppFormatCtx);
		this->ppFormatCtx = nullptr;
	}
