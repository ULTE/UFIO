...
public:
	...
	{
		int err{};
		this->s = ::ufio::noexcept_call(pa_simple_new, server, name, dir, dev, stream_name, ss, map, attr,
	...
	basic_pulseaudio_simple_file &operator=(basic_pulseaudio_simple_file &&__restrict hd) noexcept
	{
		if (this->s) [[likely]]
		{
			::ufio::noexcept_call(pa_simple_free, this->s);
		}
		this->s = hd.s;
		hd.s = nullptr;
		return *this;
...
{
	int err{};
	if (::ufio::noexcept_call(pa_simple_read, ps, first, static_cast<::std::size_t>(last - first),
...
{
	int err{};
	if (::ufio::noexcept_call(pa_simple_write, ps, first, static_cast<::std::size_t>(last - first),
								 __builtin_addressof(err)) < 0) [[unlikely]]
	{
		throw_pulse_audio_error(err);
	}
...
inline void read_all_bytes_underflow_define(basic_pulseaudio_simple_io_observer<ch_type> iob, ::std::byte *first,
											::std::byte *last)
{
	return ::ufio::details::pa_simple_read_underflow_impl(iob.s, first, last);
}
...
inline void write_all_bytes_overflow_define(basic_pulseaudio_simple_io_observer<ch_type> iob, ::std::byte const *first,
											::std::byte const *last)
{
	return ::ufio::details::pa_simple_write_overflow_impl(iob.s, first, last);
}
