...
/*
Create python's PyBytesObject by using ufio's api
*/
#include "../../ufio_core.h"
#include <Python.h>
...
inline void py_inc_dec_ref_impl(void *p) noexcept
{
	...
	::ufio::noexcept_call(py_inc_dec_ref_model_impl<dec>, p);
}
...
{
	auto p{::ufio::noexcept_call(pybytes_fromstringandsize_model_impl, v, len)};
	if (p == nullptr)
	{
		::ufio::fast_terminate();
	}
...
inline PyObject *pyunicode_asencodedstring_impl(PyObject *self, char const *name, char const *value) noexcept
{
	...
	return ::ufio::noexcept_call(pyunicode_asencodedstring_model_impl, self, name, value);
}
...
#endif
	{
		if (::ufio::noexcept_call(pybytes_asstringandsize_model_impl, self, __builtin_addressof(s),
...
public:
	...
	{
		::ufio::details::py_inc_dec_ref_impl<false>(this->p);
	}
	pyobject_file &operator=(pyobject_file const &pob) noexcept
	{
		::ufio::details::py_inc_dec_ref_impl<true>(this->p);
		::ufio::details::py_inc_dec_ref_impl<false>(this->p = pob.p);
		return *this;
	}
											  ...
											  char const *last) noexcept
{
	...
	auto p{::ufio::details::pybytes_fromstringandsize_impl(first, static_cast<Py_ssize_t>(diff))};
	::ufio::details::py_inc_dec_ref_impl<false>(p);
...
{
	return ::ufio::basic_general_concat<false, char, pyobject_file>(::std::forward<Args>(args)...);
}

template <typename... Args>
inline pyobject_file concatln_pyobject_file(Args &&...args)
{
	return ::ufio::basic_general_concat<true, char, pyobject_file>(::std::forward<Args>(args)...);
}

inline pyobject_file print_alias_define(io_alias_t, pyobject_io_observer pyiob) noexcept
{
	pyobject_file pyob_repr{::ufio::noexcept_call(::ufio::details::pyobject_repr_model_impl, pyiob.p)};
	return pyobject_file(::ufio::details::pyunicode_asencodedstring_impl(
...
status_io_print_forward(io_alias_type_t<char_type>, pyobject_io_observer piob) noexcept
{
	...
	{
		return ::ufio::details::pybytes_asstringandsize_impl(piob.p);
	}
	else if constexpr (sizeof(char) == sizeof(char_type))
	{
		auto ret{::ufio::details::pybytes_asstringandsize_impl(piob.p)};
