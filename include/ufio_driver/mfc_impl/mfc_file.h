...
{
	::ufio::details::mfc_write_impl(hd.phandle, ::std::to_address(first), ::std::to_address(last));
}

template <::std::integral T>
inline void scatter_write(basic_mfc_io_observer<T> hd, io_scatters_t scatters)
{
	::ufio::details::mfc_scatter_write_impl(hd.phandle, scatters.base, scatters.len);
}
