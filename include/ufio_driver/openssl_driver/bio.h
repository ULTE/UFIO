...
{
	FILE *fp{};
	::ufio::noexcept_call(BIO_ctrl, bio, BIO_C_GET_FILE_PTR, 0, reinterpret_cast<char *>(__builtin_addressof(fp)));
	return fp;
}
...
inline int bio_to_fd(BIO *bio) noexcept
{
	...
	{
		int fd{-1};
		::ufio::noexcept_call(::BIO_ctrl, bio, BIO_C_GET_FD, 0, reinterpret_cast<char *>(__builtin_addressof(fd)));
		return fd;
	}
	return ::ufio::details::fp_to_fd(fp);
}
...
inline decltype(auto) get_cookie_data_from_void_ptr_data(void *data) noexcept
{
	if constexpr (::std::is_trivially_copyable_v<stm> && sizeof(stm) <= sizeof(void *))
	{
		...
		alignas(alignof(stm))::ufio::freestanding::array<char, sizeof(stm)> a;
...
struct bio_io_cookie_functions_t
{
	...
	explicit bio_io_cookie_functions_t()
	{
		...
		if constexpr (::ufio::operations::decay::defines::has_any_of_read_bytes_operations<value_type>)
		{
			functions.bread = [](BIO *bbio, char *buf, ::std::size_t size, ::std::size_t *readd) noexcept -> int {
				...
				try
				{
					...
					*readd = static_cast<::std::size_t>(
						::ufio::operations::read_some_bytes(
							::ufio::details::get_cookie_data_from_bio_data<value_type>(bbio),
		...
		if constexpr (::ufio::operations::decay::defines::has_any_of_write_bytes_operations<value_type>)
		{
			functions.bwrite = [](BIO *bbio, char const *buf, ::std::size_t size,
				...
				try
				{
					...
					*written = static_cast<::std::size_t>(
						::ufio::operations::write_some_bytes(
							::ufio::details::get_cookie_data_from_bio_data<value_type>(bbio),
...
{
	auto bio{::ufio::noexcept_call(BIO_new, methods)};
	if (bio == nullptr)
	{
		throw_openssl_error();
	}
...
inline BIO *construct_bio_by_t(void *ptr)
{
	...
	::ufio::noexcept_call(BIO_set_data, bp, ptr);
	return bp;
}
...
{
	auto bio{::ufio::noexcept_call(BIO_new_fp, fp, om)};
...
inline BIO *open_bio_with_fp(FILE *fp, ::ufio::open_mode om)
{
...
public:
	...
	basic_bio_file(::ufio::io_cookie_type_t<stm>, Args &&...args)
		: basic_bio_io_observer<char_type>{details::construct_bio_by_args<stm>(::std::forward<Args>(args)...)}
	{
	}

	template <c_family family>
	basic_bio_file(basic_c_family_file<family, char_type> &&bmv, ufio::open_mode om)
		: basic_bio_io_observer<char_type>{::ufio::details::open_bio_with_fp(bmv.fp, om)}
	{
		bmv.fp = nullptr;
	}
	basic_bio_file(basic_posix_file<char_type> &&bmv, ufio::open_mode om)
		: basic_bio_file(basic_c_file<char_type>(::std::move(bmv), om), om)
	{
	}
#if (defined(_WIN32) && !defined(__WINE__)) || defined(__CYGWIN__)
	template <win32_family family>
	basic_bio_file(basic_win32_family_file<family, char_type> &&win32_handle, ufio::open_mode om)
...
inline ::std::byte *bio_read_impl(BIO *bio, ::std::byte *first, ::std::byte *last)
{
	...
	if (::ufio::noexcept_call(BIO_read_ex, bio, first, static_cast<::std::size_t>(last - first),
...
inline ::std::byte const *bio_write_impl(BIO *bio, ::std::byte const *first, ::std::byte const *last)
{
	...
	if (::ufio::noexcept_call(BIO_write_ex, bio, first, static_cast<::std::size_t>(last - first),
...
{
	return status(posix_io_observer{::ufio::details::bio_to_fd(bio)});
}
#endif
} // namespace details
...
inline ::std::byte *read_some_bytes_underflow_define(basic_bio_io_observer<ch_type> iob, ::std::byte *first,
													 ::std::byte *last)
{
	return ::ufio::details::bio_read_impl(iob.bio, first, last);
}
...
inline ::std::byte const *write_some_bytes_overflow_define(basic_bio_io_observer<ch_type> iob, ::std::byte const *first,
														   ::std::byte const *last)
{
	return ::ufio::details::bio_write_impl(iob.bio, first, last);
}
...
inline ::ufio::intfpos_t bio_io_seekbytes_impl(BIO *bio, ::ufio::intfpos_t offset, seekdir s)
{
	...
	return ::ufio::operations::decay::io_stream_seek_bytes_decay(::ufio::posix_io_observer{fd}, offset, s);
}

inline ::std::byte *bio_pread_impl(BIO *bio, ::std::byte *first, ::std::byte *last, ::ufio::intfpos_t offset)
{
	auto fd{bio_to_fd(bio)};
	if (fd == -1)
	{
	...
	::ufio::posix_io_observer piob{fd};
	return ::ufio::operations::decay::pread_some_bytes_decay(piob, first, last, offset);
}

inline ::std::byte const *bio_pwrite_impl(BIO *bio, ::std::byte const *first, ::std::byte const *last,
										  ::ufio::intfpos_t offset)
{
	auto fd{bio_to_fd(bio)};
	if (fd == -1)
	{
	...
	::ufio::posix_io_observer piob{fd};
	return ::ufio::operations::decay::pwrite_some_bytes_decay(piob, first, last, offset);
}

} // namespace details

template <::std::integral ch_type>
inline ::ufio::intfpos_t io_stream_seek_bytes_define(basic_bio_io_observer<ch_type> iob, ::ufio::intfpos_t offset,
														seekdir s)
{
	return ::ufio::details::bio_io_seekbytes_impl(iob.bio, offset, s);
}

template <::std::integral ch_type>
inline ::std::byte *pread_some_bytes_underflow_define(basic_bio_io_observer<ch_type> iob, ::std::byte *first,
													  ::std::byte *last, ::ufio::intfpos_t offset)
{
	return ::ufio::details::bio_pread_impl(iob.bio, first, last, offset);
}
...
inline ::std::byte const *pwrite_some_bytes_overflow_define(basic_bio_io_observer<ch_type> iob,
															...
															::ufio::intfpos_t offset)
{
	return ::ufio::details::bio_pwrite_impl(iob.bio, first, last, offset);
}
...
inline void print_define_openssl_error(output out)
{
	...
	{
		::ufio::noexcept_call(ERR_print_errors, out.bio);
	}
	else
	{
		bio_file bf(io_cookie_type<output>, out);
		::ufio::noexcept_call(ERR_print_errors, bf.bio);
	}
}
...
{
	::ufio::details::print_define_openssl_error(out);
}
