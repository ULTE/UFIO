...
public:
	...
	{
		return {::ufio::llvm::details::hack_fd_from_llvm_raw_fd_ostream(this->os)};
	}
...
inline void llvm_ostream_write_impl(::llvm::raw_ostream *os, void const *first_vptr, void const *last_vptr)
{
	...
	{
		::ufio::llvm::details::llvm_raw_ostream_set_obuffer_curr_ptr(
			os, ::ufio::details::non_overlapped_copy_n(first, n, os_obuffer_curr_ptr));
		return;
	}
	os->write(first, n);
}
...
{
	::ufio::llvm::details::llvm_ostream_write_impl(osiob.os, ::std::to_address(first), ::std::to_address(last));
}
...
{
	return ::ufio::llvm::details::llvm_raw_ostream_obuffer_ptr<0, char_type>(osiob.os);
}
...
{
	return ::ufio::llvm::details::llvm_raw_ostream_obuffer_ptr<1, char_type>(osiob.os);
}
...
{
	return ::ufio::llvm::details::llvm_raw_ostream_obuffer_ptr<2, char_type>(osiob.os);
}
...
{
	::ufio::llvm::details::llvm_raw_ostream_set_obuffer_curr_ptr(osiob.os, reinterpret_cast<char *>(ptr));
}

template <::std::integral char_type, typename T>
inline void obuffer_overflow(basic_general_raw_ostream_io_observer<char_type, T> osiob, char_type ch) noexcept
{
	::ufio::llvm::details::llvm_raw_ostream_overflow(osiob.os, static_cast<char>(ch));
}
