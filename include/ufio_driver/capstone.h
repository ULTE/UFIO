...
{
	auto ret{::ufio::noexcept_call(::cs_option, csh, t, value)};
	if (ret != CS_ERR_OK)
	{
		throw_cs_error(ret);
	}
...
public:
	...
	{
		::ufio::capstone::details::cs_option_wrapper_impl(this->csh, t, value);
	}
};
...
public:
	...
	cs_file &operator=(cs_file &&__restrict c) noexcept
	{
		if (this->csh) [[likely]]
		{
			::ufio::noexcept_call(cs_close, __builtin_addressof(this->csh));
		}
		this->csh = c.csh;
		c.csh = 0;
		return *this;
	...
	{
		auto ret{::ufio::noexcept_call(cs_open, arch, mode, __builtin_addressof(this->csh))};
		if (ret != CS_ERR_OK)
		{
			throw_cs_error(ret);
		}
...
public:
	...
	cs_insn_range &operator=(cs_insn_range &&__restrict other) noexcept
	{
		if (this->ins) [[likely]]
		{
			::ufio::noexcept_call(cs_free, this->ins, this->count);
		}
		this->ins = other.ins;
