...
inline constexpr ::ufio::manipulators::scalar_flags fast_pipe_hex_scl_flg{
	.base = 16, .floating = ::ufio::manipulators::floating_format::fixed};

template <win32_family family>
inline void *win32_family_refterm_fast_pipe_impl(open_mode mode, perms pm)
{
	::std::uint_least32_t process_id{::ufio::win32::GetCurrentProcessId()};
	using type = ::ufio::mnp::scalar_manip_t<fast_pipe_hex_scl_flg, ::std::uint_least32_t>;
	if constexpr (family == win32_family::wide_nt)
	{
		...
		*res = 0;
		return win32_file_ntw(::ufio::mnp::os_c_str(buffer), mode, pm).release();
	}
	else
	{
		...
		*res = 0;
		return win32_file_9xa(::ufio::mnp::os_c_str(buffer), mode, pm).release();
	}
}
