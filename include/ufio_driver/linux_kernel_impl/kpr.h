...
inline void linux_kpr_raw_write_fmt(char const *fmt, void const *start, void const *last) noexcept
{
	...
	::ufio::linux_kernel_printk(fmt, static_cast<int>(diff), start_char_ptr);
}

template <bool line>
inline void linux_kpr_raw_write(::ufio::kern k, void const *start, void const *last) noexcept
{
...
inline constexpr auto linux_kernel_printk_formatter_cal() noexcept
{
	...
	::ufio::freestanding::array<char8_t,
...
inline void deal_with_kpr_fmt_real(char const *fmt_str, kpr_scatter_struct *s, Args... args)
{
	if constexpr (n == 0)
	{
		::ufio::linux_kernel_printk(fmt_str, args...);
	}
	else
	{
...
inline void deal_with_kpr_ignore_line(char const *fmt_str, Args... args)
{
	...
	if constexpr (n == 0)
	{
		::ufio::linux_kernel_printk(fmt_str);
	}
	else
	{
...
{
	::ufio::details::linux_kpr_raw_write<false>(kpr.level, first, last);
}
#endif
...
#endif
		sizeof(ch_type) == 1 &&
		(!::ufio::details::is_ebcdic<ch_type>))
inline constexpr void print_status_define(basic_kpr<ch_type> k, Args... args)
{
	if constexpr ((::ufio::details::kpr_total_print_reserve_size_less_or_equal_than512<ch_type, Args...>(0)))
	{
		::ufio::details::deal_with_kpr_common<line>(k, args...);
	}
	else
	{
