...
struct mp3_duration_result
{
	...
	::ufio::parse_code code{::ufio::parse_code::ok};
};
...
inline mp3_duration_result compute_mp3_duration(void const *first, void const *last) noexcept
{
	...
	{
		return {0, ::ufio::parse_code::end_of_file};
	}
	...
	for (; mp3frameheadersize < static_cast<::std::size_t>(lastbyte - firstbyte);)
	{
		...
		{
			return {0, ::ufio::parse_code::invalid};
		}
		char8_t flag0;
		::memcpy(__builtin_addressof(flag0), firstbyte + 8, 1);
		if (!::ufio::char_category::is_c_upper(flag0))
		{
			return {0, ::ufio::parse_code::ok};
		}
...
#endif
				= char8_t const *;
			auto code = ::ufio::parse_by_scan(reinterpret_cast<char8constptr>(firstbyte),
