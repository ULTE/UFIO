...
struct basic_line_scanner_buffer
{
	struct buffer_type
	{
		...
		{
			::ufio::details::deallocate_iobuf_space<false, char_type>(
															 ...
															 char_type const *first, ::std::size_t sz) noexcept
{
	...
	auto new_begin_ptr{::ufio::details::allocate_iobuf_space<char_type>(new_capacity)};
	...
	::ufio::details::deallocate_iobuf_space<false, char_type>(bg_ptr, old_capacity);
										 ...
										 char_type const *it) noexcept
{
	...
	return {it + 1, ::ufio::parse_code::ok};
}
...
scan_iterative_next_line_define_impl(basic_line_scanner_buffer<char_type> &__restrict buf, char_type const *first,
									 char_type const *last) noexcept
{
	auto it{::ufio::find_lf(first, last)};
	if (it == last) [[unlikely]]
	{
...
scan_iterative_next_define(io_reserve_type_t<char_type, basic_line_scanner_buffer<char_type>>,
						   ...
						   char_type const *last) noexcept
{
	return ::ufio::details::scan_iterative_next_line_define_impl(buf, first, last);
}
...
scan_iterative_eof_define(io_reserve_type_t<char_type, basic_line_scanner_buffer<char_type>>,
						  basic_line_scanner_buffer<char_type> &__restrict buf) noexcept
{
	return ::ufio::details::scan_iterative_eof_define_line_internal(buf);
}
...
scan_iterative_contiguous_define(io_reserve_type_t<char_type, basic_line_scanner_contiguous_view<char_type>>,
								 ...
								 char_type const *last) noexcept
{
	return ::ufio::details::scan_iterative_contiguous_line_define_impl(buf, first, last);
}
