...
{
	using typed_allocator_type = ::ufio::typed_generic_allocator_adapter<allocator_type, char_type>;
...
inline constexpr char_type const *write_some_typical_case(optstmtype optstm,
														  ...
														  char_type const *first, char_type const *last)
{
	if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<optstmtype>)
	{
		...
		auto [position, scpos]{::ufio::operations::decay::scatter_write_some_bytes_decay(optstm, scatters, 2)};
		if (position == 2)
		{
		...
		else
		{
			...
			pointers.buffer_curr =
				::ufio::freestanding::copy_n(pointers.buffer_curr - scpos, scpos, pointers.buffer_begin);
			return first;
		}
	}
	else
	{
		...
		auto [position, scpos]{::ufio::operations::decay::scatter_write_some_decay(optstm, scatters, 2)};
		if (position == 2)
		{
			pointers.buffer_curr = pointers.buffer_begin;
			return last;
...
inline constexpr void write_all_typical_case(optstmtype optstm,
											 ...
											 char_type const *first, char_type const *last)
{
	if constexpr (::ufio::operations::decay::defines::has_any_of_write_bytes_operations<optstmtype>)
	{
		io_scatter_t const scatters[2]{
			{pointers.buffer_begin,
			 static_cast<::std::size_t>(reinterpret_cast<::std::byte const *>(pointers.buffer_curr) -
			...
			{first, static_cast<::std::size_t>(reinterpret_cast<::std::byte const *>(last) -
											   reinterpret_cast<::std::byte const *>(first))}};
		::ufio::operations::decay::scatter_write_all_bytes_decay(optstm, scatters, 2);
	}
	else
	{
		basic_io_scatter_t<char_type> const scatters[2]{
			{pointers.buffer_begin, static_cast<::std::size_t>(pointers.buffer_curr - pointers.buffer_begin)},
			{first, static_cast<::std::size_t>(last - first)}};
		::ufio::operations::decay::scatter_write_all_decay(optstm, scatters, 2);
	}
	pointers.buffer_curr = pointers.buffer_begin;
}
														   ...
														   char_type const *first, char_type const *last)
{
	...
	if (pointers.buffer_begin == nullptr)
	{
		...
		{
			return ::ufio::operations::decay::write_some_decay(optstm, first, last);
		}
	}
											 ...
											 char_type const *first, char_type const *last)
{
	basic_io_scatter_t<char_type> const scatters[2]{
		...
		{first, static_cast<::std::size_t>(last - first)}};
	::ufio::operations::decay::scatter_write_all_decay(pointers, scatters, 2);
	pointers.buffer_curr = pointers.buffer_begin;
}
											  ...
											  char_type const *first, char_type const *last)
{
	...
	if (pointers.buffer_begin == nullptr)
	{
		...
		{
			::ufio::operations::decay::write_all_decay(optstm, first, last);
		}
		return;
	}
...
inline constexpr void output_stream_buffer_flush_impl(optstmtype optstm, basic_io_buffer_pointers<char_type> &pointers)
{
	...
	::ufio::operations::decay::write_all_decay(optstm, pointers.buffer_begin, pointers.buffer_curr);
	pointers.buffer_curr = pointers.buffer_begin;
}
...
{
	return ::ufio::details::io_buffer::write_some_overflow_impl<typename io_buffer_type::output_char_type,
																   typename io_buffer_type::traits_type::allocator_type,
																   io_buffer_type::traits_type::output_buffer_size>(
		::ufio::operations::output_stream_ref(iobref.iobptr->handle), iobref.iobptr->output_buffer, first, last);
}
...
inline constexpr void write_all_overflow_define(basic_io_buffer_ref<io_buffer_type> iobref,
												...
												typename io_buffer_type::output_char_type const *last)
{
	return ::ufio::details::io_buffer::write_all_overflow_impl<typename io_buffer_type::output_char_type,
																  typename io_buffer_type::traits_type::allocator_type,
																  io_buffer_type::traits_type::output_buffer_size>(
		::ufio::operations::output_stream_ref(iobref.iobptr->handle), iobref.iobptr->output_buffer, first, last);
}
...
pwrite_some_overflow_define(basic_io_buffer_ref<io_buffer_type> iobref,
							...
							::ufio::intfpos_t off)
{
	return ::ufio::operations::decay::pwrite_some_decay(::ufio::operations::output_stream_ref(iobref.iobptr->handle), first, last, off);
}
...
inline constexpr void pwrite_all_overflow_define(basic_io_buffer_ref<io_buffer_type> iobref,
												 ...
												 ::ufio::intfpos_t off)
{
	::ufio::operations::decay::pwrite_all_decay(::ufio::operations::output_stream_ref(iobref.iobptr->handle), first, last, off);
}

template <typename io_buffer_type>
inline constexpr ::ufio::io_scatter_status_t
scatter_pwrite_some_overflow_define(basic_io_buffer_ref<io_buffer_type> iobref,
									::ufio::basic_io_scatter_t<typename io_buffer_type::output_char_type> pscatters,
									::std::size_t n, ::ufio::intfpos_t off)
{
	return ::ufio::operations::decay::scatter_pwrite_some_decay(::ufio::operations::output_stream_ref(iobref.iobptr->handle), pscatters, n, off);
}
...
inline constexpr void scatter_pwrite_all_overflow_define(basic_io_buffer_ref<io_buffer_type> iobref,
														 ...
														 ::std::size_t n, ::ufio::intfpos_t off)
{
	::ufio::operations::decay::scatter_pwrite_all_decay(::ufio::operations::output_stream_ref(iobref.iobptr->handle), pscatters, n, off);
}

template <typename io_buffer_type>
inline constexpr void output_stream_buffer_flush_define(basic_io_buffer_ref<io_buffer_type> iobref)
{
	::ufio::details::io_buffer::output_stream_buffer_flush_impl<typename io_buffer_type::output_char_type>(
		::ufio::operations::output_stream_ref(iobref.iobptr->handle), iobref.iobptr->output_buffer);
}
...
inline constexpr ::std::size_t
	obuffer_minimum_size_define(::ufio::io_reserve_type_t<char_type, basic_io_buffer_ref<io_buffer_type>>)
{
...
{
	::ufio::details::io_buffer::obuffer_minimum_size_flush_prepare_impl<
