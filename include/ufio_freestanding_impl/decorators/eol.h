...
struct basic_eol_converter
{
	...
	process_chars(char_type const *fromfirst, char_type const *fromlast, char_type *tofirst, char_type *tolast) noexcept
	{
		...
		if constexpr (fromscheme == eol_scheme::lf && toscheme == eol_scheme::crlf)
		{
			...
			{
				auto [fromit,
					  toit]{::ufio::details::simd_lf_crlf_process_chars(fromfirst, fromlast, tofirst, tolast)};
				fromfirst = fromit;
				tofirst = toit;
			}
		...
		else if constexpr (fromscheme == eol_scheme::crlf && toscheme == eol_scheme::lf)
		{
			...
			{
				auto [fromit,
					  toit]{::ufio::details::simd_crlf_lf_process_chars(fromfirst, fromlast, tofirst, tolast)};
				fromfirst = fromit;
				tofirst = toit;
			}
			for (; fromfirst != fromlast && tofirst != tolast;)
						   ...
						   (fromscheme == eol_scheme::cr && toscheme == eol_scheme::lf))
		{
			...
			{
				auto [fromit,
					  toit]{::ufio::details::simd_lf_cr_process_chars<cr>(fromfirst, fromlast, tofirst, tolast)};
				fromfirst = fromit;
				tofirst = toit;
			}
			for (; fromfirst != fromlast && tofirst != tolast;)
...
using native_eol =
	...
	lf_crlf
#else
	::ufio::decorators::no_op_decorators
#endif
	;
