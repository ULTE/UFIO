				  ...
				  output_char_type *tolast)
	{
		if constexpr (!::std::same_as<char_type, input_char_type>)
		{
...
#endif
			{
				...
				::ufio::details::local_operator_new_array_ptr<input_char_type> fromptr(fromdiff);
				::ufio::details::non_overlapped_copy_n(fromfirst, fromdiff, fromptr.ptr);
		...
		else
		{
			...
			if (fromdiff < diff)
			{
				auto remainedstop{::ufio::details::non_overlapped_copy_n(fromfirst, fromdiff, remainedit)};
				auto [fromit, toit] = deco.process_chars(remained_buffer, remainedstop, tofirst, tolast);
				tofirst = toit;
				fromfirst += fromdiff;
				remained = static_cast<::std::size_t>(
					::ufio::freestanding::copy_n(fromit, static_cast<::std::size_t>(remainedstop - fromit),
													remained_buffer) -
					remained_buffer);
			...
			if (remained)
			{
				::ufio::details::non_overlapped_copy_n(fromfirst, diff, remainedit);
			...
			if (fromit != fromlast && toit != tolast)
			{
				...
				::ufio::details::non_overlapped_copy_n(fromit, remained, remained_buffer);
				fromit = fromlast;
			}
			return {fromit, toit};
		}
