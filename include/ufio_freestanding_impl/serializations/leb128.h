...
{
	if constexpr (::ufio::details::my_unsigned_integral<T>)
	{
		if constexpr (sizeof(T) <= sizeof(unsigned))
		{
			return ::ufio::manipulators::basic_leb128_get_put<unsigned>{static_cast<unsigned>(t)};
		}
		else if constexpr (sizeof(T) <= sizeof(::std::size_t))
		{
			return ::ufio::manipulators::basic_leb128_get_put<::std::size_t>{static_cast<::std::size_t>(t)};
		}
		else
		{
			return ::ufio::manipulators::basic_leb128_get_put<::std::remove_cvref_t<T>>{t};
		}
	}
	else
	{
		...
		{
			return ::ufio::manipulators::basic_leb128_get_put<int>{static_cast<int>(t)};
		}
		else if constexpr (sizeof(T) <= sizeof(::std::ptrdiff_t))
		{
			return ::ufio::manipulators::basic_leb128_get_put<::std::ptrdiff_t>{static_cast<::std::ptrdiff_t>(t)};
		}
		else
		{
			return ::ufio::manipulators::basic_leb128_get_put<::std::remove_cvref_t<T>>{t};
...
template <::std::integral char_type, ::ufio::details::my_integral T>
inline constexpr ::std::size_t
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::basic_leb128_get_put<T>>) noexcept
{
	return ::ufio::details::leb128_length_val<T>;
}
...
print_reserve_define(io_reserve_type_t<char_type, ::ufio::manipulators::basic_leb128_get_put<T>>, char_type *iter,
					 ::ufio::manipulators::basic_leb128_get_put<T> v) noexcept
{
	return ::ufio::details::pr_rsv_leb128_impl(iter, v.reference);
}
...
namespace details
{
template <::std::integral char_type, ::ufio::details::my_integral I>
...
{
	using unsigned_char_type = ::std::make_unsigned_t<char_type>;
	using U = ::ufio::details::my_make_unsigned_t<I>;
	...
	for (; begin != end; cnt += 7)
	{
		...
		++begin;
		if constexpr (::ufio::details::my_signed_integral<I>)
		{
...
{
	return ::ufio::details::scn_ctx_define_leb128_impl(state.group_count, begin, end, *t.reference);
}
...
{
	return ::ufio::details::scn_cnt_define_leb128_impl(begin, end, *t.reference);
}
