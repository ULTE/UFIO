...
{
	using t1aliastype =
		::std::conditional_t<::ufio::details::alias_return_lvalue_ref<T1>,
							 ::std::conditional_t<::ufio::details::cond_value_transferable<T1>,
												  ::std::remove_cvref_t<T1>, ::std::remove_cvref_t<T1> const &>,
							 ::std::remove_cvref_t<decltype(ufio::io_print_alias(::std::forward<T1>(t1)))>>;
	using t2aliastype =
		::std::conditional_t<::ufio::details::alias_return_lvalue_ref<T2>,
							 ::std::conditional_t<::ufio::details::cond_value_transferable<T2>,
												  ::std::remove_cvref_t<T2>, ::std::remove_cvref_t<T2> const &>,
							 ::std::remove_cvref_t<decltype(ufio::io_print_alias(::std::forward<T2>(t2)))>>;
	...
	if constexpr (type_match)
	{
		if (pred)
		{
			return ::ufio::io_print_alias(::std::forward<T1>(t1));
		}
		else
		{
			return ::ufio::io_print_alias(::std::forward<T2>(t2));
		}
	}
	else
	{
		...
		{
			return condition<t2aliastype, t1aliastype>{::ufio::io_print_alias(::std::forward<T2>(t2)),
													   ::ufio::io_print_alias(::std::forward<T1>(t1)), !pred};
		}
		else
		{
			return condition<t1aliastype, t2aliastype>{::ufio::io_print_alias(::std::forward<T1>(t1)),
													   ::ufio::io_print_alias(::std::forward<T2>(t2)), pred};
		}
	}
}
...
template <typename T1, typename T2>
concept cond_transferable_value = ::std::is_trivially_copyable_v<::ufio::manipulators::condition<T1, T2>> &&
#if (defined(_WIN32) && !defined(__WINE__)) || defined(__CYGWIN__)
								  sizeof(::ufio::manipulators::condition<T1, T2>) <= 8u
#else
								  sizeof(::ufio::manipulators::condition<T1, T2>) <= (sizeof(::std::size_t) * 2)
#endif
	;
} // namespace details
...
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>) noexcept
{
...
print_scatter_define(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>,
					 ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
...
inline constexpr basic_io_scatter_t<char_type>
print_scatter_define(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>,
					 ::ufio::manipulators::condition<T1, T2> const &c)
{
	if (c.pred)
	{
		return {print_scatter_define(io_reserve_type<char_type, T1>, c.t1)};
...
template <::std::integral char_type, typename T1, typename T2>
	requires((details::cond_ok_dynamic_rsv_printable_impl<char_type, T1> &&
			 ...
			 details::cond_transferable_value<T1, T2>)
inline constexpr ::std::size_t
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>,
				   ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
		return ::ufio::details::cond_print_reserve_size_impl<char_type, T1>(c.t1);
	}
	else
	{
		return ::ufio::details::cond_print_reserve_size_impl<char_type, T2>(c.t2);
	}
}
...
print_reserve_define(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>, char_type *iter,
					 ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
		return ::ufio::details::cond_print_reserve_define_impl<char_type, T1>(iter, c.t1);
	}
	else
	{
		return ::ufio::details::cond_print_reserve_define_impl<char_type, T2>(iter, c.t2);
	}
}
...
template <::std::integral char_type, typename T1, typename T2, typename bop>
	requires((details::cond_ok_printable_impl<char_type, T1> && details::cond_ok_printable_impl<char_type, T2>) &&
			 (!(details::cond_ok_dynamic_rsv_printable_impl<char_type, T1> &&
				details::cond_ok_dynamic_rsv_printable_impl<char_type, T2>)) &&
			 details::cond_transferable_value<T1, T2>)
inline constexpr void print_define(io_reserve_type_t<char_type, ::ufio::manipulators::condition<T1, T2>>, bop b,
								   ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
		::ufio::operations::print_freestanding<false>(b, c.t1);
	}
	else
	{
		::ufio::operations::print_freestanding<false>(b, c.t2);
	}
}
