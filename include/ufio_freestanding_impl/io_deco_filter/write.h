																...
																typename outstmtype::output_char_type *bufferlast)
{
	while (first != last)
	{
		...
		::ufio::operations::decay::write_all_decay(outstm, bufferfirst, bufferit);
		first = it;
	}
}
														 ...
														 output_char_type const *first, output_char_type const *last)
{
	...
	::ufio::details::buffer_alloc_arr_ptr<typename outstmtype::output_char_type, false, allocator_type> arrptr(
		output_buffer_size);
	::ufio::details::decowrite_all_overflow_common_define_impl(outstm, outdeco, first, last, arrptr.ptr,
												   ...
												   ::std::size_t n, ::std::size_t output_buffer_size)
{
	...
	for (::std::size_t i{}; i != n; ++i)
	{
		...
		::ufio::details::decowrite_all_overflow_common_define_impl(outstm, outdeco, base, base + sn, arrptr.ptr,
																	  arrptr.ptr + output_buffer_size);
	}
}
...
inline constexpr void deco_scatter_write_all_overflow_define_impl(outstmtype outstm, outdecotype outdeco,
																  ...
																  ::std::size_t n)
{
	::ufio::details::deco_scatter_write_all_overflow_common_define_impl<allocator_type>(outstm, outdeco, scatters, n,
																						   output_buffer_size);
}
						  ...
						  typename basic_io_deco_filter_ref<io_buffer_type>::output_char_type const *last)
{
	...
	::ufio::details::decowrite_all_overflow_define_impl<typename io_buffer_type::traits_type::allocator_type,
														   io_buffer_type::traits_type::output_buffer_size>(
		::ufio::operations::output_stream_ref(idoref.handle),
		::ufio::operations::refs::output_decorators_ref(idoref.decorators), first, last);
}
...
{
	auto &idoref{*filter.idoptr};
	::ufio::details::deco_scatter_write_all_overflow_define_impl<
		typename io_buffer_type::traits_type::allocator_type, io_buffer_type::traits_type::output_buffer_size>(
		::ufio::operations::output_stream_ref(idoref.handle),
		::ufio::operations::refs::output_decorators_ref(idoref.decorators), scatters, n);
}
