...
public:
	...
	inline explicit constexpr basic_io_deco_filter(decotype &&d, Args &&...args)
		: handle(::std::forward<Args>(args)...), decorators(::ufio::freestanding::forward<decotype>(d))
	{
	}
	...
	{
		::ufio::details::clear_basic_io_filter_pointers(*this);
		if constexpr (::ufio::details::has_reopen_impl<handle_type, Args...>)
		{
	...
	{
		::ufio::details::clear_basic_io_filter_pointers(*this);
		if constexpr (::ufio::details::has_close_impl<handle_type>)
		{
			handle.close();
		}
		else
	...
	{
		::ufio::details::destroy_basic_io_filter(*this);
