...
inline constexpr bool execution_charset_is(char const (&str)[N1], char8_t const (&encoding)[N2]) noexcept
{
	...
	else
	{
		...
		{
			char8_t ch{::ufio::char_category::to_c_upper(static_cast<char8_t>(str[i]))};
			char8_t ch1{::ufio::char_category::to_c_upper(static_cast<char8_t>(encoding[i]))};
			if (ch != ch1)
			{
				return false;
			}
...
inline constexpr encoding_scheme execution_charset_encoding_scheme() noexcept
{
	...
	if constexpr (::ufio::details::is_ebcdic<char_type_no_cvref_t>)
	{
	...
	else
	{
		if constexpr (::std::same_as<char_type_no_cvref_t, char>)
		{
			...
			if constexpr (::ufio::details::execution_charset_is(__GNUC_EXECUTION_CHARSET_NAME, u8"GB18030") ||
						  ::ufio::details::execution_charset_is(__GNUC_EXECUTION_CHARSET_NAME, u8"GBK"))
			{
				return encoding_scheme::gb18030;
			}
			else
		...
		else if constexpr (::std::same_as<char_type_no_cvref_t, wchar_t>)
		{
			...
			if constexpr (sizeof(wchar_t) == 1 &&
						  (::ufio::details::execution_charset_is(__GNUC_WIDE_EXECUTION_CHARSET_NAME, u8"GB18030") ||
						   ::ufio::details::execution_charset_is(__GNUC_WIDE_EXECUTION_CHARSET_NAME, u8"GBK")))
			{
				return encoding_scheme::gb18030;
			}
			else
			{
				if constexpr (::ufio::details::wide_is_none_utf_endian)
				{
