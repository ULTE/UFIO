...
{
	::ufio::details::decay::print_control_single<false>(buffer, arg);
#if 0
	::ufio::details::decay::print_control_fallback_single(buffer, arg);
#endif
	...
	{
		::ufio::parse_code p{scan_context_eof_define(io_reserve_type<char_type, T>, s, t)};
		if (p != ::ufio::parse_code::ok)
		{
			::ufio::throw_parse_code(p);
		}
	}
}
...
concept inplace_to_decay_detect =
	...
	(sizeof...(Args) != 0 &&
	 ::ufio::operations::decay::defines::print_freestanding_params_okay<char_type, Args...> &&
...
{
	constexpr bool failed{::ufio::details::inplace_to_decay_detect<char_type, T, Args...>};
	if constexpr (failed)
	{
					   ...
					   ...))
		{
						  ...
						  (!(contiguous_scannable<char_type, T> && sizeof...(args) == 1)))
			{
				...
				{
					::ufio::details::inplace_to_decay_buffer_scatter_context_impl<char_type>(state, t, args...);
				}
				else if constexpr (no_need_dynamic_reserve)
				{
					constexpr ::std::size_t maximum_reserve_size{
						::ufio::details::calculate_print_normal_maxium_size<char_type, false, Args...>()};
					char_type buffer[maximum_reserve_size];
					::ufio::details::inplace_to_decay_buffer_context_impl<char_type>(buffer, state, t, args...);
				}
				else
				{
					::std::size_t const maximum_reserve_size{
						::ufio::details::calculate_print_normal_dynamic_maxium_main<char_type, false>(0, args...)};
					::ufio::details::local_operator_new_array_ptr<char_type> heap_buffer(maximum_reserve_size);
					::ufio::details::inplace_to_decay_buffer_context_impl<char_type>(heap_buffer.ptr, state, t,
																						args...);
				}
			}
			else if constexpr (contiguous_scannable<char_type, T>)
			{
				...
				{
					::ufio::details::to_deal_with_contiguous_single_scatter<char_type>(t, args...);
				}
				else if constexpr (((reserve_printable<char_type, Args>) && ...))
				{
					constexpr ::std::size_t total_size{
						::ufio::details::decay::calculate_scatter_reserve_size<char_type, Args...>()};
					char_type buffer[total_size];
					auto ret{::ufio::details::to_impl_with_reserve_recursive(buffer, args...)};
					::ufio::details::deal_with_single_to<char_type>(buffer, ret, t);
				}
				else
				{
					::std::size_t const maximum_reserve_size{
						::ufio::details::calculate_scatter_dynamic_reserve_size_with_scatter<char_type>(args...)};
					::ufio::details::local_operator_new_array_ptr<char_type> heap_buffer(maximum_reserve_size);
					auto ret{::ufio::details::to_impl_with_reserve_recursive(heap_buffer.ptr, args...)};
					::ufio::details::deal_with_single_to<char_type>(heap_buffer.ptr, ret, t);
				}
			}
		}
		else
		{
			...
			auto ref{::ufio::operations::output_stream_ref(buffer)};
			if constexpr (context_scannable<char_type, T> &&
						  (!(contiguous_scannable<char_type, T> && sizeof...(args) == 1)))
			{
				typename ::std::remove_cvref_t<decltype(scan_context_type(io_reserve_type<char_type, T>))>::type state;
				::ufio::details::inplace_to_decay_context_impl(ref, state, t, args...);
			}
			else if constexpr (contiguous_scannable<char_type, T>)
			{
				::ufio::operations::decay::print_freestanding_decay<false>(ref, args...);
				::ufio::details::deal_with_single_to<char_type>(buffer.buffer_begin, buffer.buffer_curr, t);
			}
			else
			{
...
template <::std::integral char_type, typename T, typename... Args>
	requires ::ufio::details::inplace_to_decay_detect<char_type, T, Args...>
...
concept can_do_inplace_to = requires(T &t, Args &&...args) {
	::ufio::details::basic_inplace_to_decay_model<char_type>(
		::ufio::io_scan_forward<char_type>(::ufio::io_scan_alias(t)),
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<char_type, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<char_type>(::ufio::io_scan_forward<char_type>(::ufio::io_scan_alias(t)),
													 io_print_forward<char_type>(io_print_alias(args))...);
	}
	else
	{
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<char, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<char>(::ufio::io_scan_forward<char>(::ufio::io_scan_alias(t)),
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<wchar_t, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<wchar_t>(::ufio::io_scan_forward<wchar_t>(::ufio::io_scan_alias(t)),
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<char8_t, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<char8_t>(::ufio::io_scan_forward<char8_t>(::ufio::io_scan_alias(t)),
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<char16_t, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<char16_t>(::ufio::io_scan_forward<char16_t>(::ufio::io_scan_alias(t)),
...
{
	constexpr bool failed{::ufio::details::can_do_inplace_to<char32_t, T, Args...>};
	if constexpr (failed)
	{
		::ufio::basic_inplace_to_decay<char32_t>(::ufio::io_scan_forward<char32_t>(::ufio::io_scan_alias(t)),
...
inline constexpr T basic_to_decay(Args... args)
{
	...
	else if constexpr (failed)
	{
		...
		{
			T v{};
			basic_inplace_to_decay<char_type>(::ufio::io_scan_forward<char_type>(::ufio::io_scan_alias(v)),
											  args...);
			return v;
		}
		else
...
inline constexpr T basic_to(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<char_type, T>(io_print_forward<char_type>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
...
[[nodiscard]] inline constexpr T to(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<char, T>(io_print_forward<char>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
...
[[nodiscard]] inline constexpr T wto(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<wchar_t, T>(io_print_forward<wchar_t>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
...
[[nodiscard]] inline constexpr T u8to(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<char8_t, T>(io_print_forward<char8_t>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
...
[[nodiscard]] inline constexpr T u16to(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<char16_t, T>(io_print_forward<char16_t>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
...
[[nodiscard]] inline constexpr T u32to(Args &&...args)
{
	...
	if constexpr (failed)
	{
		return ::ufio::decay::basic_to_decay<char32_t, T>(io_print_forward<char32_t>(io_print_alias(args))...);
	}
	else
	{
		static_assert(failed, "either somes args not printable or some type not detectable");
