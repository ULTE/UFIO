...
inline constexpr void transmit_all_main_impl(optstmtype optstm, instmtype instm, ::ufio::uintfpos_t totransmit)
{
	/*
	...
	*/
	...
	while (totransmit)
	{
		...
		::ufio::operations::decay::read_all_decay(instm, buffer_start, iter);
...
template <typename optstmtype, typename instmtype>
inline constexpr decltype(auto) transmit_all_decay(optstmtype optstm, instmtype instm, ::ufio::uintfpos_t totransmit)
{
#if 0
	...
	if constexpr (::ufio::operations::decay::defines::has_output_or_io_stream_mutex_ref_define<optstmtype>)
	{
		...
		return ::ufio::operations::decay::transmit_all_decay(
	...
	{
		return ::ufio::details::transmit_all_main_impl(optstm, instm, totransmit);
	}
}
...
template <typename optstmtype, typename instmtype>
inline constexpr decltype(auto) transmit_all(optstmtype &&optstm, instmtype &&instm, ::ufio::uintfpos_t totransmit)
{
	return ::ufio::operations::decay::transmit_all_decay(::ufio::operations::output_stream_ref(optstm),
															::ufio::operations::input_stream_ref(instm), totransmit);
}
