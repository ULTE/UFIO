...
[[nodiscard]] inline constexpr bool scan_single_impl(input in, T arg)
{
...
#endif
	{
		if constexpr (precise_reserve_scannable<char_type, T>)
		{
			constexpr ::std::size_t n{scan_precise_reserve_size(io_reserve_type<char_type, T>)};
			char_type buffer[n];
			if constexpr (::ufio::details::asan_state::current == ::ufio::details::asan_state::activate)
			{
				char_type const *p{buffer};
				::ufio::operations::decay::read_all_decay(in, buffer, buffer + n);
				if constexpr (precise_reserve_scannable_no_error<char_type, T>)
				{
...
{
	if constexpr (::ufio::operations::decay::defines::has_status_scan_define<input>)
	{
		return status_scan_define(instm, args...);
	}
	else if constexpr (::ufio::operations::decay::defines::has_input_or_io_stream_mutex_ref_define<input>)
	{
		::ufio::operations::decay::stream_ref_decay_lock_guard lg{
			::ufio::operations::decay::input_stream_mutex_ref_decay(instm)};
		return scan_freestanding_decay(::ufio::operations::decay::input_stream_unlocked_ref_decay(instm), args...);
	}
	else if constexpr (::ufio::operations::decay::defines::has_ibuffer_basic_operations<input>)
	{
		return (::ufio::details::scan_single_impl(instm, args) && ...);
	}
	else if constexpr (::ufio::operations::defines::available_add_ibuf<input>)
	{
		static_assert(::ufio::operations::decay::defines::has_status_scan_define<input>, 
			"If you want to scan this type of file, please add ::ufio::basic_ibuf. "
			"At the same time, please ensure that this type of file is not blocked, or scanning will block the endless loop.");
		return false;
	}
	else
	{
		static_assert(::ufio::operations::decay::defines::has_status_scan_define<input>, "type not scannable.");
		return false;
	}
}
