																		...
																		::std::size_t n)
{
	...
	if constexpr (::ufio::operations::decay::defines::has_scatter_write_some_bytes_overflow_define<outstmtype>)
	{
	...
	{
		::ufio::details::scatter_write_all_bytes_cold_impl(outsm, pscatters, n);
		return {n, 0};
	}
						...
						::ufio::operations::decay::defines::has_write_some_overflow_define<outstmtype>))
	{
		...
		return ::ufio::details::scatter_write_some_cold_impl(
							...
							outstmtype>))
	{
		...
		::ufio::operations::decay::output_stream_seek_bytes_decay(
			outsm, ::ufio::fposoffadd_scatters(0, pscatters, ret), ::ufio::seekdir::cur);
		return ret;
	}
	else if constexpr (sizeof(char_type) == 1 &&
						...
						::ufio::operations::decay::defines::has_scatter_pwrite_some_overflow_define<outstmtype>))
	{
		...
		::ufio::operations::decay::output_stream_seek_decay(outsm, ::ufio::fposoffadd_scatters(0, pscatters, ret),
															   ::ufio::seekdir::cur);
		return ret;
	}
}
																   ...
																   ::std::size_t n)
{
	if constexpr (::ufio::operations::decay::defines::has_output_or_io_stream_mutex_ref_define<outstmtype>)
	{
		...
		return ::ufio::details::scatter_write_some_bytes_impl(
	...
	else if constexpr (::ufio::operations::decay::defines::has_obuffer_basic_operations<outstmtype>)
	{
		...
		for (; i != e; ++i)
		{
...
#endif
			{
				using char_type_const_ptr
					...
					= char_type const *;
				curr =
					::ufio::details::non_overlapped_copy_n(reinterpret_cast<char_type_const_ptr>(base), len, curr);
				buffptrdiff -= len;
			}
			else
			{
		...
		{
			auto ret{
				::ufio::details::scatter_write_some_bytes_cold_impl(outsm, i, static_cast<::std::size_t>(e - i))};
	...
	{
		return ::ufio::details::scatter_write_some_bytes_cold_impl(outsm, pscatters, n);
	}
}
...
{
	using char_type = typename outstmtype::output_char_type;
	if constexpr (::ufio::operations::decay::defines::has_scatter_write_all_bytes_overflow_define<outstmtype>)
	{
	...
	else if constexpr (::ufio::operations::decay::defines::has_write_all_bytes_overflow_define<outstmtype>)
	{
		for (auto i{pscatters}, e{pscatters + n}; i != e; ++i)
		{
			...
			::ufio::details::write_all_bytes_impl(outsm, base, base + len);
		}
	}
	else if constexpr (::ufio::operations::decay::defines::has_scatter_write_some_bytes_overflow_define<outstmtype>)
	{
		for (;;)
		{
			auto ret{::ufio::details::scatter_write_some_bytes_impl(outsm, pscatters, n)};
			...
			if (pisc)
			{
				...
				::ufio::details::write_all_bytes_impl(outsm, base + pisc, base + pi.len);
				++retpos;
			}
			pscatters += retpos;
			n -= retpos;
...
#endif
			= basic_io_scatter_t<char_type> const *;
		::ufio::details::scatter_write_all_impl(outsm, reinterpret_cast<scattermayalias_const_ptr>(pscatters), n);
	}
	else if constexpr (::ufio::operations::decay::defines::has_output_or_io_stream_seek_bytes_define<outstmtype> &&
					   (::ufio::operations::decay::defines::has_pwrite_all_bytes_overflow_define<outstmtype> ||
						::ufio::operations::decay::defines::has_scatter_pwrite_all_bytes_overflow_define<
...
inline constexpr void scatter_write_all_bytes_impl(outstmtype outsm, io_scatter_t const *pscatters, ::std::size_t n)
{
	if constexpr (::ufio::operations::decay::defines::has_output_or_io_stream_mutex_ref_define<outstmtype>)
	{
		::ufio::operations::decay::stream_ref_decay_lock_guard lg{
			::ufio::operations::decay::output_stream_mutex_ref_decay(outsm)};
		return ::ufio::details::scatter_write_all_bytes_impl(
			::ufio::operations::decay::output_stream_unlocked_ref_decay(outsm), pscatters, n);
	}
	else if constexpr (::ufio::operations::decay::defines::has_obuffer_basic_operations<outstmtype>)
	{
...
#endif
		{
			return ::ufio::details::scatter_write_all_bytes_cold_impl(outsm, i, static_cast<::std::size_t>(e - i));
		}
	}
	else
	{
		return ::ufio::details::scatter_write_all_bytes_cold_impl(outsm, pscatters, n);
	}
}
