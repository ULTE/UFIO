...
inline constexpr contiguous_scatter_result find_continuous_scatters_n()
{
	...
	if constexpr (::ufio::scatter_printable<char_type, Arg>)
	{
		...
		ret.neededscatters = ::ufio::details::intrinsics::add_or_overflow_die_chain(ret.neededscatters,
																					   static_cast<::std::size_t>(1));
	}
	else if constexpr (::ufio::reserve_printable<char_type, Arg>)
	{
		if constexpr (sizeof...(Args) != 0)
		{
			ret = find_continuous_scatters_n<char_type, Args...>();
		...
		constexpr ::std::size_t sz{print_reserve_size(::ufio::io_reserve_type<char_type, Arg>)};
		static_assert(sz != 0);
		++ret.position;
		ret.neededscatters = ::ufio::details::intrinsics::add_or_overflow_die_chain(ret.neededscatters,
																					   static_cast<::std::size_t>(1));
		ret.neededspace = ::ufio::details::intrinsics::add_or_overflow_die_chain(ret.neededspace, sz);
		ret.hasreserve = true;
	}
	else if constexpr (::ufio::dynamic_reserve_printable<char_type, Arg>)
	{
		if constexpr (sizeof...(Args) != 0)
		{
			ret = find_continuous_scatters_n<char_type, Args...>();
	...
	else if constexpr (::ufio::reserve_scatters_printable<char_type, Arg>)
	{
		if constexpr (sizeof...(Args) != 0)
		{
			ret = find_continuous_scatters_n<char_type, Args...>();
		}
		constexpr auto scatszres{print_reserve_scatters_size(::ufio::io_reserve_type<char_type, Arg>)};
		static_assert(scatszres.scatters_size != 0);
		ret.hasscatters = true;
		ret.hasreserve = true;
		++ret.position;
		ret.neededspace =
			::ufio::details::intrinsics::add_or_overflow_die_chain(ret.neededspace, scatszres.reserve_size);
		ret.neededscatters =
			::ufio::details::intrinsics::add_or_overflow_die_chain(ret.neededscatters, scatszres.scatters_size);
	}
	return ret;
}
...
{
	if constexpr (::ufio::reserve_printable<char_type, Arg> && !findscatter)
	{
		constexpr ::std::size_t sz{print_reserve_size(::ufio::io_reserve_type<char_type, Arg>)};
		if constexpr (sizeof...(Args) == 0)
		{
		...
		else
		{
			...
			return {res.position + 1, ::ufio::details::intrinsics::add_or_overflow_die_chain(res.neededspace, sz)};
		}
	}
	else if constexpr (::ufio::scatter_printable<char_type, Arg> && findscatter)
	{
		if constexpr (sizeof...(Args) == 0)
		{
			return {1, 0};
...
inline constexpr bool minimum_buffer_output_stream_require_size_constant_impl =
	(N < obuffer_minimum_size_define(::ufio::io_reserve_type<typename output::output_char_type, output>));

template <typename output, ::std::size_t N>
concept minimum_buffer_output_stream_require_size_impl =
	::ufio::operations::decay::defines::has_obuffer_minimum_size_operations<output> &&
	minimum_buffer_output_stream_require_size_constant_impl<output, N>;

template <::std::size_t sz>
	requires(sz != 0)
inline constexpr void scatter_rsv_update_times(::ufio::io_scatter_t *first, ::ufio::io_scatter_t *last) noexcept
{
	if constexpr (sz != 1)
	{
...
inline ::ufio::details::decay::basic_reserve_scatters_define_byte_result<char_type>
...
#endif
		= io_scatter_t *;
	auto result{print_reserve_scatters_define(::ufio::io_reserve_type<char_type, ::std::remove_cvref_t<T>>,
...
{
	return ::ufio::details::decay::prrsvsct_byte_common_rsvsc_impl(pscatters, buffer, t).scatters_pos_ptr;
}
...
inline constexpr void print_control_single(output outstm, T t)
{
	...
	constexpr bool asan_activated{::ufio::details::asan_state::current == ::ufio::details::asan_state::activate};
	...
	if constexpr (scatter_printable<char_type, value_type>)
	{
		...
		{
			scatter=print_scatter_define(::ufio::io_reserve_type<char_type,value_type>,t);
		}
		if constexpr(line)
		{
			...
			else if constexpr(::ufio::operations::decay::defines::has_obuffer_basic_operations<output>)
			{
				auto curr=obuffer_curr(out);
				auto end=obuffer_end(out);
				::std::size_t const len{scatter.len};
		...
		if constexpr (line)
		{
			if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<
							  output>)
			{
				::ufio::io_scatter_t scatters[2]{
					{t.base, t.len * sizeof(char_type)},
					{__builtin_addressof(char_literal_v<u8'\n', char_type>), sizeof(char_type)}};
				::ufio::operations::decay::scatter_write_all_bytes_decay(outstm, scatters, 2);
			}
			else
			{
				::ufio::basic_io_scatter_t<char_type> scatters[2]{
					t, {__builtin_addressof(char_literal_v<u8'\n', char_type>), 1}};
				::ufio::operations::decay::scatter_write_all_decay(outstm, scatters, 2);
			}
		}
		else
		{
			::ufio::operations::decay::write_all_decay(outstm, t.base, t.base + t.len);
		}
	}
	else if constexpr (reserve_printable<char_type, value_type>)
	{
		constexpr ::std::size_t real_size{print_reserve_size(::ufio::io_reserve_type<char_type, value_type>)};
		...
		if constexpr(contiguous_output_stream<output>)
		{
			...
			if(diff<size)[[unlikely]]
				fast_terminate();
			auto it{print_reserve_define(::ufio::io_reserve_type<char_type,value_type>,bcurr,t)};
			if constexpr(line)
			{
				*it=lfch;
				++it;
...
#endif
		{
			if constexpr (::ufio::operations::decay::defines::has_obuffer_basic_operations<output> &&
						  !asan_activated)
			{
				...
				if constexpr (minimum_buffer_output_stream_require_size_impl<output, size>)
				{
					...
					bcurr = print_reserve_define(::ufio::io_reserve_type<char_type, value_type>, bcurr, t);
					if constexpr (line)
					{
						*bcurr = lfch;
						++bcurr;
				...
				else
				{
					...
					{
						::ufio::operations::decay::write_all_decay(outstm, buffer, bcurr);
					}
				}
			}
			else
			{
				char_type buffer[size];
				char_type *i{print_reserve_define(::ufio::io_reserve_type<char_type, value_type>, buffer, t)};
				if constexpr (line)
				{
					*i = lfch;
					++i;
				}
				::ufio::operations::decay::write_all_decay(outstm, buffer, i);
			}
		}
	}
	else if constexpr (dynamic_reserve_printable<char_type, value_type>)
	{
		::std::size_t size{print_reserve_size(::ufio::io_reserve_type<char_type, value_type>, t)};
		if constexpr (line)
		{
		...
		if constexpr(contiguous_output_stream<output>)
		{
			auto bcurr{obuffer_curr(outstm)};
			auto bend{obuffer_end(outstm)};
			auto it{print_reserve_define(::ufio::io_reserve_type<char_type,value_type>,bcurr,t,size)};
			::std::size_t diff{static_cast<::std::size_t>(bend-bcurr)};
			if(diff<size)[[unlikely]]
				fast_terminate();
			if constexpr(line)
...
#endif
		{
						  ...
						  !asan_activated)
			{
				...
				auto toptr{curr};
				bool smaller{static_cast<::std::ptrdiff_t>(size) < diff};
				::ufio::details::local_operator_new_array_ptr<char_type> newptr;
				if (!smaller)
#if __has_cpp_attribute(unlikely)
					[[unlikely]]
#endif
				{
					newptr.ptr = toptr = ::ufio::details::allocate_iobuf_space<
						char_type,
						typename ::ufio::details::local_operator_new_array_ptr<char_type>::allocator_type>(size);
					newptr.size = size;
				}

				auto it{print_reserve_define(::ufio::io_reserve_type<char_type, value_type>, toptr, t)};
				if constexpr (line)
				{
					*it = lfch;
					++it;
...
#endif
				{
					::ufio::operations::decay::write_all_decay(outstm, toptr, it);
				}
			}
			else
			{
				::ufio::details::local_operator_new_array_ptr<char_type> newptr(size);
				auto it{print_reserve_define(::ufio::io_reserve_type<char_type, value_type>, newptr.ptr, t)};
				if constexpr (line)
				{
					*it = lfch;
					++it;
				}
				::ufio::operations::decay::write_all_decay(outstm, newptr.ptr, it);
			}
		}
	}
	else if constexpr (reserve_scatters_printable<char_type, value_type>)
	{
		constexpr auto sz{print_reserve_scatters_size(::ufio::io_reserve_type<char_type, value_type>)};
...
#endif
		{
			if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<
							  output>)
			{
				::ufio::io_scatter_t scattersbuffer[scattersnum];
				char_type buffer[sz.reserve_size];
				::ufio::io_scatter_t *ptr{
					::ufio::details::decay::prrsvsct_byte_common_impl(scattersbuffer, buffer, t)};
				if constexpr (line)
				{
					*ptr = ::ufio::details::decay::line_scatter_common<char_type, void>;
					++ptr;
				}
				::ufio::operations::decay::scatter_write_all_bytes_decay(outstm, scattersbuffer, static_cast<::std::size_t>(ptr - scattersbuffer));
				return;
			}
		}
		::ufio::basic_io_scatter_t<char_type> scattersbuffer[scattersnum];
		char_type buffer[sz.reserve_size];
		auto ptr{print_reserve_scatters_define(::ufio::io_reserve_type<char_type, ::std::remove_cvref_t<T>>,
											   scattersbuffer, buffer, t)
					 .scatters_pos_ptr};
		if constexpr (line)
		{
			*ptr = ::ufio::details::decay::line_scatter_common<char_type>;
			++ptr;
		}
		::ufio::operations::decay::scatter_write_all_decay(outstm, scattersbuffer, static_cast<::std::size_t>(ptr - scattersbuffer));
	}
	else if constexpr (::ufio::transcode_imaginary_printable<char_type, value_type>)
	{
		// todo?
	}
		...
		if constexpr (::ufio::operations::decay::defines::has_obuffer_basic_operations<output>)
		{
			for (;;)
			{
...
#endif
				{
					...
					{
						::ufio::operations::decay::output_stream_buffer_flush_decay(outstm);
						bcurr = obuffer_curr(outstm);
						bed = obuffer_end(outstm);
...
#endif
						{
							...
							for (;;)
							{
								...
								if constexpr (line)
								{
									...
									::ufio::operations::decay::write_all_decay(outstm, buffer, resit);
									if (done)
									{
										return;
									}
				...
				else
				{
...
#endif
					{
						if constexpr (line)
						{
							::ufio::operations::decay::char_put_decay(outstm, lfch);
						}
						return;
					}
				}
	...
	{
		print_define(::ufio::io_reserve_type<char_type, value_type>, outstm, t);
		if constexpr (line)
		{
			::ufio::operations::decay::char_put_decay(outstm, lfch);
		}
...
inline constexpr char_type* printrsvcontiguousimpl(char_type* iter,Arg arg,Args... args)
{
	...
	{
		constexpr
			::std::size_t sz{print_reserve_size(::ufio::io_reserve_type<char_type,Arg>)};
		return {ret.position+1,
			::ufio::details::add_overflow(ret.neededspace,sz),
...
inline constexpr char_type *print_n_reserve(char_type *ptr, T t, Args... args)
{
	...
	{
		ptr = print_reserve_define(::ufio::io_reserve_type<char_type, ::std::remove_cvref_t<T>>, ptr, t);
									   ...
									   Args... args)
{
	if constexpr (n != 0)
	{
		...
		{
			basic_io_scatter_t<char_type> sct{print_scatter_define(::ufio::io_reserve_type<char_type, T>, t)};
			if constexpr (::std::same_as<scattertype, void>)
			{
...
inline constexpr ::std::size_t ndynamic_print_reserve_size(T t, Args... args)
{
	...
	{
		if constexpr (::ufio::dynamic_reserve_printable<char_type, nocvreft>)
		{
			return print_reserve_size(::ufio::io_reserve_type<char_type, nocvreft>, t);
		}
		else
		{
			return 0;
	...
	else
	{
		if constexpr (::ufio::dynamic_reserve_printable<char_type, nocvreft>)
		{
			return ::ufio::details::intrinsics::add_or_overflow_die(
				print_reserve_size(::ufio::io_reserve_type<char_type, nocvreft>, t),
				::ufio::details::decay::ndynamic_print_reserve_size<n - 1, char_type>(args...));
		}
		else
		{
			return ::ufio::details::decay::ndynamic_print_reserve_size<n - 1, char_type>(args...);
		}
	}
}
...
{
	using nocvreft = ::std::remove_cvref_t<T>;
	if constexpr (::ufio::reserve_printable<char_type, nocvreft> ||
				  ::ufio::dynamic_reserve_printable<char_type, nocvreft>)
	{
		return true;
	}
	else
													...
													char_type *ptr, T t, Args... args)
{
	if constexpr (n != 0)
	{
		...
		{
			ptr = print_reserve_define(::ufio::io_reserve_type<char_type, nocvreft>, ptr, t);
			if constexpr (::ufio::details::decay::print_next_is_reserve<char_type, Args...>())
			{
				return ::ufio::details::decay::print_n_scatters_reserve_cont<needprintlf, n - 1, char_type>(
			...
			else
			{
				...
				if constexpr (1 < n)
				{
					return ::ufio::details::decay::print_n_scatters_reserve<needprintlf, n - 1, char_type>(
						pscatters, ptr, args...);
				}
			}
		}
											   ...
											   Args... args)
{
	if constexpr (n != 0)
	{
		...
		if constexpr (::ufio::reserve_printable<char_type, nocvreft> ||
					  ::ufio::dynamic_reserve_printable<char_type, nocvreft>)
		{
			auto ptred{print_reserve_define(::ufio::io_reserve_type<char_type, nocvreft>, ptr, t)};
			if constexpr (sizeof...(Args) != 0 &&
						  ::ufio::details::decay::print_next_is_reserve<char_type, Args...>())
			{
				return ::ufio::details::decay::print_n_scatters_reserve_cont<needprintlf, n - 1, char_type>(
		...
		else if constexpr (::ufio::scatter_printable<char_type, nocvreft>)
		{
			...
			if constexpr (n == 1 && needprintlf)
			{
				*pscatters = ::ufio::details::decay::line_scatter_common<char_type, scattertype>;
				++pscatters;
			}
			if constexpr (1 < n)
			{
				return ::ufio::details::decay::print_n_scatters_reserve<needprintlf, n - 1, char_type>(pscatters,
																										  ptr, args...);
			}
		}
		else if constexpr (::ufio::reserve_scatters_printable<char_type, nocvreft>)
		{
			if constexpr (::std::same_as<scattertype, void>)
			{
				auto pit{::ufio::details::decay::prrsvsct_byte_common_rsvsc_impl(pscatters, ptr, t)};
				if constexpr (1 < n)
				{
					return ::ufio::details::decay::print_n_scatters_reserve<needprintlf, n - 1, char_type>(
			...
			{
				auto pit{
					print_reserve_scatters_define(::ufio::io_reserve_type<char_type, nocvreft>, pscatters, ptr, t)};
				if constexpr (1 < n)
				{
					return ::ufio::details::decay::print_n_scatters_reserve<needprintlf, n - 1, char_type>(
						pit.scatters_pos_ptr, pit.reserve_pos_ptr, args...);
...
inline constexpr void print_controls_impl(outputstmtype optstm, T t, Args... args)
{
	...
	using scatter_type = ::std::conditional_t<
		::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<outputstmtype>,
		io_scatter_t, basic_io_scatter_t<char_type>>;
	constexpr contiguous_scatter_result res{
		::ufio::details::decay::find_continuous_scatters_n<char_type, T, Args...>()};
	...
	else
	{
		...
		if constexpr (res.hasscatters && !res.hasreserve && !res.hasdynamicreserve)
		{
			constexpr ::std::size_t scatterscount{res.neededscatters + static_cast<::std::size_t>(needprintlf)};
			{
				...
				::ufio::details::decay::print_n_scatters<res.position, char_type>(scatters, t, args...);
				if constexpr (needprintlf)
				{
					scatters[n] = ::ufio::details::decay::line_scatter_common<
						char_type, ::std::conditional_t<::std::same_as<scatter_type, io_scatter_t>, void, char_type>>;
				}
				if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<
								  outputstmtype>)
				{
					::ufio::operations::decay::scatter_write_all_bytes_decay(optstm, scatters, scatterscount);
				}
				else
				{
					::ufio::operations::decay::scatter_write_all_decay(optstm, scatters, scatterscount);
				}
			}
		}
		else
		{
			...
			if constexpr (!res.hasscatters)
			{
				...
				{
					::std::size_t dynsz{
						::ufio::details::decay::ndynamic_print_reserve_size<res.position, char_type>(t, args...)};
					::std::size_t totalsz{::ufio::details::intrinsics::add_or_overflow_die(mxsize, dynsz)};
					::ufio::details::local_operator_new_array_ptr<char_type> newptr(totalsz);
					char_type *buffer{newptr.ptr};
					char_type *ptred{
						::ufio::details::decay::print_n_reserve<res.position, char_type>(buffer, t, args...)};
					if constexpr (needprintlf)
					{
						*ptred = ::ufio::char_literal_v<u8'\n', char_type>;
						++ptred;
					}
					::ufio::operations::decay::write_all_decay(optstm, buffer, ptred);
				}
				else if constexpr (res.hasreserve)
				{
					if constexpr (res.neededspace == 0)
					{
						if constexpr (needprintlf)
						{
							::ufio::operations::decay::char_put_decay(optstm,
																		 ::ufio::char_literal_v<u8'\n', char_type>);
						}
					}
					else
					{
			...
			else if constexpr (res.hasreserve && !res.hasdynamicreserve)
			{
				...
				auto ptr{::ufio::details::decay::print_n_scatters_reserve<needprintlf, res.position, char_type>(
				...
				if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<
								  outputstmtype>)
				{
					::ufio::operations::decay::scatter_write_all_bytes_decay(optstm, scatters, diff);
				}
				else
				{
					::ufio::operations::decay::scatter_write_all_decay(optstm, scatters, diff);
				}
			}
...
inline constexpr void print_controls_buffer_impl(outputstmtype optstm, T t, Args... args)
{
	if constexpr (skippings != 0)
	{
		::ufio::details::decay::print_controls_buffer_impl<line, outputstmtype, skippings - 1>(optstm, args...);
	}
	else if constexpr (sizeof...(Args) == 0)
	{
		print_control_single<line>(optstm, t);
	...
	else
	{
		...
		constexpr auto scatters_result{
			::ufio::details::decay::find_continuous_scatters_reserve_n<true, char_type, T, Args...>()};
		using scatter_type = ::std::conditional_t<
			::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<outputstmtype>,
			io_scatter_t, basic_io_scatter_t<char_type>>;
		if constexpr (scatters_result.position != 0)
		{
			...
			scatter_type scatters[scatterscount];
			::ufio::details::decay::print_n_scatters<scatters_result.position, char_type>(scatters, t, args...);
			if constexpr (::ufio::operations::decay::defines::has_any_of_write_or_seek_pwrite_bytes_operations<
							  outputstmtype>)
			{
				if constexpr (needprintlf)
				{
					scatters[scatterscount - 1] = ::ufio::details::decay::line_scatter_common<char_type, void>;
				}
				::ufio::operations::decay::scatter_write_all_bytes_decay(optstm, scatters, scatterscount);
			}
			else
			{
				if constexpr (needprintlf)
				{
					scatters[scatterscount - 1] = ::ufio::details::decay::line_scatter_common<char_type>;
				}
				::ufio::operations::decay::scatter_write_all_decay(optstm, scatters, scatterscount);
			}
			if constexpr (scatters_result.position != n)
			{
				::ufio::details::decay::print_controls_buffer_impl<line, outputstmtype,
		...
		else
		{
			constexpr auto rsvresult{
				::ufio::details::decay::find_continuous_scatters_reserve_n<false, char_type, T, Args...>()};
			if constexpr (1 < rsvresult.position)
			{
				...
				if constexpr (minimum_buffer_output_stream_require_size_impl<outputstmtype, buffersize>)
				{
					...
					bcurr =
						::ufio::details::decay::print_n_reserve<rsvresult.position, char_type>(bcurr, t, args...);
					if constexpr (needprintlf)
					{
						*bcurr = ::ufio::char_literal_v<u8'\n', char_type>;
						++bcurr;
					}
				...
				else
				{
					...
					{
						::ufio::operations::decay::write_all_decay(optstm, buffer, bcurr);
					}
				}
				if constexpr (rsvresult.position != n)
				{
					::ufio::details::decay::print_controls_buffer_impl<line, outputstmtype, rsvresult.position - 1>(
						optstm, args...);
				}
			}
			else
			{
				::ufio::details::decay::print_control_single<line && sizeof...(args) == 0>(optstm, t);
				if constexpr (sizeof...(args) != 0)
				{
					::ufio::details::decay::print_controls_buffer_impl<line>(optstm, args...);
				}
			}
		}
	}
...
{
	if constexpr (::ufio::operations::decay::defines::has_status_print_define<outputstmtype>)
	{
	...
	else if constexpr (sizeof...(Args) == 0)
	{
		...
		{
			using char_type = typename outputstmtype::output_char_type;
			return ::ufio::operations::decay::char_put_decay(optstm, char_literal_v<u8'\n', char_type>);
		}
		else
		{
			return;
	...
	else if constexpr (::ufio::operations::decay::defines::has_output_or_io_stream_mutex_ref_define<outputstmtype>)
	{
		::ufio::operations::decay::stream_ref_decay_lock_guard lg{
			::ufio::operations::decay::output_stream_mutex_ref_decay(optstm)};
		return ::ufio::operations::decay::print_freestanding_decay<line>(
			::ufio::operations::decay::output_stream_unlocked_ref_decay(optstm), args...);
	}
	else if constexpr (::ufio::operations::decay::defines::has_obuffer_basic_operations<outputstmtype>)
	{
		return ::ufio::details::decay::print_controls_buffer_impl<line>(optstm, args...);
	}
	else
	{
		return ::ufio::details::decay::print_controls_impl<line>(optstm, args...);
	}
}
...
inline constexpr decltype(auto) print_freestanding_decay_cold(outputstmtype optstm, Args... args)
{
	...
	if (true) [[unlikely]]
#endif
		return ::ufio::operations::decay::print_freestanding_decay<line>(optstm, args...);
}
...
concept print_freestanding_params_okay =
	...
	((::ufio::printable<char_type, Args> || ::ufio::reserve_printable<char_type, Args> ||
	  ::ufio::dynamic_reserve_printable<char_type, Args> || ::ufio::scatter_printable<char_type, Args> ||
	  ::ufio::reserve_scatters_printable<char_type, Args> || ::ufio::context_printable<char_type, Args> ||
	  ::ufio::transcode_imaginary_printable<char_type, Args>) &&
	 ...);

template <typename output, typename... Args>
concept print_freestanding_okay =
	::ufio::operations::decay::defines::print_freestanding_params_okay<typename output::output_char_type, Args...>;
...
template <typename char_type, typename... Args>
concept print_freestanding_params_okay = ::ufio::operations::decay::defines::print_freestanding_params_okay<char_type,
																											   decltype(::ufio::io_print_forward<char_type>(::ufio::io_print_alias(::std::declval<Args>())))...>;

template <typename output, typename... Args>
concept print_freestanding_okay = ::ufio::operations::decay::defines::print_freestanding_okay<
	decltype(::ufio::operations::output_stream_ref(::std::declval<output>())),
	decltype(::ufio::io_print_forward<typename decltype(::ufio::operations::output_stream_ref(
				 ::std::declval<output>()))::output_char_type>(::ufio::io_print_alias(::std::declval<Args>())))...>;
...
{
	::ufio::operations::decay::print_freestanding_decay<line>(
		::ufio::operations::output_stream_ref(outstm),
		io_print_forward<typename decltype(::ufio::operations::output_stream_ref(outstm))::output_char_type>(
			io_print_alias(args))...);
}
