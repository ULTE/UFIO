							   ...
							   char_type const *first, char_type const *last)
{
	...
	else
	{
		...
		::ufio::details::non_overlapped_copy_n(bob.begin_ptr, to_allocate, pbuffer);
	}
	bob.begin_ptr = pbuffer;
	bob.end_ptr = pbuffer + to_allocate;
	auto curit{pbuffer + rlsz};
	bob.curr_ptr = ::ufio::details::non_overlapped_copy(first, last, curit);
}
...
inline constexpr void write_all_overflow_define(
	...
	char_type const *first, char_type const *last)
{
	::ufio::details::write_all_overflow_define_impl(*bdobr.dob_ptr, first, last);
}
...
{
	::ufio::details::grow_twice_define_impl(*bdobr.dob_ptr);
