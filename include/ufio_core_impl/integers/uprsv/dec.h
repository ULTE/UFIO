...
namespace ufio::details::uprsv::decbase
{
...
inline constexpr char_type *uprsv_handle8_impl(char_type *iter, ::std::uint_least32_t first8) noexcept
{
	...
	if constexpr (isfirst)
	{
		...
		if (first8 < UINT32_C(10000))
		{
			if (first8 < UINT32_C(100))
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan10 + (first8 << 1), tocopybytes);
				iter += 1 + (!lessthan10);
			}
			else
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan10 + (f01 << 1), tocopybytes);
				iter += 1 + (!lessthan10);
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (f2 << 1), tocopybytes);
				iter += 2;
			}
		}
		else
		{
			...
			{
				low = ::ufio::details::intrinsics::umul_least_32(first8, magic32, high);
#if __has_cpp_attribute(assume)
				[[assume(high < UINT32_C(100))]];
#endif
				bool const lessthan10{high < UINT32_C(10)};
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan10 + (high << 1), tocopybytes);
				iter += 1 + (!lessthan10);
			}
			else
			{
				::std::uint_least64_t const temp{(magic48 * first8) >> 16};
				low = ::ufio::details::intrinsics::unpack_generic(temp, high);
#if __has_cpp_attribute(assume)
				[[assume(high < UINT32_C(100))]];
#endif
				bool const lessthan10{high < UINT32_C(10)};
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan10 + (high << 1), tocopybytes);
				iter += 1 + (!lessthan10);
				low = ::ufio::details::intrinsics::umul_least_32(low, UINT32_C(100), high);
#if __has_cpp_attribute(assume)
				[[assume(high < UINT32_C(100))]];
#endif
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (high << 1), tocopybytes);
				iter += 2;
			}
			...
			high = ::ufio::details::intrinsics::umulh_least_32(low, UINT32_C(100));
#if __has_cpp_attribute(assume)
			[[assume(high < UINT32_C(100))]];
#endif
			::ufio::details::intrinsics::typed_memcpy(iter + 2, digitstb + (high << 1), tocopybytes);
	...
	else
	{
		...
		::std::uint_least32_t low{::ufio::details::intrinsics::unpack_generic(temp, high)};
#if __has_cpp_attribute(assume)
		[[assume(high < UINT32_C(100))]];
#endif
		::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (high << 1), tocopybytes);
		...
		[[assume(high < UINT32_C(100))]];
#endif
		::ufio::details::intrinsics::typed_memcpy(iter + 4, digitstb + (high << 1), tocopybytes);

		high = ::ufio::details::intrinsics::umulh_least_32(low, UINT32_C(100));
#if __has_cpp_attribute(assume)
		[[assume(high < UINT32_C(100))]];
#endif
		::ufio::details::intrinsics::typed_memcpy(iter + 6, digitstb + (high << 1), tocopybytes);
...
inline constexpr char_type *uprsv128_18digits_impl(char_type *iter, ::std::uint_least64_t value) noexcept
{
	...
	::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (a1 << 1), tocopybytes);
	iter += 2;
...
inline constexpr char_type *uprsv128_impl(char_type *iter, __uint128_t value) noexcept
{
	...
	::std::uint_least64_t low{::ufio::details::intrinsics::unpack_generic(value, high)};
...
#if __has_cpp_attribute(assume)
		constexpr auto pow10val{::ufio::details::compile_time_pow<::std::uint_least64_t>(10, 18)};
