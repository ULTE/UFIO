...
namespace ufio::details::uprsv::hexbase
{
...
inline constexpr char_type *uprsv_hex_handle8_impl(char_type *iter, ::std::uint_least32_t first4) noexcept
{
	...
	if constexpr (isfirst)
	{
		if (first4 < onex4)
		{
			if (first4 < onex2)
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan16 + (first4 << 1), tocopybytes);
				iter += 1 + (!lessthan16);
			}
			else
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + lessthan16 + (f0 << 1), tocopybytes);
				iter += 1 + (!lessthan16);
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + f2, tocopybytes);
				iter += 2;
			}
		}
		else
		{
			if (first4 < onex6)
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter + 2, digitstb + (f4 << 1), tocopybytes);
				iter += 4;
			}
			else
			{
				...
				::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (f2 << 1), tocopybytes);
				::ufio::details::intrinsics::typed_memcpy(iter + 2, digitstb + (f4 << 1), tocopybytes);
				::ufio::details::intrinsics::typed_memcpy(iter + 4, digitstb + (f6 << 1), tocopybytes);
				iter += 6;
			}
		}
	}
	else
	{
		...
		::std::uint_least32_t const f4{static_cast<::std::uint_least32_t>((first4 >> 8) & mask8)};
		::std::uint_least32_t const f6{static_cast<::std::uint_least32_t>(first4 & mask8)};
		::ufio::details::intrinsics::typed_memcpy(iter, digitstb + (f0 << 1), tocopybytes);
		::ufio::details::intrinsics::typed_memcpy(iter + 2, digitstb + (f2 << 1), tocopybytes);
		::ufio::details::intrinsics::typed_memcpy(iter + 4, digitstb + (f4 << 1), tocopybytes);
		::ufio::details::intrinsics::typed_memcpy(iter + 6, digitstb + (f6 << 1), tocopybytes);
		iter += 8;
	}
	return iter;
}
...
inline constexpr char_type *uprsv_hex64_impl(char_type *iter, ::std::uint_least64_t value) noexcept
{
	...
	::std::uint_least32_t low{::ufio::details::intrinsics::unpack_generic(value, high)};
