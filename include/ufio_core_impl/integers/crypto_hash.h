...
{
	constexpr auto tb{::ufio::details::digits_table<char_type, 16, uppercase>};
...
inline constexpr ::std::size_t print_reserve_size(io_reserve_type_t<char_type, ::ufio::basic_hex_encode<uppercase>>,
												  ::ufio::basic_hex_encode<uppercase> e) noexcept
{
	return ::ufio::details::hex_encoding_prv_size_cal(e.reference, e.last);
}

template <::std::integral char_type, bool uppercase>
inline constexpr char_type *print_reserve_define(io_reserve_type_t<char_type, ::ufio::basic_hex_encode<uppercase>>,
												 char_type *iter, ::ufio::basic_hex_encode<uppercase> e) noexcept
{
	return ::ufio::details::crypto_hash_pr_df_impl<uppercase>(e.reference, e.last, iter);
}
...
inline constexpr ::ufio::basic_hex_encode<false> hex_encode(Iter first, Iter last) noexcept
{
...
template <::std::contiguous_iterator Iter>
	requires ::std::is_trivially_copyable_v<::std::iter_value_t<Iter>>
inline constexpr ::ufio::basic_hex_encode<true> hex_encode_upper(Iter first, Iter last) noexcept
{
	if constexpr (::std::same_as<::std::iter_value_t<Iter>, ::std::byte>)
	{
		return {::std::to_address(first), ::std::to_address(last)};
...
inline constexpr ::ufio::basic_hex_encode<false> hex_encode(rg &&r) noexcept
{
...
template <::std::ranges::contiguous_range rg>
	requires ::std::is_trivially_copyable_v<::std::ranges::range_value_t<rg>>
inline constexpr ::ufio::basic_hex_encode<true> hex_encode_upper(rg &&r) noexcept
{
	return hex_encode_upper(::std::to_address(::std::ranges::begin(r)), ::std::to_address(::std::ranges::end(r)));
}

template <::std::integral char_type, typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<char_type, T> basic_as_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}

template <typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<char, T> as_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}

template <typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<wchar_t, T> was_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}

template <typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<char8_t, T> u8as_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}

template <typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<char16_t, T> u16as_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}

template <typename T>
inline constexpr ::ufio::basic_crypto_hash_as_file<char32_t, T> u32as_file(T &hashctx) noexcept
{
	return {__builtin_addressof(hashctx)};
}
...
template <::ufio::manipulators::digest_format d, ::std::size_t digest_size>
...
inline constexpr ::std::size_t cal_crypto_hash_resrv_size() noexcept
{
	...
	constexpr ::std::size_t v{d == ::ufio::manipulators::digest_format::raw_bytes ? digest_size
																					 : (digest_size << 1u)};
	return v;
}
...
template <::ufio::manipulators::digest_format d, ::std::integral char_type>
...
{
	if constexpr (d == ::ufio::manipulators::digest_format::raw_bytes)
	{
...
#endif
		{
			return ::ufio::details::non_overlapped_copy_n(reinterpret_cast<char unsigned const *>(buffer),
															 digest_size, iter);
		}
	}
	else
	{
		return ::ufio::details::crypto_hash_pr_df_impl < d == ::ufio::manipulators::digest_format::upper > (buffer, buffer + digest_size, iter);
	}
}

template <::ufio::manipulators::digest_format d, typename T, ::std::integral char_type>
inline constexpr char_type *prv_srv_hash_df_common_impl(char_type *iter, T const &t) noexcept
{
	if constexpr (::ufio::manipulators::compile_time_size_crypto_hash_context<T>)
	{
	...
	else
	{
		...
		if constexpr (::ufio::details::context_digest_byte_ptr_impl<T>)
		{
		...
		{
			::ufio::details::local_operator_new_array_ptr<::std::byte> bufferf(digest_size);
...
template <::ufio::manipulators::digest_format d, typename T, ::std::random_access_iterator Iter>
inline constexpr Iter prv_srv_hash_df_impl(Iter iter, T const &t) noexcept
{
	...
	{
		return ::ufio::details::prv_srv_hash_df_impl<d>(::std::to_address(iter), t) - ::std::to_address(iter) + iter;
	}
	else
	{
...
#endif
			{
				return ::ufio::details::prv_srv_hash_df_common_impl<d>(iter, t);
			}
			else
#endif
			{
	...
	if constexpr (context_digest_to_byte_ptr_runtime_impl<T>)
	{
		if constexpr (d == ::ufio::manipulators::digest_format::raw_bytes)
		{
...
#endif
			{
				...
				return ::ufio::details::copy_to_hash_df_commom_impl < d ==
					   ::ufio::manipulators::digest_format::upper > (buffer, iter, static_cast<::std::size_t>(ret - buffer));
			}
			else
#endif
			{
				...
				else
				{
					...
					return ::ufio::details::copy_to_hash_df_commom_impl<d>(iter, buffer,
	...
	else
	{
		if constexpr (d == ::ufio::manipulators::digest_format::raw_bytes)
		{
...
#endif
			{
				...
				return ::ufio::details::copy_to_hash_df_commom_impl < d ==
					   ::ufio::manipulators::digest_format::upper > (buffer, iter, digest_size);
			}
			else
#endif
			{
				...
				else
				{
					::std::byte buffer[digest_size];
					cal_hash_internal<T>(base, len, buffer);
					return ::ufio::details::copy_to_hash_df_commom_impl<d>(iter, buffer, digest_size);
				}
			}
		}
		else
...
template <::std::integral char_type, ::ufio::manipulators::digest_format d,
		  ::ufio::manipulators::compile_time_size_crypto_hash_context T>
	requires(static_cast<::std::size_t>(d) < static_cast<::std::size_t>(3))
inline constexpr ::std::size_t
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::hash_digest_t<d, T const &>>) noexcept
{
	return ::ufio::details::crypto_hash_resrv_size_cache<d, ::std::remove_cvref_t<T>::digest_size>;
}

template <::std::integral char_type, ::ufio::manipulators::digest_format d,
		  ::ufio::manipulators::runtime_size_crypto_hash_context T>
	requires(static_cast<::std::size_t>(d) < static_cast<::std::size_t>(3))
inline constexpr ::std::size_t
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::hash_digest_t<d, T const &>>,
				   ::ufio::manipulators::hash_digest_t<d, T const &> digest) noexcept
{
	if constexpr (d == ::ufio::manipulators::digest_format::raw_bytes)
	{
...
template <::ufio::manipulators::digest_format d, ::ufio::manipulators::crypto_hash_context T,
		  ::std::integral char_type>
inline constexpr char_type *
print_reserve_define(::ufio::io_reserve_type_t<char_type, ::ufio::manipulators::hash_digest_t<d, T const &>>,
					 char_type *iter, ::ufio::manipulators::hash_digest_t<d, T const &> t) noexcept
{
	return ::ufio::details::prv_srv_hash_df_impl<d>(iter, t.reference);
}

template <::std::integral char_type, ::ufio::manipulators::digest_format d,
		  ::ufio::manipulators::crypto_hash_context T>
	requires(static_cast<::std::size_t>(d) < static_cast<::std::size_t>(3))
inline constexpr ::std::size_t
print_reserve_size(io_reserve_type_t<char_type, ::ufio::manipulators::hash_compress_t<d, T>>) noexcept
{
	return ::ufio::details::crypto_hash_resrv_size_cache<d, ::std::remove_cvref_t<T>::digest_size>;
}
template <::ufio::manipulators::digest_format d, ::ufio::manipulators::crypto_hash_context T,
		  ::std::integral char_type>
inline constexpr char_type *
print_reserve_define(::ufio::io_reserve_type_t<char_type, ::ufio::manipulators::hash_compress_t<d, T>>,
					 char_type *iter, ::ufio::manipulators::hash_compress_t<d, T> t) noexcept
{
	return ::ufio::details::prv_srv_hash_compress_df_impl<d, T>(iter, t.base, t.len);
}
} // namespace ufio
