	...
	compile_time_type_punning_copy_n(range_type const *first, ::std::size_t bytes, ::std::byte *out)
{
...
#endif
	{
		...
		else
		{
			...
			{
				::ufio::freestanding::array<::std::byte, sizeof(range_type)> arr{
					::std::bit_cast<::ufio::freestanding::array<::std::byte, sizeof(range_type)>>(*i)};
...
#endif
	{
		::ufio::freestanding::fill_n(first, count, value);
	}
	else
#endif
	{
...
inline constexpr void my_fill(fwd_iter first, fwd_iter last, T value)
{
	...
	{
		::ufio::freestanding::fill_n(first, last - first, value);
	}
	else
	{
		::ufio::freestanding::fill(first, last, value);
	}
}
...
inline constexpr T compile_pow10{::ufio::details::compile_time_pow<::std::remove_cvref_t<T>>(10, pow)};

template <my_integral T, ::std::size_t pow>
inline constexpr T compile_pow5{::ufio::details::compile_time_pow<::std::remove_cvref_t<T>>(5, pow)};

template <my_integral T, ::std::size_t pow>
inline constexpr T compile_pow2{::ufio::details::compile_time_pow<::std::remove_cvref_t<T>>(2, pow)};
...
inline constexpr ::std::size_t cal_buffer_size()
{
	...
	static_assert(sizeof(char_type) <= UFIO_BUFFER_SIZE);
#endif
	if constexpr (iobuf)
	{
		return
	...
	else
	{
		return
#ifdef UFIO_BUFFER_SIZE
			UFIO_BUFFER_SIZE // avoid BUFSIZ macro since it is a cancer and often set incorrectly
#else
#ifdef UFIO_TRANSMIT_ON_STACK
			4096
#else
			131072u
#endif
