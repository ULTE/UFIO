	...
	else // dynamic_reserve_printable<char_type, T::value_type>
	{
		...
		for (::std::size_t i{}; i != t.size; ++i, ++curr_ptr)
		{
			retval += print_reserve_size(io_reserve_type<char_type, typename T::value_type>,
										 ::ufio::io_print_forward<char_type>(::ufio::io_print_alias(*curr_ptr)));
		}
		return retval;
	}
}
												 ...
												 char_type *__restrict ptr, sized_range_view_t<char_type, It> t)
{
	...
	{
		::ufio::freestanding::non_overlapped_copy_n(t.sep.base, t.sep.len, ptr);
		ptr += t.sep.len;
		ptr = print_reserve_define(io_reserve_type<char_type, value_type>, ptr,
								   ::ufio::io_print_forward<char_type>(::ufio::io_print_alias(*curr_ptr)));
	}
								   ...
								   range_view_t<char_type, It> t)
{
	...
	auto curr_ptr{t.begin};
	::ufio::operations::print_freestanding<false>(out, *curr_ptr);
	for (++curr_ptr; curr_ptr != t.end; ++curr_ptr)
	{
		::ufio::operations::print_freestanding<false>(out, t.sep, *curr_ptr);
	}
}
...
{
	using ::ufio::range_view_t;
	using ::ufio::sized_range_view_t;
	...
	auto printed =
		::ufio::basic_io_scatter_t<char_type>{reinterpret_cast<char_type const *>(__builtin_addressof(sep)), 1};
	if constexpr (::ufio::reserve_printable<char_type, value_type>)
	{
...
template <::std::ranges::range rg, ::ufio::constructible_to_os_c_str T>
