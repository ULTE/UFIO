...
{
	::ufio::freestanding::array<bool, 256u> tb;
...
inline constexpr bool is_c_space(char_type ch) noexcept
{
	...
	else
	{
		...
		else if constexpr (::std::same_as<char_type, wchar_t> && ::ufio::details::wide_is_none_utf_endian)
		{
...
inline constexpr Iter find_lf(Iter first, Iter last)
{
	...
	{
		return ::ufio::details::find_lf_simd_impl(::std::to_address(first), ::std::to_address(last)) -
			   ::std::to_address(first) + first;
	}
	else
	{
		return ::ufio::freestanding::find(first, last, ::ufio::char_literal_v<u8'\n', char_type>);
	}
}
...
{
	return ::ufio::details::find_space_impl<false, true>(begin, end);
}
...
{
	return ::ufio::details::find_space_impl<false, false>(begin, end);
}
...
{
	return ::ufio::details::find_space_impl<true, true>(begin, end);
}
...
{
	return ::ufio::details::find_space_impl<true, false>(begin, end);
}
