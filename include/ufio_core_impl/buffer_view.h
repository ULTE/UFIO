...
[[nodiscard]] inline constexpr Iter read(basic_ibuffer_view<ch_type>& view,Iter first,Iter last) noexcept
{
	...
	if(view_diff<diff)
		diff=view_diff;
	auto it{::ufio::details::non_overlapped_copy_n(view.curr_ptr,static_cast<::std::size_t>(view_diff),first)};
	view.curr_ptr+=diff;
	return it;
}
#endif
...
inline constexpr void write(basic_obuffer_view<ch_type>& view,Iter first,Iter last) noexcept
{
	...
	if(view_diff<diff)
		fast_terminate();
	view.curr_ptr=::ufio::details::non_overlapped_copy_n(first,static_cast<::std::size_t>(diff),view.curr_ptr);
}
#endif
