...
#if (defined(__linux__) && defined(__KERNEL__)) || defined(UFIO_USE_LINUX_KERNEL_ALLOCATOR)
#include "linux_kernel.h"
#endif

#if (defined(UFIO_ENABLE_MIMALLOC) || defined(UFIO_USE_MIMALLOC)) && (!defined(_MSC_VER) || defined(__clang__))
...
#if defined(UFIO_USE_CUSTOM_GLOBAL_ALLOCATOR)
	custom_global_allocator
#elif defined(UFIO_USE_MIMALLOC) && (!defined(_MSC_VER) || defined(__clang__))
	mimalloc_allocator
#elif (defined(__linux__) && defined(__KERNEL__)) || defined(UFIO_USE_LINUX_KERNEL_ALLOCATOR)
	linux_kmalloc_allocator
#elif (                                                                                                                \
	(__STDC_HOSTED__ == 1 && (!defined(_GLIBCXX_HOSTED) || _GLIBCXX_HOSTED == 1) && !defined(_LIBCPP_FREESTANDING)) || \
	defined(UFIO_ENABLE_HOSTED_FEATURES))
#if defined(_WIN32) && !defined(__CYGWIN__) && !defined(__WINE__) && !defined(UFIO_USE_C_MALLOC)
#if defined(_DEBUG) && defined(_MSC_VER)
	wincrt_malloc_dbg_allocator
#else
	::std::conditional_t<::ufio::asan::asan_status::current == ::ufio::asan::asan_status::none,
...
#if defined(UFIO_USE_CUSTOM_THREAD_LOCAL_ALLOCATOR)
