...
struct basic_http_header_buffer
{
	...
	inline constexpr ::ufio::manipulators::basic_os_str_known_size_without_null_terminated<char_type>
	request() const noexcept
	{
		return ::ufio::manipulators::os_str_known_size_without_null_terminated<char_type>(
...
inline constexpr auto scan_context_type(
	io_reserve_type_t<char_type,
					  ::ufio::parameter<::ufio::basic_http_header_buffer<char_type, buffer_size> &>>) noexcept
{
...
inline constexpr parse_code determine_http_header_location(basic_http_header_buffer<ch_type, buffer_size> &b) noexcept
{
	...
	{
		i = ::ufio::details::find_ch_impl<u8' ', false>(i, e);
	...
	++i;
	i = ::ufio::details::find_ch_impl<u8' ', true>(i, e);
	// for(;i!=e&&*i==char_literal_v<u8' ',ch_type>;++i);
	if (i == e)
	{
		return parse_code::invalid;
	...
	i = ::ufio::details::find_ch_impl<u8'\r', false>(i, e);
									 ...
									 basic_http_header_buffer<char_type, buffer_size> &bufferref) noexcept
{
	...
	if (state != 0)
	{
		...
		case 1:
		{
			...
			{
				state = 2u;
				if (!::ufio::details::try_copy_into_buffer(bufferref, first1, last))
				{
		...
		case 3:
		{
			...
			++first;
			if (!::ufio::details::try_copy_into_buffer(bufferref, first1, first))
			{
				return {first, parse_code::invalid};
			}
			return {first, ::ufio::details::determine_http_header_location(bufferref)};
		}
		}
	}
	state = 0;
	for (;;)
	{
		first = ::ufio::details::find_ch_impl<u8'\r', false>(first, last);
...
inline constexpr parse_result<char_type const *> scan_context_define(
	io_reserve_type_t<char_type, ::ufio::parameter<basic_http_header_buffer<char_type, buffer_size> &>>,
	http_buffer_parse_context &statetp, char_type const *first1, char_type const *last,
	::ufio::parameter<basic_http_header_buffer<char_type> &> t) noexcept
{
	return ::ufio::details::http_header_scan_context_define_impl(statetp.state, first1, last, t.reference);
}

template <::std::integral char_type, ::std::size_t buffer_size>
inline constexpr parse_code
scan_context_eof_define(io_reserve_type_t<char_type, ::ufio::parameter<basic_http_header_buffer<char_type> &>>,
						http_buffer_parse_context,
						::ufio::parameter<basic_http_header_buffer<char_type, buffer_size> &>) noexcept
{
	return parse_code::invalid;
}
...
{
	::ufio::manipulators::basic_os_str_known_size_without_null_terminated<char_type> key, value;
};
...
inline constexpr basic_http_line_generator<char_type> &operator++(basic_http_line_generator<char_type> &b) noexcept
{
	...
	b.current = ::ufio::details::find_ch_impl<u8'\n', false>(b.current, last);
	...
	line_last = ::ufio::details::find_ch_impl<u8'\r', false>(line_last, last);
	//	for(;line_last!=last&&*line_last!=char_literal_v<u8'\r',char_type>;++line_last);
	++b.current;
	b.current = ::ufio::details::find_ch_impl<u8' ', true>(b.current, line_last);
	//	for(;b.current!=line_last&&*b.current==char_literal_v<u8' ',char_type>;++b.current);
	auto curr{b.current};
	curr = ::ufio::details::find_ch_impl<u8':', false>(curr, line_last);
	...
	curr = ::ufio::details::find_ch_impl<u8' ', true>(curr, line_last);
	//	for(;curr!=line_last&&*curr==char_literal_v<u8' ',char_type>;++curr);
	b.value_start = curr;
...
inline constexpr basic_http_line<char_type> operator*(basic_http_line_generator<char_type> &b) noexcept
{
	return {::ufio::manipulators::os_str_known_size_without_null_terminated<char_type>(
			...
			::ufio::manipulators::os_str_known_size_without_null_terminated<char_type>(
