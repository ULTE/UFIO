...
#endif
	{
		::ufio::details::overlapped_copy_buffer_ptr<::std::byte> tempbuffer(n);
...
#endif
	{
		return ::ufio::details::bytes_copy_naive_n_impl(first, n, dest);
	}
	else
#endif
	{
...
{
	return ::ufio::freestanding::bytes_copy_n(first, static_cast<::std::size_t>(last - first), dest);
}
...
#endif
		{
			auto buffer{::std::bit_cast<::ufio::freestanding::array<::std::byte, sizeof(T)>>(first)};
