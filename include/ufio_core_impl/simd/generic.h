	...
	simd_vector
{
	...
	{
		(*this) = ::ufio::details::wrap_add_common(*this, other);
	}
	inline constexpr simd_vector<T, N> &operator*=(simd_vector<T, N> const &other) noexcept
	{
		return *this = (*this) * other;
	...
	{
		return ::ufio::details::wrap_minus_common(::ufio::details::all_zero_simd_vector_mask<T, N>, *this);
	}
	...
	inline constexpr simd_vector<T, N> operator~() const noexcept
	{
		constexpr bool using_simd{(sizeof(vec_type) == 16 && ::ufio::details::cpu_flags::sse2_supported) ||
								  ...
								  (sizeof(vec_type) == 64 && (::ufio::details::cpu_flags::avx512dq_supported ||
															  ::ufio::details::cpu_flags::avx512f_supported))};
		if constexpr (using_simd)
		{
			return (::ufio::details::all_one_simd_vector_mask<T, N>)^(*this);
		}
		else
		{
			return ::ufio::details::generic_simd_self_create_op_impl(*this, [](T v) noexcept { return ~v; });
		}
	}
		...
		requires(::std::integral<value_type> && (N * sizeof(T) == 16 || N * sizeof(T) == 32))
	{
		...
		else
		{
...
#endif
			{
				...
				{
					if constexpr (::ufio::details::cpu_flags::sse3_supported)
					{
						__m128i temp_vec = __builtin_bit_cast(__m128i, *this);
						__m128i mask = __builtin_bit_cast(
							__m128i, ::ufio::details::simd_byte_swap_shuffle_mask<sizeof(value_type), N>);
				...
				else if constexpr (sizeof(vec_type) == 32 && ::ufio::details::cpu_flags::avx2_supported)
				{
					...
					__m256i mask = __builtin_bit_cast(
						__m256i, ::ufio::details::simd_byte_swap_shuffle_mask<sizeof(value_type), N>);
				...
				else
				{
					...
					__m512i mask = __builtin_bit_cast(
						__m512i, ::ufio::details::simd_byte_swap_shuffle_mask<sizeof(value_type), N>);
				...
				if constexpr (sizeof(vec_type) == 16 && ::ufio::details::cpu_flags::armneon_supported)
				{
					if constexpr (sizeof(T) == 8)
					{
			...
			::ufio::details::generic_simd_self_op_impl(*this, [](T &t) { t = ::ufio::byte_swap(t); });
		}
	}
};
