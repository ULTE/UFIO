...
inline constexpr bool calculate_can_simd_vector_run_with_cpu_instruction(::std::size_t sizeofsimdvector) noexcept
{
	...
	{
		if constexpr (::ufio::details::cpu_flags::wasmsimd128_supported ||
					  ::ufio::details::cpu_flags::sse2_supported ||
					  ::ufio::details::cpu_flags::armneon_supported ||
					  ::ufio::details::cpu_flags::loongarch_simd128_lsx_supported)
		{
			return true;
		}
	}
	else if (sizeofsimdvector == 32)
	{
		if constexpr (::ufio::details::cpu_flags::avx2_supported ||
					  ::ufio::details::cpu_flags::loongarch_simd256_lasx_supported)
		{
			return true;
		}
	}
	...
	{
		if constexpr (::ufio::details::cpu_flags::avx512f_supported ||
...
#endif
	bool
	is_all_zeros_impl(::ufio::intrinsics::simd_vector<T, n> const &vec) noexcept
{
#if __cpp_if_consteval >= 202106L || __cpp_lib_is_constant_evaluated >= 201811L
	...
	__has_builtin(__builtin_aarch64_reduc_umax_scal_v4si_uu)
			...
			if constexpr (::ufio::details::cpu_flags::sse4_2_supported)
			{
				__m128i a = __builtin_bit_cast(__m128i, vec);
				return _mm_testz_si128(a, a);
			}
			else if constexpr (::ufio::details::cpu_flags::sse2_supported)
			{
				__m128i a = __builtin_bit_cast(__m128i, vec);
...
#endif
#elif (defined(__x86_64__) || defined(_M_AMD64) || defined(__i386__) || defined(_M_IX86))
			if constexpr (::ufio::details::cpu_flags::avx_supported)
			{
		...
		else if constexpr (sizeof(::ufio::intrinsics::simd_vector<T, n>) == 64)
		{
#if defined(__has_builtin) && __has_cpp_attribute(__gnu__::__vector_size__)
...
{
	return ::ufio::details::is_all_zeros_impl(vec);
}
