...
{
	::ufio::freestanding::array<::std::uint_least8_t, sizeof(T)> arr;
	for (auto &e : arr)
	{
...
inline constexpr T generic_simd_comparision_common_impl(T const &a, T const &b, Func func) noexcept
{
	...
	return ::ufio::details::generic_simd_create_op_impl(a, b,
...
template <typename T, ::std::size_t N>
inline constexpr ::ufio::intrinsics::simd_vector<T, N> all_zero_simd_vector_mask{};

template <typename T, ::std::size_t N>
inline constexpr ::ufio::intrinsics::simd_vector<T, N> all_one_simd_vector_mask{
	create_value_mx<::ufio::intrinsics::simd_vector<T, N>>()};

template <typename T, ::std::size_t N>
inline constexpr ::ufio::intrinsics::simd_vector<T, N>
wrap_add_common(::ufio::intrinsics::simd_vector<T, N> const &a,
				::ufio::intrinsics::simd_vector<T, N> const &b) noexcept
{
...
#endif
	{
		...
		using vec_type = ::ufio::intrinsics::simd_vector<T, N>;
		if constexpr (sizeof(vec_type) == 16 && ::ufio::details::cpu_flags::sse2_supported)
		{
		...
		else if constexpr (sizeof(vec_type) == 32 && ::ufio::details::cpu_flags::avx2_supported)
		{
			if constexpr (::std::integral<T>)
			{
		...
		{
			if constexpr (::std::integral<T> && ::ufio::details::cpu_flags::avx512bw_supported)
			{
			...
			else if constexpr (::std::floating_point<T> && ::ufio::details::cpu_flags::avx512f_supported)
			{
#if !defined(__clang__)
				if constexpr (sizeof(T) == sizeof(float) / 2)
				{
...
#elif defined(__arm64__) || defined(__aarch64__) || defined(_M_ARM64) || defined(__arm__) || defined(_M_ARM)
		using vec_type = ::ufio::intrinsics::simd_vector<T, N>;
		if constexpr (::ufio::details::cpu_flags::armneon_supported)
		{
			if constexpr (::std::integral<T>)
			{
...
template <typename T, ::std::size_t N>
inline constexpr ::ufio::intrinsics::simd_vector<T, N>
wrap_minus_common(::ufio::intrinsics::simd_vector<T, N> const &a,
				  ::ufio::intrinsics::simd_vector<T, N> const &b) noexcept
{
#if __cpp_if_consteval >= 202106L
	if !consteval
...
inline constexpr auto create_byte_swap_mask() noexcept
{
	...
	::ufio::freestanding::array<char unsigned, totaln> v1;
	for (char unsigned i{}; i != resn; ++i)
	{
		v1[i] = i;
	}
	auto v2 = __builtin_bit_cast(::ufio::freestanding::array<T, N>, v1);
	for (auto &e : v2)
	{
		e = ::ufio::byte_swap(e);
	}
	return __builtin_bit_cast(::ufio::intrinsics::simd_vector<char unsigned, totaln>, v2);
}
template <::std::size_t sizeoft, ::std::size_t N>
...
{
	return ::ufio::details::wrap_add_common(a, b);
}
...
inline constexpr simd_vector<T, N> operator-(simd_vector<T, N> const &a, simd_vector<T, N> const &b) noexcept
{
	return ::ufio::details::wrap_minus_common(a, b);
}
...
inline constexpr simd_vector<T, N> operator*(simd_vector<T, N> const &a, simd_vector<T, N> const &b) noexcept
{
	...
	return ::ufio::details::generic_simd_create_op_impl(a, b, [](T va, T vb) noexcept -> T { return va * vb; });
}
...
inline constexpr simd_vector<T, N> operator&(simd_vector<T, N> const &a, simd_vector<T, N> const &b) noexcept
{
...
#endif
	{
		...
		else if constexpr (sizeof(vec_type) == 32 && ::ufio::details::cpu_flags::avx_supported)
		{
			__m256 amm = __builtin_bit_cast(__m256, a);
			__m256 bmm = __builtin_bit_cast(__m256, b);
		...
		{
			if constexpr (::ufio::details::cpu_flags::avx512dq_supported)
			{
				__m512 amm = __builtin_bit_cast(__m512, a);
				__m512 bmm = __builtin_bit_cast(__m512, b);
				return __builtin_bit_cast(vec_type, _mm512_and_ps(amm, bmm));
			}
			else if constexpr (::ufio::details::cpu_flags::avx512f_supported)
			{
				__m512i amm = __builtin_bit_cast(__m512i, a);
				__m512i bmm = __builtin_bit_cast(__m512i, b);
...
inline constexpr simd_vector<T, N> operator<<(simd_vector<T, N> const &a, unsigned i) noexcept
{
...
#endif
	{
		...
		if constexpr (2 <= sizeof(T) && sizeof(T) <= 8)
		{
			...
			{
				if constexpr (::ufio::details::cpu_flags::avx2_supported)
				{
					__m256i amm = __builtin_bit_cast(__m256i, a);
					if constexpr (sizeof(T) == 2)
					{
				...
				else if constexpr (::ufio::details::cpu_flags::avx_supported)
				{
					__m256i amm = __builtin_bit_cast(__m256i, a);
			...
			{
				if constexpr (::ufio::details::cpu_flags::avx512bw_supported)
				{
					__m512i amm = __builtin_bit_cast(__m512i, a);
					if constexpr (sizeof(T) == 2)
					{
	...
	return ::ufio::details::generic_simd_self_create_op_impl(a, [i](T va) { return va << i; });
}
...
inline constexpr simd_vector<T, N> operator<(simd_vector<T, N> const &a, simd_vector<T, N> const &b) noexcept
{
...
#endif
	{
		...
		if constexpr (sizeof(vec_type) == 16 && ::ufio::details::cpu_flags::sse2_supported)
		{
			if constexpr (::std::integral<T>)
			{
				if constexpr (::ufio::details::cpu_flags::avx512vl_supported &&
							  ::ufio::details::cpu_flags::avx512bw_supported)
				{
					__m128i amm = __builtin_bit_cast(__m128i, a);
					__m128i bmm = __builtin_bit_cast(__m128i, b);
					if constexpr (::std::signed_integral<T>)
	...
	return ::ufio::details::generic_simd_comparision_common_impl(a, b, [](T va, T vb) noexcept -> bool { return va < vb; });
}
...
inline constexpr simd_vector<T, N> operator==(simd_vector<T, N> const &a, simd_vector<T, N> const &b) noexcept
{
...
#endif
	{
		...
		else if constexpr (sizeof(vec_type) == 64 && ::ufio::details::cpu_flags::avx512f_supported)
		{
			__m512i amm = __builtin_bit_cast(__m512i, a);
			__m512i bmm = __builtin_bit_cast(__m512i, b);
			if constexpr (sizeof(T) == 1)
...
{
	using vec_type = simd_vector<T, N>;
	constexpr bool using_avx512{::ufio::details::cpu_flags::avx512f_supported &&
								::ufio::details::cpu_flags::avx512bw_supported &&
								::ufio::details::cpu_flags::avx512vl_supported};
	constexpr bool using_simd{(sizeof(vec_type) == 16 && ::ufio::details::cpu_flags::sse2_supported) ||
							  (sizeof(vec_type) == 32 && ::ufio::details::cpu_flags::avx2_supported)};
	if constexpr (using_simd && (!using_avx512))
	{
		return (::ufio::details::all_zero_simd_vector_mask<T, N>) == (a == b);
	}
	else
	{
#if __cpp_if_consteval >= 202106L
