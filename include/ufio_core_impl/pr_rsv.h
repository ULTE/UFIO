...
inline constexpr ::std::size_t pr_rsv_size_impl() noexcept
{
	...
	if constexpr (alias_printable<nocvref_t>)
	{
		using alias_type = decltype(print_alias_define(
			::ufio::io_alias, *static_cast<nocvref_t const *>(static_cast<void const *>(nullptr))));
...
template <::std::integral char_type, typename T>
inline constexpr ::std::size_t pr_rsv_size{::ufio::details::pr_rsv_size_impl<char_type, T>()};
...
inline constexpr Iter pr_rsv_to_iterator_unchecked(Iter it, T &&t) noexcept
{
	...
	{
		using alias_type = decltype(print_alias_define(::ufio::io_alias, t));
		constexpr bool error{reserve_printable<char_type, alias_type>};
		if constexpr (error)
		{
			return print_reserve_define(io_reserve_type<char_type, alias_type>, it,
										print_alias_define(::ufio::io_alias, t));
		}
		else
		{
			static_assert(error, "type is not reserve_printable");
...
template <::std::integral char_type, typename T>
inline constexpr ::ufio::parse_result<char_type const *> parse_by_scan_impl(char_type const *first,
																			   char_type const *last, T t) noexcept
{
	if constexpr (::ufio::precise_reserve_scannable<char_type, T>)
	{
		...
		if (diff < n) [[unlikely]]
		{
			return {first, ::ufio::parse_code::end_of_file};
		}
		if constexpr (precise_reserve_scannable_no_error<char_type, T>)
		{
			...
			return {first + n, ::ufio::parse_code::ok};
		}
		else
		{
	...
	else if constexpr (::ufio::contiguous_scannable<char_type, T>)
	{
		return scan_contiguous_define(io_reserve_type<char_type, T>, first, last, t);
	}
	else if constexpr (::ufio::context_scannable<char_type, T>)
	{
...
#endif
inline constexpr ::ufio::parse_result<char_type const *> parse_by_scan(char_type const *first, char_type const *last,
																		  T &&t) noexcept
{
	using mytype = decltype(io_scan_forward<char_type>(io_scan_alias(t)));
	constexpr bool allscannable{::ufio::precise_reserve_scannable<char_type, mytype> ||
								::ufio::contiguous_scannable<char_type, mytype> ||
								::ufio::context_scannable<char_type, mytype>};
	if constexpr (allscannable)
	{
		return ::ufio::details::parse_by_scan_impl(first, last, io_scan_forward<char_type>(io_scan_alias(t)));
	}
	else
	{
