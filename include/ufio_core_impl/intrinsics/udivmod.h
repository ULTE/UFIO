...
inline constexpr tuint<T> udivbigbysmalltosmalldefault(T u1, T u0, T v) noexcept
{
	...
	if constexpr (sizeof(T) == sizeof(::std::uint_least64_t))
	{
...
#endif
		{
			...
			auto quotient{::ufio::intrinsics::msvc::x86::_udiv128(u1, u0, v, __builtin_addressof(remainder))};
#else
			T quotient, remainder;
...
#endif
		{
			return ::ufio::intrinsics::msvc::x86::__shiftleft128(low, high, static_cast<char unsigned>(shift));
		}
	}
#endif
	constexpr unsigned n_udword_bits = ::std::numeric_limits<T>::digits;
...
#endif
		{
			return ::ufio::intrinsics::msvc::x86::__shiftright128(low, high, static_cast<char unsigned>(shift));
		}
	}
#endif
	constexpr unsigned n_udword_bits = ::std::numeric_limits<T>::digits;
...
template <typename T>
inline constexpr ::ufio::intrinsics::tuints<T> udivmod(T dividendlow, T dividendhigh, T divisorlow,
														  T divisorhigh) noexcept
{
	...
	if (divisorhigh == 0)
	{
		...
		if (dividendhigh < divisorlow)
		{
			...
			auto temp = ::ufio::intrinsics::udivbigbysmalltosmalldefault(dividendhigh, dividendlow, divisorlow);
	...
	divisorhigh = ::ufio::intrinsics::shiftleft(divisorlow, divisorhigh, shift);
	divisorlow <<= shift;
	...
	{
		carry = 0;
		dividendlow = ::ufio::intrinsics::subc(dividendlow, divisorlow, carry, carry);
		dividendhigh = ::ufio::intrinsics::subc(dividendhigh, divisorhigh, carry, carry);
		constexpr T zero{};
		T mask{zero - carry};
		T templow{divisorlow & mask}, temphigh{divisorhigh & mask};
		carry = !carry;
		quotientlow = ::ufio::intrinsics::addc(quotientlow, quotientlow, carry, carry);
		carry = 0;
		dividendlow = ::ufio::intrinsics::addc(dividendlow, templow, carry, carry);
		dividendhigh = ::ufio::intrinsics::addc(dividendhigh, temphigh, carry, carry);
		divisorlow = ::ufio::intrinsics::shiftright(divisorlow, divisorhigh, 1u);
		divisorhigh >>= 1u;
	} while (shift--);
