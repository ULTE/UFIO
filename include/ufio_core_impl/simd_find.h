...
{
	::ufio::freestanding::array<char_type, N> arr;
...
inline constexpr ::ufio::freestanding::array<char_type, N> characters_array_impl{
...
inline constexpr ::ufio::freestanding::array<char_type, N> wide_characters_array_impl{
	create_find_simd_vector_with_unsigned_toggle<signed_disposition, char_type, N>(lfch)};
																  ...
																  Func func) noexcept
{
	...
	using simd_vector_type = ::ufio::intrinsics::simd_vector<char_type, N>;
																				   ...
																				   wchar_t const *last) noexcept
{
	...
	constexpr unsigned N{vec_size / sizeof(char_type)};
	using simd_vector_type = ::ufio::intrinsics::simd_vector<char_type, N>;
	using unsigned_simd_vector_type = ::ufio::intrinsics::simd_vector<unsigned_char_type, N>;
	using signed_simd_vector_type = ::ufio::intrinsics::simd_vector<signed_char_type, N>;
																	  ...
																	  char_type const *last) noexcept
{
...
#endif
	{
		...
		else if constexpr (::ufio::details::optimal_simd_vector_run_with_cpu_instruction_size)
		{
			first = find_simd_constant_simd_common_impl<
				lfch, findnot, ::ufio::details::optimal_simd_vector_run_with_cpu_instruction_size>(first, last);
		}
	...
	if constexpr (findnot)
	{
		return ::ufio::freestanding::find_not(first, last, lfchct);
	}
	else
	{
		return ::ufio::freestanding::find(first, last, lfchct);
	}
}
...
{
	constexpr ::std::size_t initialdiffn{::ufio::details::optimal_simd_vector_run_with_cpu_instruction_size
											 ? ::ufio::details::optimal_simd_vector_run_with_cpu_instruction_size
...
inline constexpr Iter find_space_common_iterator_generic_impl(Iter begin, Iter end)
{
	if constexpr (ishtml)
	{
		if constexpr (findnot)
		{
			for (; begin != end && ufio::char_category::is_html_whitespace(*begin); ++begin)
				;
		}
		else
		{
	...
	else
	{
		if constexpr (findnot)
		{
			for (; begin != end && ufio::char_category::is_c_space(*begin); ++begin)
				;
		}
		else
		{
...
#endif
	{
		if constexpr (::ufio::details::optimal_simd_vector_run_with_cpu_instruction_size)
		{
	...
	return find_simd_small_optimization_common_impl<false>(
		...
		[](char_type ch) noexcept {
			if constexpr (ishtml)
			{
				if constexpr (findnot)
				{
					return ufio::char_category::is_html_whitespace(ch);
				}
				else
				{
					return !ufio::char_category::is_html_whitespace(ch);
			...
			else
			{
				if constexpr (findnot)
				{
					return ufio::char_category::is_c_space(ch);
				}
				else
				{
					return !ufio::char_category::is_c_space(ch);
...
inline constexpr Iter find_space_impl(Iter first, Iter last)
{
	...
	if constexpr (!::std::is_volatile_v<value_type> && ::std::contiguous_iterator<Iter>)
	{
		...
		{
			return ::ufio::details::find_space_common_impl<ishtml, findnot>(first, last);
		}
		else if constexpr (::std::is_pointer_v<Iter>)
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{first};
			return ::ufio::details::find_space_common_impl<ishtml, findnot>(first_const_ptr, last) -
				   first_const_ptr + first;
		}
		else
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{::std::to_address(first)};
			return ::ufio::details::find_space_common_impl<ishtml, findnot>(first_const_ptr,
																			   ::std::to_address(last)) -
				   first_const_ptr + first;
		}
	}
...
inline constexpr Iter find_ch_impl(Iter first, Iter last)
{
	...
	if constexpr (!::std::is_volatile_v<value_type> && ::std::contiguous_iterator<Iter>)
	{
		if constexpr (::std::same_as<value_type, ::std::remove_cvref_t<value_type> const>)
		{
			return ::ufio::details::find_simd_constant_common_impl<lfch, findnot>(first, last);
		}
		else if constexpr (::std::is_pointer_v<Iter>)
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{first};
			return ::ufio::details::find_simd_constant_common_impl<lfch, findnot>(first_const_ptr, last) -
				   first_const_ptr + first;
		}
		else
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{::std::to_address(first)};
			return ::ufio::details::find_simd_constant_common_impl<lfch, findnot>(first_const_ptr,
																					 ::std::to_address(last)) -
				   first_const_ptr + first;
		}
	}
