...
namespace ufio::details::string_hack
{
...
#define UFIO_MSVC_STL_INSERT_STRING_ANNOTATION
...
// we leave this UFIO_MSVC_STL_INSERT_STRING_ANNOTATION macro for users to use

inline constexpr bool msvc_stl_xstring_activate_string_annotation{
#ifdef UFIO_MSVC_STL_INSERT_STRING_ANNOTATION
	true
#endif
};
...
{
#ifdef UFIO_MSVC_STL_INSERT_STRING_ANNOTATION
	if constexpr (::ufio::details::string_hack::msvc_stl_xstring_activate_string_annotation)
	{
														   ...
														   typename ::std::basic_string<elem, traits, alloc>::size_type const _New_size) noexcept
{
	...
	if (_Capacity <= _Small_string_capacity || !::ufio::details::string_hack::msvc_stl_xstring_get_asan_string_should_annotate())
	{
		return;
	}
	...
	//   [_Aligned._End, _End) valid
	::ufio::details::string_hack::msvc_stl_sanitizer_annotate_contiguous_container(_Aligned._First, _Aligned._End, _Old_fixed, _New_fixed);
}
															...
															typename ::std::basic_string<elem, traits, alloc>::size_type const _New_size) noexcept
{
	...
	decltype(auto) _My_data{::ufio::details::string_hack::hack_scary_val(str)};
	::ufio::details::string_hack::msvc_stl_xstring_Apply_annotation<elem, traits, alloc>(_My_data._Myptr(), _My_data._Myres, _Old_size, _New_size);
}
...
inline constexpr void set_end_ptr(T &str, typename T::value_type *ptr) noexcept
{
	...
	if constexpr (::ufio::details::string_hack::msvc_stl_xstring_activate_string_annotation)
	{
		::ufio::details::string_hack::msvc_stl_xstring_Modify_annotation(str, scv._Mysize, newsize);
	}
	scv._Mysize = newsize;
}
