...
template <::std::integral char_type, my_floating_point flt, ::ufio::manipulators::floating_format mf>
inline constexpr ::std::size_t lc_print_rsv_iec559_size(basic_lc_all<char_type> const *all)
{
	using trait = iec559_traits<flt>;
	if constexpr (mf == ::ufio::manipulators::floating_format::fixed)
	{
		//(+/-)(significants+sep)
	...
	else if constexpr (mf == ::ufio::manipulators::floating_format::scientific)
	{
...
template <::std::integral char_type, manipulators::scalar_flags flags, ::ufio::details::my_floating_point flt>
...
{
	return ::ufio::details::lc_print_reserve_float_size_impl<char_type, flags, flt>(all);
}
												 ...
												 manipulators::scalar_manip_t<flags, flt> f) noexcept
{
	...
	if constexpr (flags.floating == manipulators::floating_format::hexfloat)
	{
		...
		)
		{
			...
			{
				return ::ufio::details::lc_print_rsvhexfloat_define_impl<
	...
	else
	{
		if constexpr (::std::same_as<::std::remove_cvref_t<flt>, long double> &&
					  sizeof(flt) == sizeof(double)) // this is the case on xxx-windows-msvc
		{
			return ::ufio::details::lc_print_rsvflt_define_impl<flags.showpos, flags.uppercase, flags.uppercase_e,
