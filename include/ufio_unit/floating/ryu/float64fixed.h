...
inline constexpr char_type *prsv_fp_zero_precision_impl(char_type *iter, ::std::size_t precision) noexcept
{
	...
	*iter = ::ufio::char_literal_v<(comma ? u8',' : u8'.'), char_type>;
	++iter;
	return ::ufio::details::prsv_fill_zero_impl(iter, precision);
}
...
{
	using trait = ::ufio::details::iec559_traits<flt>;
	...
	auto [mantissa, exponent, sign] = ::ufio::details::get_punned_result(f);
	iter = print_rsv_fp_sign_impl<showpos>(iter, sign);
	if (exponent == exponent_mask_u32)
	{
		return ::ufio::details::prsv_fp_nan_impl<uppercase>(iter, mantissa);
	}
	if (!mantissa && !exponent)
	{
		return ::ufio::details::prsv_fp_zero_precision_impl<comma>(iter, precision);
	}
