																...
																::std::uint_least32_t m10len) noexcept
{
	...
	{
		*iter = ::ufio::char_literal_add<char_type>(m10);
		++iter;
		return iter;
	}
	else
...
template <typename flt, bool uppercase_e, ::ufio::manipulators::floating_format mt, ::std::integral char_type>
inline constexpr char_type *lc_print_rsv_fp_decision_impl(basic_lc_all<char_type> const *all, char_type *iter,
														  typename iec559_traits<flt>::mantissa_type m10,
														  ::std::int_least32_t e10) noexcept
{
	if constexpr (mt == ::ufio::manipulators::floating_format::general)
	{
		if (-5 < e10 && e10 < 7)
		{
			return lc_print_rsv_fp_fixed_decision_impl<flt>(all, iter, m10, e10);
		}
		return lc_print_rsv_fp_decision_impl<flt, uppercase_e, ::ufio::manipulators::floating_format::scientific>(
			all, iter, m10, e10);
	}
	else if constexpr (mt == ::ufio::manipulators::floating_format::scientific)
	{
		if (m10 < 10u) [[unlikely]]
		{
			*iter = ::ufio::char_literal_add<char_type>(m10);
...
template <bool showpos, bool uppercase, bool uppercase_e, ::ufio::manipulators::floating_format mt, typename flt,
		  ::std::integral char_type>
inline constexpr char_type *lc_print_rsvflt_define_impl(basic_lc_all<char_type> const *all, char_type *iter,
														flt f) noexcept
{
	if constexpr (::ufio::manipulators::floating_format::fixed == mt && uppercase_e)
	{
	...
	else
	{
		...
		{
			if constexpr (mt != ::ufio::manipulators::floating_format::scientific)
			{
				*iter = char_literal_v<u8'0', char_type>;
				++iter;
				return iter;
		...
		if constexpr (mt == ::ufio::manipulators::floating_format::fixed)
		{
			return lc_print_rsv_fp_fixed_decision_impl<flt>(all, iter, m10, e10);
		}
		else
