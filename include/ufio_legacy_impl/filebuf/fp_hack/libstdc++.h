...
inline FILE *fp_hack(T *fb) noexcept
{
					   ...
					   ::std::same_as<::std::char_traits<char_type>, traits_type>)
	{
		if (fb) [[likely]]
		{
			...
			if (fbfb)
			{
				return ::ufio::details::streambuf_hack::fp_hack_impl(fbfb);
			}
#else
						  ...
						  ::std::same_as<char_type, char32_t>)
			{
				...
				if (my_type)
				{
					::std::size_t mytypelen{::ufio::cstr_len(my_type)};
					if (::ufio::details::symbol_cmp_equal(::ufio::details::libstdcxx_streambufname<2, char_type>, my_type, mytypelen))
					{
						return static_cast<::__gnu_cxx::stdio_sync_filebuf<char_type, traits_type> *>(fb)->file();
					}
					else if (::ufio::details::symbol_cmp_equal(::ufio::details::libstdcxx_streambufname<0, char_type>, my_type, mytypelen) ||
							 ::ufio::details::symbol_cmp_equal(::ufio::details::libstdcxx_streambufname<1, char_type>, my_type, mytypelen))
					{
																   ...
																   ::std::ios_base::openmode mode)
{
	...
	constexpr ::std::size_t buffer_size{ufio::details::cal_buffer_size<CharT, true>()};
...
[[nodiscard]]
#endif
inline ::std::basic_filebuf<CharT, Traits> *open_hacked_basic_filebuf(::std::__c_file *fp, ::ufio::open_mode mode)
{
