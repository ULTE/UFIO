...
namespace ufio::details::streambuf_hack
{
...
inline FILE *stdinbuf_stdoutbuf_fp_hack(T *stdbuf) noexcept
{
	...
	::ufio::details::my_memcpy(__builtin_addressof(fp),
...
inline FILE *fp_hack_fbf(::std::basic_filebuf<char_type, traits_type> *fbuf) noexcept
{
	...
	return ::ufio::details::streambuf_hack::fp_hack_impl(fbuf);
}
...
inline FILE *fp_hack(T *fb) noexcept
{
	...
	{
		return ::ufio::details::streambuf_hack::fp_hack_fbf(fb);
	}
					   ...
					   ::std::same_as<::std::char_traits<char_type>, traits_type>)
	{
		if (fb) [[likely]]
		{
			...
			{
				char const *my_type{::ufio::rtti_hack::abi_type_info_name_or_nullptr(fb)};
				if (my_type)
				{
					::std::size_t mytypelen{::std::strlen(my_type)};
					if (::ufio::details::symbol_cmp_equal(::ufio::details::libcxx_streambufname<0, char_type>, my_type, mytypelen) ||
						::ufio::details::symbol_cmp_equal(::ufio::details::libcxx_streambufname<1, char_type>, my_type, mytypelen))
					{
						return ::ufio::details::streambuf_hack::stdinbuf_stdoutbuf_fp_hack(fb);
					}
					else if (::ufio::details::symbol_cmp_equal(::ufio::details::libcxx_streambufname<2, char_type>, my_type, mytypelen))
					{
						return ::ufio::details::streambuf_hack::fp_hack_impl(static_cast<::std::basic_filebuf<char_type, traits_type> *>(fb));
					}
				}
			}
		}
...
{
	::ufio::details::my_memcpy(reinterpret_cast<::std::byte *>(fb) + libcxx_om_location<char_type, traits_type>,
								  __builtin_addressof(mode), sizeof(mode));
	::ufio::details::my_memcpy(reinterpret_cast<::std::byte *>(fb) + libcxx_fp_location<char_type, traits_type>,
...
inline ::std::basic_filebuf<CharT, Traits> *open_libcxx_hacked_basic_filebuf_impl(FILE *fp, ::std::ios::openmode mode)
{
	...
	::ufio::details::my_memcpy(reinterpret_cast<::std::byte *>(fb) + libcxx_fp_location<CharT, Traits>,
								  __builtin_addressof(fp), sizeof(fp));
	::ufio::details::my_memcpy(reinterpret_cast<::std::byte *>(fb) + libcxx_om_location<CharT, Traits>,
								  __builtin_addressof(mode), sizeof(mode));
...
[[nodiscard]]
#endif
inline ::std::basic_filebuf<CharT, Traits> *open_hacked_basic_filebuf(FILE *fp, ::ufio::open_mode mode)
{
	return open_libcxx_hacked_basic_filebuf_impl<CharT, Traits>(
		fp, ::ufio::details::calculate_fstream_file_open_mode(mode));
}
