...
public:
	...
	basic_filebuf_file(::ufio::io_construct_t, basic_c_family_io_observer<family, char_type> ciob, open_mode mode)
		: basic_filebuf_io_observer<CharT, Traits>{
			  ::ufio::details::streambuf_hack::open_hacked_basic_filebuf<CharT, Traits>(ciob.fp, mode)}
	{
	}
	template <c_family family>
	basic_filebuf_file(basic_c_family_file<family, char_type> &&chd, open_mode mode)
		: basic_filebuf_io_observer<CharT, Traits>{
			  ::ufio::details::streambuf_hack::open_hacked_basic_filebuf<CharT, Traits>(chd.fp, mode)}
	{
		chd.fp = nullptr;
	}

	basic_filebuf_file(io_temp_t)
		: basic_filebuf_file(::ufio::basic_c_file_unlocked<char_type>(::ufio::io_temp),
							 ::ufio::open_mode::in | ::ufio::open_mode::out)
	{
	}
