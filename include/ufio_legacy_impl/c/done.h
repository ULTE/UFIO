										   ...
										   FILE *__restrict fp)
{
	if (fp == stdin)
	{
		::ufio::noexcept_call(::fflush, stdout);
	}
#if defined(__NEWLIB__) && !defined(__CYGWIN__)
	struct _reent rent{};
...
inline ::std::byte *read_some_bytes_underflow_define(::ufio::basic_c_family_io_observer<family, char_type> ciob,
													 ::std::byte *first, ::std::byte *last)
{
	if constexpr (family == ::ufio::c_family::unlocked || family == ::ufio::c_family::emulated_unlocked)
	{
		return ::ufio::details::c_unlocked_read_some_bytes_impl(ciob.fp, first, last);
	}
	else
	{
		return ::ufio::details::c_read_some_bytes_impl(ciob.fp, first, last);
	}
}

template <::ufio::c_family family, ::std::integral char_type>
inline ::std::byte const *
write_some_bytes_overflow_define(::ufio::basic_c_family_io_observer<family, char_type> ciob,
								 ::std::byte const *first, ::std::byte const *last)
{
	if constexpr (family == ::ufio::c_family::unlocked || family == ::ufio::c_family::emulated_unlocked)
	{
		return ::ufio::details::c_unlocked_write_some_bytes_impl(ciob.fp, first, last);
	}
	else
	{
		return ::ufio::details::c_write_some_bytes_impl(ciob.fp, first, last);
	}
}
