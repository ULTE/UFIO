...
inline ::std::byte *hack_wide_data(FILE *fp) noexcept
{
	...
	::std::byte *value;
	::ufio::details::my_memcpy(__builtin_addressof(value),
...
inline char_type *hack_wp(FILE *fp) noexcept
{
	...
	char_type *value;
	::ufio::details::my_memcpy(__builtin_addressof(value), hack_wide_data(fp) + off, sizeof(wchar_t *));
	return value;
}
template <::std::size_t position, ::std::integral char_type>
	requires(sizeof(char_type) == sizeof(char32_t))
...
{
	constexpr ::std::size_t off{position * sizeof(size_t)};
	::ufio::details::my_memcpy(hack_wide_data(fp) + off, __builtin_addressof(ptr), sizeof(wchar_t *));
}
} // namespace details::fp_wide_hack
