...
inline constexpr void write(basic_c_family_io_observer<c_family::emulated_unlocked, char_type> ciob, Iter first,
							Iter last)
{
	::ufio::details::avr_libc_write_internal_impl(ciob.fp, reinterpret_cast<char const *>(::std::to_address(first)),
...
{
	::ufio::details::avr_libc_scatter_write_impl_with_normal_write(ciob.fp, scatters.base, scatters.len);
}
						   ...
						   Iter last)
{
	...
	return (::ufio::details::avr_libc_read_internal_impl(ciob.fp, first_addr,
