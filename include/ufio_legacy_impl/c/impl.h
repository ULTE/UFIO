...
template <c_family family>
inline ::ufio::intfpos_t my_c_io_seek_impl(FILE *, ::ufio::intfpos_t, seekdir)
{
	avr_libc_nosup_impl();
}
...
#else
template <c_family family>
inline ::ufio::intfpos_t my_c_io_seek_impl(FILE *fp, ::ufio::intfpos_t offset, seekdir s)
{
	...
	*/
	if constexpr (family == c_family::unlocked || family == c_family::emulated_unlocked)
	{
		...
		return static_cast<::ufio::intfpos_t>(val);
#endif
#else
	...
	(defined(__MINGW32__) && !__has_include(<_mingw_stat64.h>))
		if constexpr (sizeof(long) < sizeof(::ufio::intfpos_t))
		{
			if (offset < static_cast<::ufio::intfpos_t>(::std::numeric_limits<long>::min()) ||
				offset > static_cast<::ufio::intfpos_t>(::std::numeric_limits<long>::max()))
			{
				throw_posix_error(EINVAL);
			}
		}
		...
		return static_cast<::ufio::intfpos_t>(static_cast<long unsigned>(val));
#else
		if (
#if (defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) && !defined(__CYGWIN__)
...
inline FILE *my_c_open_tmp_file()
{
	...
	throw_posix_error(EINVAL);
#else
	::ufio::posix_file pf(io_temp);
	auto fp{::ufio::details::my_c_file_open_impl(pf.fd, ::ufio::open_mode::in | ::ufio::open_mode::out)};
	pf.release();
	return fp;
#endif
}
...
public:
		...
		requires(family == c_family::standard || family == c_family::emulated)
	{
		...
		if constexpr (false)
		{
			::ufio::details::ucrt_lock_file(fp);
		}
		else
		{
...
//	_flockfile(fp);	//TO FIX undefined reference to `__cygwin_lock_lock' why?
		...
		requires(family == c_family::standard || family == c_family::emulated)
	{
		...
		if constexpr (false)
		{
			::ufio::details::ucrt_unlock_file(fp);
		}
		else
		{
...
template <c_family family, ::std::integral ch_type>
inline ::ufio::intfpos_t io_stream_seek_bytes_define(basic_c_family_io_observer<family, ch_type> cfhd,
...
public:
	...
	basic_c_family_file(basic_posix_family_file<pfamily, char_type> &&phd, open_mode om)
		: basic_c_family_io_observer<family, ch_type>{::ufio::details::my_c_file_open_impl(phd.fd, om)}
	{
		phd.fd = -1;
	}
#if (defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) || defined(__CYGWIN__)
	...
	explicit constexpr basic_c_family_file(io_construct_t, basic_posix_family_io_observer<pfamily, ch_type> piob, open_mode om) noexcept
		: basic_c_family_io_observer<family, char_type>{::ufio::details::my_c_file_open_impl(piob.fd, om)}
	{
	}
#endif
	basic_c_family_file(io_temp_t)
		: basic_c_family_io_observer<family, ch_type>{::ufio::details::my_c_open_tmp_file()}
	{
	}
};
...
#include "uclibc.h"
#elif defined(UFIO_LIBC_CUSTOM_BUFFER_PTRS)
#include "custom.h"
#endif
