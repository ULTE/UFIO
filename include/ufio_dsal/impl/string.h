...
inline constexpr ::ufio::basic_allocation_least_result<chtype *> string_allocate_init(chtype const *first, ::std::size_t n) noexcept
{
	...
	*::ufio::freestanding::non_overlapped_copy_n(first, n, ptr) = 0;
	return {ptr, static_cast<::std::size_t>(allocn - 1u)};
}

template <typename allocator_type, ::std::integral chtype>
inline constexpr void string_heap_dilate_uncheck(::ufio::containers::details::string_internal<chtype> &imp, ::std::size_t rsize, chtype *pnullterminate) noexcept
{
...
template <typename allocator_type, ::std::integral chtype>
inline constexpr void string_push_back_heap_grow_twice(::ufio::containers::details::string_internal<chtype> &imp, chtype *pnullterminate) noexcept
{
	...
	return ::ufio::containers::details::string_heap_dilate_uncheck<allocator_type>(imp, bfsizep1mul2, pnullterminate);
}
...
public:
	...
	using string_view_type = ::ufio::containers::basic_string_view<char_type>;
	using cstring_view_type = ::ufio::containers::basic_cstring_view<char_type>;

	::ufio::containers::details::string_internal<char_type> imp;
	...
	explicit constexpr basic_string(size_type n, char_type ch) noexcept
	{
		...
		else
		{
			...
			*::ufio::freestanding::uninitialized_fill(ptr, ptr + n, ch) = 0;
		}
	}
...
private:
	constexpr void construct_impl(char_type const *otherptr, size_type othern) noexcept
	{
		...
		{
			auto newres{::ufio::containers::details::string_allocate_init<allocator_type>(otherptr, othern)};
...
public:
	...
	constexpr basic_string(basic_string const &other) noexcept
	{
		...
		auto [ptr, allocn] = ::ufio::containers::details::string_allocate_init<allocator_type>(otherbegin, stringlength);
...
private:
	constexpr void assign_impl(char_type const *otherptr, ::std::size_t othern) noexcept
	{
		...
		{
			this->destroy();
			auto [ptr, allocn] = ::ufio::containers::details::string_allocate_init<allocator_type>(otherptr, othern);
			this->imp.end_ptr = (this->imp.begin_ptr = thisbegin = ptr) + allocn;
		}
		*(this->imp.curr_ptr = ::ufio::freestanding::overlapped_copy_n(otherptr, othern, thisbegin)) = 0;
	}
...
public:
	...
	constexpr void assign_characters(size_type n, char_type ch) noexcept
	{
		...
		*(this->imp.curr_ptr = ::ufio::freestanding::uninitialized_fill_n(beginptr, n, ch)) = 0;
	}
...
private:
	...
	constexpr pointer insert_cold_impl(pointer insertpos, char_type const *otherptr, size_type othern) noexcept
	{
		...
		auto it{ptr};
		it = ::ufio::freestanding::non_overlapped_copy(beginptr, insertpos, it);
		auto retit{it};
		it = ::ufio::freestanding::non_overlapped_copy_n(otherptr, othern, it);
		it = ::ufio::freestanding::non_overlapped_copy(insertpos, currptr, it);
		*it = 0;
	...
	constexpr void append_cold_impl(char_type const *otherptr, size_type othern) noexcept
	{
		...
		this->imp.end_ptr = ptr + static_cast<size_type>(allocn - 1u);
		auto it{ptr};
		it = ::ufio::freestanding::non_overlapped_copy(beginptr, currptr, it);
		it = ::ufio::freestanding::non_overlapped_copy_n(otherptr, othern, it);
		*it = 0;
		this->imp.curr_ptr = it;
		if (!is_sso)
	...
	constexpr void append_impl(char_type const *otherptr, size_type othern) noexcept
	{
		...
		*(this->imp.curr_ptr = ::ufio::freestanding::overlapped_copy_n(otherptr, othern, currptr)) = 0;
	}
...
private:
	...
	{
		::ufio::containers::details::string_push_back_heap_grow_twice<allocator_type>(this->imp, __builtin_addressof(this->nullterminator));
	}
...
public:
	...
	constexpr void reserve(size_type new_cap) noexcept
	{
		...
		::ufio::containers::details::string_heap_dilate_uncheck<allocator_type>(this->imp, new_cap, __builtin_addressof(this->nullterminator));
	}
	...
	constexpr void shrink_to_fit() noexcept
	{
		...
		::ufio::containers::details::string_heap_dilate_uncheck<allocator_type>(this->imp, static_cast<size_type>(this->imp.curr_ptr - this->imp.begin_ptr), __builtin_addressof(this->nullterminator));
	}
...
private:
	constexpr pointer insert_impl(pointer ptr, char_type const *otherptr, size_type othern) noexcept
	{
		...
		*newcurrptr = 0;
		auto lastptr{::ufio::freestanding::copy_backward(ptr, currptr, newcurrptr)};
		auto retptr{::ufio::freestanding::copy_backward(otherptr, otherptr + othern, lastptr)};
...
private:
	...
	{
		*(this->imp.curr_ptr = ::ufio::freestanding::my_copy(last, this->imp.curr_ptr, first)) = 0;
		return first;
	}
...
private:
	...
	constexpr pointer replace_cold_impl(pointer first, pointer last, const_pointer otherptr, size_type othern) noexcept
	{
		...
		this->imp.end_ptr = ptr + static_cast<size_type>(allocn - 1u);
		auto it{ptr};
		it = ::ufio::freestanding::non_overlapped_copy(beginptr, first, it);
		auto retit{it};
		it = ::ufio::freestanding::non_overlapped_copy_n(otherptr, othern, it);
		it = ::ufio::freestanding::non_overlapped_copy(last, currptr, it);
		*it = 0;
		this->imp.curr_ptr = it;
		if (!is_sso)
		{
	...
	constexpr pointer replace_impl(pointer first, pointer last, const_pointer otherdata, size_type othersize) noexcept
	{
		...
		{
			auto itr{::ufio::freestanding::overlapped_copy_n(otherdata, othersize, first)};
			if (itr != last)
			{
				*(this->imp.curr_ptr = ::ufio::freestanding::copy_backward(itr, last, currptr)) = 0;
			}
			return first;
		}
...
public:
	...
	inline constexpr cstring_view_type subview_back(size_type count) const noexcept
	{
		...
		return cstring_view_type(::ufio::containers::null_terminated, beginptr + (thisn - count), count);
	}
	...
	[[msvc::forceinline]]
#endif
	inline constexpr string_view_type subview(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
		auto beginptr{this->imp.begin_ptr};
		auto currptr{this->imp.curr_ptr};
		size_type const thisn{static_cast<size_type>(currptr - beginptr)};
		...
		if (val < count)
		{
			if (count != ::ufio::containers::npos) [[unlikely]]
			{
				::ufio::fast_terminate();
			}
			count = val;
	...
	[[msvc::forceinline]]
#endif
	inline constexpr string_view_type subview_unchecked(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
		auto beginptr{this->imp.begin_ptr};
		if (count == ::ufio::containers::npos)
		{
			auto currptr{this->imp.curr_ptr};
			size_type const thisn{static_cast<size_type>(currptr - beginptr)};
			size_type const val{thisn - pos};
	...
	inline constexpr basic_string substr(size_type idx, size_type count = ::ufio::containers::npos) const noexcept
	{
	...
	inline constexpr void resize_and_overwrite(size_type count, Operation op) noexcept
	{
		...
		if (thissize < count)
		{
			...
			if (thiscap < count)
			{
				::ufio::containers::details::string_heap_dilate_uncheck<allocator_type>(this->imp, count, __builtin_addressof(this->nullterminator));
	...
	inline constexpr void resize(size_type count, char_type ch) noexcept
	{
		...
		*(this->imp.curr_ptr = ::ufio::freestanding::uninitialized_fill_n(currptr, static_cast<size_type>(count - thissize), ch)) = 0;
	}
...
inline constexpr void erase(::ufio::containers::basic_string<chtype, allocator1> const &c, U const &value)
{
...
inline constexpr void erase_if(::ufio::containers::basic_string<chtype, allocator1> const &c, Pred pred)
{
...
constexpr bool operator==(::ufio::containers::basic_string<chtype, allocator1> const &lhs, ::ufio::containers::basic_string<chtype, allocator2> const &rhs) noexcept
{
	return ::std::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}

template <::std::integral chtype, typename allocator1>
constexpr bool operator==(::ufio::containers::basic_string<chtype, allocator1> const &lhs, ::ufio::containers::basic_string_view<chtype> rhs) noexcept
{
	return ::std::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}

template <::std::integral chtype, typename allocator1>
constexpr bool operator==(::ufio::containers::basic_string_view<chtype> lhs, ::ufio::containers::basic_string<chtype, allocator1> const &rhs) noexcept
{
	return ::std::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}

template <::std::integral char_type, typename allocator1, ::std::size_t n>
constexpr bool operator==(::ufio::containers::basic_string<char_type, allocator1> a, char_type const (&buffer)[n]) noexcept
{
...
template <::std::integral char_type, typename allocator1, ::std::size_t n>
constexpr bool operator==(char_type const (&buffer)[n], ::ufio::containers::basic_string<char_type, allocator1> a) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral chtype, typename allocator1, typename allocator2>
constexpr auto operator<=>(::ufio::containers::basic_string<chtype, allocator1> const &lhs, ::ufio::containers::basic_string<chtype, allocator2> const &rhs) noexcept
{
	return ::std::lexicographical_compare_three_way(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend(), ::std::compare_three_way{});
}

template <::std::integral chtype, typename allocator1>
constexpr auto operator<=>(::ufio::containers::basic_string<chtype, allocator1> const &lhs, ::ufio::containers::basic_string_view<chtype> rhs) noexcept
{
	return ::std::lexicographical_compare_three_way(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend(), ::std::compare_three_way{});
}

template <::std::integral chtype, typename allocator1>
constexpr auto operator<=>(::ufio::containers::basic_string_view<chtype> lhs, ::ufio::containers::basic_string<chtype, allocator1> const &rhs) noexcept
{
	return ::std::lexicographical_compare_three_way(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend(), ::std::compare_three_way{});
}

template <::std::integral char_type, typename allocator1, ::std::size_t n>
constexpr auto operator<=>(::ufio::containers::basic_string<char_type, allocator1> a, char_type const (&buffer)[n]) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral char_type, typename allocator1, ::std::size_t n>
constexpr auto operator<=>(char_type const (&buffer)[n], ::ufio::containers::basic_string<char_type, allocator1> a) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral chtype, typename alloctype>
inline constexpr ::ufio::basic_io_scatter_t<chtype>
...
{
	return basic_string<char_type, allocator_type>(::ufio::containers::basic_string_view<char_type>(first, static_cast<::std::size_t>(last - first)));
}
...
template <::std::integral chtype, typename alloctype>
inline constexpr chtype *strlike_begin(::ufio::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype> &str) noexcept
{
	return str.imp.begin_ptr;
}

template <::std::integral chtype, typename alloctype>
inline constexpr chtype *strlike_curr(::ufio::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype> &str) noexcept
{
	return str.imp.curr_ptr;
}

template <::std::integral chtype, typename alloctype>
inline constexpr chtype *strlike_end(::ufio::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype> &str) noexcept
{
	return str.imp.end_ptr;
}

template <::std::integral chtype, typename alloctype>
inline constexpr void strlike_set_curr(::ufio::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype> &str, chtype *p) noexcept
{
	*(str.imp.curr_ptr = p) = 0;
}

template <::std::integral chtype, typename alloctype>
inline constexpr void strlike_reserve(::ufio::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype> &str, ::std::size_t n) noexcept
{
	str.reserve(n);
}

template <::std::integral chtype, typename alloctype>
inline constexpr ::ufio::io_strlike_reference_wrapper<chtype, basic_string<chtype, alloctype>> io_strlike_ref(::ufio::io_alias_t, basic_string<chtype, alloctype> &str) noexcept
{
	return {__builtin_addressof(str)};
}

template <::std::integral chtype, typename alloctype>
inline constexpr void swap(::ufio::containers::basic_string<chtype, alloctype> &a, ::ufio::containers::basic_string<chtype, alloctype> &b) noexcept
{
	a.swap(b);
}
