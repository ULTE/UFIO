...
inline void *grow_to_byte_size_iter_impl(vector_model &imp, void *iter, ::std::size_t newcap, ::std::size_t gap, ::std::size_t size, ::std::size_t alignment) noexcept
{
	...
	auto newiter = ::ufio::freestanding::nonoverlapped_bytes_copy(reinterpret_cast<::std::byte const *>(old_begin_ptr), reinterpret_cast<::std::byte const *>(iter),
																	 reinterpret_cast<::std::byte *>(begin_ptr));
	::ufio::freestanding::nonoverlapped_bytes_copy(reinterpret_cast<::std::byte const *>(iter), reinterpret_cast<::std::byte const *>(old_curr_ptr),
...
inline constexpr void *move_backward_impl(vector_model &imp, void *iter, ::std::size_t size, ::std::size_t alignment) noexcept
{
	...
	::ufio::freestanding::uninitialized_move_backward(reinterpret_cast<::std::byte *>(iter), currptr, currptrp1);
	return iter;
}
...
inline constexpr void *grow_to_size_iter_impl(vector_model &imp, void *iter, ::std::size_t newcap) noexcept
{
	...
	{
		return ::ufio::containers::details::vector::grow_to_size_iter_impl<allocator, size, allocator::default_alignment>(imp, iter, newcap);
	}
	else
	{
		return ::ufio::containers::details::vector::detemplate::grow_to_size_iter_impl<allocator>(imp, iter, newcap, size, alignment);
	}
}
...
inline constexpr void *grow_to_size_impl(vector_model &imp, ::std::size_t newcap) noexcept
{
	if constexpr (alignment < allocator::default_alignment)
	{
		return ::ufio::containers::details::vector::grow_to_size_impl<allocator, size, allocator::default_alignment>(imp, newcap);
	}
	else
	{
		return ::ufio::containers::details::vector::detemplate::grow_to_size_impl<allocator>(imp, newcap, size, alignment);
	}
}
...
inline constexpr void *grow_twice_impl(vector_model &imp) noexcept
{
	if constexpr (alignment < allocator::default_alignment)
	{
		return ::ufio::containers::details::vector::grow_twice_impl<allocator, size, allocator::default_alignment>(imp);
	}
	else
	{
		return ::ufio::containers::details::vector::detemplate::grow_twice_impl<allocator>(imp, size, alignment);
	}
}
...
inline constexpr void *grow_twice_iter_impl(vector_model &imp, void *iter) noexcept
{
	if constexpr (alignment < allocator::default_alignment)
	{
		return ::ufio::containers::details::vector::grow_twice_iter_impl<allocator, size, allocator::default_alignment>(imp, iter);
	}
	else
	{
		return ::ufio::containers::details::vector::detemplate::grow_twice_iter_impl<allocator>(imp, iter, size, alignment);
	}
}
...
inline constexpr void *move_backward_impl(vector_model &imp, void *iter) noexcept
{
	if constexpr (alignment < allocator::default_alignment)
	{
		return ::ufio::containers::details::vector::move_backward_impl<allocator, size, allocator::default_alignment>(imp, iter);
	}
	else
	{
		return ::ufio::containers::details::vector::detemplate::move_backward_impl<allocator>(imp, iter, size, alignment);
	}
}
...
public:
	...
	::ufio::containers::details::vector_internal<T> imp;
...
public:
	explicit constexpr vector(size_type n) noexcept(::ufio::freestanding::is_zero_default_constructible_v<value_type> || noexcept(value_type()))
	{
		if constexpr (::ufio::freestanding::is_zero_default_constructible_v<value_type>)
		{
	...
	explicit constexpr vector(size_type n, ::ufio::for_overwrite_t) noexcept(::std::is_trivially_default_constructible_v<value_type> || ::ufio::freestanding::is_zero_default_constructible_v<value_type> || noexcept(value_type()))
	{
		...
		else if constexpr (::ufio::freestanding::is_zero_default_constructible_v<value_type>)
		{
			imp.begin_ptr = typed_allocator_type::allocate_zero(n);
			imp.end_ptr = imp.curr_ptr = imp.begin_ptr + n;
		}
	...
	explicit constexpr vector(::ufio::freestanding::from_range_t, R &&rg)
	{
...
#endif
			{
				::ufio::freestanding::nonoverlapped_bytes_copy(
...
private:
	...
	{
		if constexpr (::ufio::freestanding::is_trivially_relocatable_v<value_type>)
		{
#if (__cpp_if_consteval >= 202106L || __cpp_lib_is_constant_evaluated >= 201811L) && __cpp_constexpr_dynamic_alloc >= 201907L
#if __cpp_if_consteval >= 202106L
			if !consteval
...
#endif
			{
				return reinterpret_cast<pointer>(::ufio::containers::details::vector::grow_to_size_iter_impl<allocator_type, sizeof(value_type), alignof(value_type)>(*reinterpret_cast<::ufio::containers::details::vector_model *>(__builtin_addressof(imp)),
																																										 iter, n));
			}
		}
		...
		auto newiter{::ufio::freestanding::uninitialized_relocate(old_begin_ptr, iter, new_begin_ptr)};
		::ufio::freestanding::uninitialized_relocate(iter, old_curr_ptr, newiter + n);
		if constexpr (typed_allocator_type::has_deallocate)
		{
...
#endif
			{
				return reinterpret_cast<pointer>(::ufio::containers::details::vector::grow_twice_iter_impl<allocator_type, sizeof(value_type), alignof(value_type)>(*reinterpret_cast<::ufio::containers::details::vector_model *>(__builtin_addressof(imp)),
																																									   iter));
			}
		}
		std::size_t const cap{static_cast<size_type>(imp.end_ptr - imp.begin_ptr)};
		return this->grow_to_size_iter_impl(::ufio::containers::details::cal_grow_twice_size<sizeof(value_type), false>(cap), iter, 1);
	}
	constexpr pointer move_backward_common_impl(pointer iter) noexcept
	{
...
#endif
			{
				return reinterpret_cast<pointer>(::ufio::containers::details::vector::move_backward_impl<allocator, sizeof(value_type), alignof(value_type)>(
					*reinterpret_cast<::ufio::containers::details::vector_model *>(__builtin_addressof(imp)),
					iter));
			}
		}
		if (imp.curr_ptr == imp.end_ptr) [[unlikely]]
		...
		::ufio::freestanding::uninitialized_move_backward(iter, currptr, currptrp1);
		return iter;
	}
...
#endif
			{
				::ufio::containers::details::vector::grow_to_size_impl<allocator_type, sizeof(value_type), alignof(value_type)>(
					*reinterpret_cast<::ufio::containers::details::vector_model *>(__builtin_addressof(imp)), newcap);
				return;
			}
		}
...
#endif
			{
				::ufio::containers::details::vector::grow_twice_impl<allocator_type, sizeof(value_type), alignof(value_type)>(
					*reinterpret_cast<::ufio::containers::details::vector_model *>(__builtin_addressof(imp)));
				return;
			}
		}
		std::size_t const cap{static_cast<size_type>(imp.end_ptr - imp.begin_ptr)};
		grow_to_size_impl(::ufio::containers::details::cal_grow_twice_size<sizeof(value_type), false>(cap));
	}
...
private:
	constexpr pointer erase_common(pointer it) noexcept
	{
		...
		::ufio::freestanding::uninitialized_relocate(it + 1, lastele, it);
	...
	constexpr pointer erase_iters_common(pointer first, pointer last) noexcept
	{
		...
		imp.curr_ptr = ::ufio::freestanding::uninitialized_relocate(last, currptr, first);
		return first;
	}
...
struct is_trivially_relocatable<::ufio::containers::vector<T, Alloc>>
{
	inline static constexpr bool value = true;
};

template <typename T, typename Alloc>
struct is_zero_default_constructible<::ufio::containers::vector<T, Alloc>>
{
	inline static constexpr bool value = true;
};
