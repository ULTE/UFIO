...
namespace ufio::operations::defines
{
template <typename T>
...
#endif
			{
				return reinterpret_cast<Iter2>(::ufio::freestanding::bytes_copy(reinterpret_cast<::std::byte const *>(first), reinterpret_cast<::std::byte const *>(last), reinterpret_cast<::std::byte *>(dest)));
			}
		}
...
#endif
		{
			::ufio::details::overlapped_copy_buffer_ptr<iter2valuetype> tempbuffer(static_cast<::std::size_t>(::std::distance(first, last)));
...
constexpr Iter2 uninitialized_relocate(Iter1 first, Iter1 last, Iter2 dest) noexcept
{
	...
	else
	{
		...
		using iter2valuetype = ::std::iter_value_t<Iter2>;
		if constexpr (::std::is_pointer_v<Iter1> && ::std::is_pointer_v<Iter2> &&
					  (::ufio::freestanding::is_trivially_relocatable_v<iter1valuetype> &&
					   ::ufio::freestanding::is_trivially_relocatable_v<iter2valuetype> &&
					   (::std::same_as<iter1valuetype, iter2valuetype> ||
						((::std::integral<iter1valuetype> || ::std::same_as<iter1valuetype, ::std::byte>) &&
						 (::std::integral<iter2valuetype> || ::std::same_as<iter2valuetype, ::std::byte>) &&
						 sizeof(iter1valuetype) == sizeof(iter2valuetype)))))
		...
		else if constexpr (::std::same_as<iter1valuetype, iter2valuetype> && ::ufio::operations::defines::has_uninitialized_relocate_define<iter1valuetype>)
		{
...
constexpr Iter2 uninitialized_move(Iter1 first, Iter1 last, Iter2 dest) noexcept
{
	...
	else
	{
		...
		else if constexpr (::std::same_as<iter1valuetype, iter2valuetype> && ::ufio::operations::defines::has_uninitialized_move_backward_define<iter1valuetype>)
		{
	...
	else
	{
						 ...
						 sizeof(iter1valuetype) == sizeof(iter2valuetype)))))
		{
...
#endif
			{
				...
				::ufio::freestanding::bytes_copy(reinterpret_cast<::std::byte const *>(first), reinterpret_cast<::std::byte const *>(last), reinterpret_cast<::std::byte *>(d_start));
				return d_start;
			}
		}
		else if constexpr (::std::same_as<iter1valuetype, iter2valuetype> && ::ufio::operations::defines::has_uninitialized_move_backward_define<iter1valuetype>)
...
{
	return ::ufio::freestanding::uninitialized_fill(first, first + n, ele);
}
