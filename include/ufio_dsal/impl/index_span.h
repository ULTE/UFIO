...
public:
	...
	using span_type = ::ufio::containers::span<element_type>;
	...
	explicit constexpr index_span(::ufio::containers::index_unchecked_t, Iter first) noexcept(noexcept(::std::to_address(first)))
		: ptr{::std::to_address(first)}
	{}
	template <::std::ranges::contiguous_range R>
		requires(::std::same_as<value_type, ::std::ranges::range_value_t<R>>)
	explicit constexpr index_span(::ufio::containers::index_unchecked_t, R &&range) noexcept(noexcept(::std::ranges::data(range)))
		: ptr{::std::ranges::data(range)}
	{}
	template <::std::ranges::contiguous_range R>
		requires(::std::same_as<value_type, ::std::ranges::range_value_t<R>> && !::std::same_as<::std::remove_cvref_t<R>, ::ufio::containers::index_span<element_type, N>>)
	...
	[[msvc::forceinline]]
#endif
	inline constexpr ::ufio::containers::span<element_type> subspan_front(size_type count) const noexcept
	{
		if (N < count) [[unlikely]]
		{
			::ufio::fast_terminate();
		}
		return ::ufio::containers::span<element_type>(this->ptr, count);
	}
	...
	[[msvc::forceinline]]
#endif
	inline constexpr ::ufio::containers::span<element_type> subspan_front_unchecked(size_type count) const noexcept
	{
		return ::ufio::containers::span<element_type>(this->ptr, count);
	}
	...
	[[msvc::forceinline]]
#endif
	inline constexpr span_type subspan(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
	...
	[[msvc::forceinline]]
#endif
	inline constexpr span_type subspan_unchecked(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
		size_type const val{this->n - pos};
...
template <typename T, ::std::size_t N>
inline constexpr ::ufio::containers::index_span<::std::byte const, sizeof(T) * N> as_index_bytes(::ufio::containers::index_span<T, N> sp) noexcept
{
	return ::ufio::containers::index_span<::std::byte const, sizeof(T) * N>(static_cast<::std::byte const *>(static_cast<void const *>(sp.ptr)));
}

template <typename T, ::std::size_t N>
	requires(!::std::is_const_v<T>)
inline constexpr ::ufio::containers::index_span<::std::byte, sizeof(T) * N> as_index_writable_bytes(::ufio::containers::index_span<T, N> sp) noexcept
{
	return ::ufio::containers::index_span<::std::byte, sizeof(T) * N>(static_cast<::std::byte *>(static_cast<void *>(sp.ptr)));
}

template <typename T, ::std::size_t N>
inline constexpr ::ufio::containers::span<::std::byte const> as_bytes(::ufio::containers::index_span<T, N> sp) noexcept
{
	constexpr ::std::size_t bytescount{N * sizeof(T)};
	return ::ufio::containers::span<::std::byte const>(static_cast<::std::byte const *>(static_cast<void const *>(sp.ptr)), bytescount);
}

template <typename T, ::std::size_t N>
	requires(!::std::is_const_v<T>)
inline constexpr ::ufio::containers::span<::std::byte> as_writable_bytes(::ufio::containers::index_span<T, N> sp) noexcept
{
	constexpr ::std::size_t bytescount{N * sizeof(T)};
	return ::ufio::containers::span<::std::byte>(static_cast<::std::byte *>(static_cast<void *>(sp.ptr)), bytescount);
}

template <typename T, ::std::size_t N1, ::std::size_t N2>
	requires ::std::equality_comparable<T>
constexpr bool operator==(::ufio::containers::index_span<T, N1> a, ::ufio::containers::index_span<T, N2> b)
{
	if constexpr (N1 != N2)
	{
		return false;
...
constexpr auto operator<=>(::ufio::containers::index_span<T, N1> a, ::ufio::containers::index_span<T, N2> b)
{
