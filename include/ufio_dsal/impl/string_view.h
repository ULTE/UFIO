...
public:
	...
	explicit constexpr basic_string_view(::ufio::freestanding::from_range_t, rg &&r) noexcept
		: ptr{::std::ranges::cdata(r)}, n{::std::ranges::size(r)}
	{
	}

	constexpr basic_string_view(::ufio::manipulators::basic_os_c_str<char_type> osstr) noexcept
		: ptr(osstr.ptr), n(::ufio::cstr_len(osstr.ptr))
	{}
	constexpr basic_string_view(::ufio::manipulators::basic_os_c_str_with_known_size<char_type> osstr) noexcept
		: ptr(osstr.ptr), n(osstr.n)
	{}
	constexpr basic_string_view(::ufio::manipulators::basic_os_str_known_size_without_null_terminated<char_type> osstr) noexcept
		: ptr(osstr.ptr), n(osstr.n)
	{}
	...
	[[msvc::forceinline]]
#endif
	inline constexpr basic_string_view subview(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
	...
	[[msvc::forceinline]]
#endif
	inline constexpr basic_string_view subview_unchecked(size_type pos, size_type count = ::ufio::containers::npos) const noexcept
	{
		size_type const val{this->n - pos};
...
private:
	...
	inline constexpr size_type find_character_common(char_type ch, size_type pos) const noexcept
	{
		...
		{
			it = ::ufio::freestanding::find_not(bg, ed, ch);
		}
		else if constexpr (situation == 2)
		{
			it = ::ufio::freestanding::find_last(bg, ed, ch);
		}
		else if constexpr (situation == 3)
		{
			it = ::ufio::freestanding::find_last_not(bg, ed, ch);
		}
		if (it == ed)
		{
			return ::ufio::containers::npos;
	...
	inline constexpr size_type find_common(const_pointer s, size_type pos, size_type count) const noexcept
	{
		...
		if (count == 0)
		{
			return pos <= thisn ? pos : ::ufio::containers::npos;
		}
		if (thisn <= pos)
		{
			return ::ufio::containers::npos;
	...
	inline constexpr size_type find_first_last_common(const_pointer s, size_type pos, size_type count) const noexcept
	{
		...
		{
			it = ::ufio::freestanding::find_first_not_of(start, ed, s, sed);
		}
		else if constexpr (situation == 2)
		{
			it = ::ufio::freestanding::find_last_of(start, ed, s, sed);
		}
		else if constexpr (situation == 3)
		{
			it = ::ufio::freestanding::find_last_not_of(start, ed, s, sed);
		}
		if (it == ed)
		{
			return ::ufio::containers::npos;
...
template <::std::integral char_type>
constexpr void swap(::ufio::containers::basic_string_view<char_type> &a, ::ufio::containers::basic_string_view<char_type> &b) noexcept
{
	a.swap(b);
}

template <::std::integral char_type>
inline constexpr ::ufio::basic_io_scatter_t<char_type> print_alias_define(::ufio::io_alias_t, ::ufio::containers::basic_string_view<char_type> str) noexcept
{
	return {str.ptr, str.n};
}

template <::std::integral char_type>
constexpr bool operator==(::ufio::containers::basic_string_view<char_type> a, ::ufio::containers::basic_string_view<char_type> b) noexcept
{
	return ::std::equal(a.ptr, a.ptr + a.n, b.ptr, b.ptr + b.n);
}

template <::std::integral char_type, ::std::size_t n>
constexpr bool operator==(::ufio::containers::basic_string_view<char_type> a, char_type const (&buffer)[n]) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral char_type, ::std::size_t n>
constexpr bool operator==(char_type const (&buffer)[n], ::ufio::containers::basic_string_view<char_type> a) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral char_type>
constexpr auto operator<=>(::ufio::containers::basic_string_view<char_type> a, ::ufio::containers::basic_string_view<char_type> b) noexcept
{
	return ::std::lexicographical_compare_three_way(a.ptr, a.ptr + a.n, b.ptr, b.ptr + b.n, ::std::compare_three_way{});
}

template <::std::integral char_type, ::std::size_t n>
constexpr auto operator<=>(::ufio::containers::basic_string_view<char_type> a, char_type const (&buffer)[n]) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
...
template <::std::integral char_type, ::std::size_t n>
constexpr auto operator<=>(char_type const (&buffer)[n], ::ufio::containers::basic_string_view<char_type> a) noexcept
{
	constexpr ::std::size_t nm1{n - 1u};
