...
public:
	...
	{
		::ufio::details::create_nt_bcrypt_impl(*this, s);
	}
	...
	~nt_bcrypt_hash_file()
	{
		if (digest_buffer)
		{
			::ufio::details::deallocate_with_secure_clear<false>(digest_buffer, bcrypt_size);
		}
		if (this->hashHandle)
		{
			::ufio::win32::BCryptDestroyHash(this->hashHandle);
		}
		if (this->phAlgorithm)
		{
			::ufio::win32::BCryptCloseAlgorithmProvider(this->phAlgorithm, 0u);
		}
	}
	void update(::std::byte const *first, ::std::byte const *last)
	{
		::ufio::details::ntbcrypt_update_impl(this->hashHandle, first, last);
	}
	void do_final()
	{
		::ufio::details::ntbcrypt_do_final_impl(this->hashHandle, digest_buffer, bcrypt_size);
	}
	void reset()
	{
		this->do_final();
...
struct bcrypt_algo_guard
{
	...
	~bcrypt_algo_guard()
	{
		if (this->phalgo)
		{
			::ufio::win32::BCryptCloseAlgorithmProvider(this->phalgo, 0u);
		}
	}
...
inline void create_bcrypt_common_hash_impl(nt_bcrypt_hash_file &g, char16_t const *name)
{
	...
	::std::uint_least32_t status{
		::ufio::win32::BCryptOpenAlgorithmProvider(__builtin_addressof(phalgo), name, nullptr, 0u)};
	if (status)
	{
	...
	status = ::ufio::win32::BCryptGetProperty(phalgo, u"HashDigestLength", __builtin_addressof(hash_digest_length),
												 sizeof(hash_digest_length), __builtin_addressof(result_length), 0u);
	if (status)
	{
		throw_nt_error(status);
	}
	::ufio::details::local_operator_new_array_ptr<::std::byte> locarr(hash_digest_length);
	void *hash_handle{};
	status = ::ufio::win32::BCryptCreateHash(phalgo, __builtin_addressof(hash_handle), nullptr, 0u, nullptr, 0u,
												0x00000020);
	if (status)
	{
		throw_nt_error(status);
...
inline void ntbcrypt_update_impl(void *hashHandle, ::std::byte const *first, ::std::byte const *last)
{
	...
	{
		::std::uint_least32_t ntstatus{::ufio::win32::BCryptHashData(
	...
	else
	{
		...
		for (::std::size_t sz{static_cast<::std::size_t>(last - first)}; sz;)
		{
			...
			::std::uint_least32_t ntstatus{
				::ufio::win32::BCryptHashData(hashHandle, first, static_cast<::std::uint_least32_t>(mn), 0u)};
			if (ntstatus)
			{
				throw_nt_error(ntstatus);
			}
...
inline void ntbcrypt_do_final_impl(void *hashhandle, ::std::byte *buffer, ::std::uint_least32_t bcrypt_size)
{
	::std::uint_least32_t ntstatus{::ufio::win32::BCryptFinishHash(hashhandle, buffer, bcrypt_size, 0u)};
	if (ntstatus)
	{
		throw_nt_error(ntstatus);
	}
