...
namespace ufio::tls::cipher_suite
{

struct cipher_suite_type
{
	::ufio::freestanding::array<::std::byte, 2> array{};
};
...
inline constexpr void print_define(output &outp, cipher_suite_type const &e)
{
	...
	{
		print_freestanding(outp, u8"Unknown TLS cipher suite{", e.array.front(), ufio::manipulators::chvw(u8','),
						   e.array.back(), ufio::manipulators::chvw(u8'}'));
	}
}
