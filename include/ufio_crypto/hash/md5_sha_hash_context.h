...
struct md5_sha_common_impl
{
	...
	using counter_type = ::std::conditional_t<counterbits == 64, ::std::uint_least64_t,
											  ...
											  __uint128_t
#else
											  ::ufio::details::pesudo_uint_least128_t
#endif
											  >;
	T hasher;
	...
	constexpr void update_impl(::std::byte const *first, ::std::size_t blocks_bytes) noexcept
	{
		...
		if constexpr (::std::same_as<counter_type, ::ufio::details::pesudo_uint_least128_t>)
		{
			...
			using namespace ::ufio::details::intrinsics;
	...
	constexpr void update_cold_impl(::std::byte const *first, ::std::size_t diff) noexcept
	{
		...
		{
			::ufio::details::non_overlapped_copy_n(first, buffer_remain_space, buffer + buffer_space);
		...
		first += blocks_bytes;
		diff -= blocks_bytes;
		::ufio::details::non_overlapped_copy_n(first, diff, buffer);
		buffer_offset = diff;
	}
		...
		append_sentinal(::std::size_t final_block_offset) noexcept
	{
		...
		if constexpr (::std::endian::native != hash_endian)
		{
			if constexpr (::std::same_as<counter_type, ::ufio::details::pesudo_uint_least128_t>)
			{
				auto t{::ufio::byte_swap(ct.low)};
				ct.low = ::ufio::byte_swap(ct.high);
				ct.high = t;
			}
			else
			{
				ct = ::ufio::byte_swap(ct);
			}
		}
		constexpr ::std::size_t start_pos{block_size - sizeof(counter_type)};
		::ufio::freestanding::type_punning_to_bytes(ct, buffer + start_pos);
...
#endif
#endif
			::ufio::details::non_overlapped_copy_n(block_first, diff, buffer + buffer_offset);
			buffer_offset += diff;
		}
		else
		{
		...
		do_final() noexcept
	{
...
#endif
#endif
			::ufio::none_secure_clear(this->buffer + buffer_off, to_fill);
		...
		::ufio::none_secure_clear(this->buffer, sz);
		this->append_sentinal(buffer_offs);
	}
};
	...
	hash_digest_to_byte_ptr_common_impl(U const *digest, ::std::size_t n, ::std::byte *ptr) noexcept
{
...
#endif
	{
		for (::std::size_t i{}; i != n; ++i)
		{
			...
			::ufio::freestanding::array<::std::byte, usz> va{
				::ufio::bit_cast<::ufio::freestanding::array<::std::byte, usz>>(digest[i])};
			ptr = ::ufio::freestanding::nonoverlapped_bytes_copy_n(va.data(), usz, ptr);
		}
	}
	else
#endif
	{
		if constexpr (::std::endian::native == end)
		{
			::ufio::details::my_memcpy(ptr, digest, n * sizeof(U));
		}
		else
		{
			for (::std::size_t i{}; i != n; ++i)
			{
				auto v{::ufio::byte_swap(digest[i])};
				::ufio::details::my_memcpy(ptr, __builtin_addressof(v), usz);
				ptr += usz;
			}
		}
	}
...
inline constexpr void hash_digest_to_byte_ptr_simd16_impl(U const *digest, ::std::size_t n, ::std::byte *ptr) noexcept
{
...
#endif
	{
		...
		else
		{
			...
			::ufio::intrinsics::simd_vector<U, factor> s;
...
inline constexpr void hash_digest_to_byte_ptr_common(digest_value_type const *digest, ::std::byte *ptr) noexcept
{
	...
	if constexpr (remainder != 0)
	{
		...
		::ufio::freestanding::type_punning_to_bytes_n<remainder>(v, ptr);
	}
}
