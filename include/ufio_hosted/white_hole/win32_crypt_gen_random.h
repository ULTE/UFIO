...
inline ::std::byte *win32_crypt_gen_random_some_impl(::std::size_t hprov, ::std::byte *first, ::std::byte *last)
{
	if constexpr (sizeof(::std::size_t) <= sizeof(::std::uint_least32_t))
	{
		...
		if (!::ufio::win32::CryptGenRandom(hprov, static_cast<::std::uint_least32_t>(sz),
	...
	else
	{
		...
		while (first != last)
		{
			...
			if (!::ufio::win32::CryptGenRandom(hprov, static_cast<::std::uint_least32_t>(toreadthisround),
												  reinterpret_cast<char unsigned *>(first)))
			{
				return first;
			}
...
{
	return ::ufio::win32::details::win32_crypt_gen_random_some_impl(bcgiob.hprov, first, last);
}
...
inline void read_all_bytes_underflow_define(basic_win32_crypt_gen_random_io_observer<char_type> bcgiob,
											::std::byte *first, ::std::byte *last)
{
	::ufio::win32::details::win32_crypt_gen_random_all_impl(bcgiob.hprov, first, last);
}
...
public:
	...
	basic_win32_family_crypt_gen_random_file()
		: basic_win32_crypt_gen_random_io_observer<ch_type>{
			  ::ufio::details::win32::crypt_acquire_context_fallback<family>()}
	{
	}
	...
	operator=(basic_win32_family_crypt_gen_random_file &&__restrict other) noexcept
	{
		...
		{
			::ufio::win32::CryptReleaseContext(this->hprov, 0);
		}
	...
	void close()
	{
		if (this->hprov) [[likely]]
		{
			if (!::ufio::win32::CryptReleaseContext(this->hprov, 0))
			{
				throw_win32_error();
			}
		}
