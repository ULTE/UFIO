...
inline constexpr bool minimum_buffer_input_stream_require_size_constant_impl =
	(N < ibuffer_minimum_size_define(::ufio::io_reserve_type<typename input::input_char_type, input>));

template <typename input, ::std::size_t N>
concept minimum_buffer_input_stream_require_size_impl =
	::ufio::operations::decay::defines::has_ibuffer_minimum_size_operations<input> &&
...
#else
	if (::ufio::posix::ioctl(fd, RNDGETENTCNT, __builtin_addressof(ent)) != 0)
	{
		return 0.0;
	}
#endif
...
{
	return ::ufio::details::my_random_entropy(piob.fd);
}
#endif
...
template <::std::integral char_type, typename allocator_type = ::ufio::native_global_allocator>
using basic_ibuf_white_hole =
	basic_io_buffer<basic_native_white_hole<char_type>,
					::ufio::basic_io_buffer_traits<buffer_mode::in | buffer_mode::secure_clear, allocator_type,
...
struct basic_white_hole_engine
{
	...
	{
		if constexpr (::ufio::details::has_entroy_method_impl<handletype>)
		{
	...
	{
		result_type value;
		auto instmref{::ufio::operations::input_stream_ref(handle)};
		if constexpr (::ufio::details::minimum_buffer_input_stream_require_size_impl<decltype(instmref),
																						sizeof(result_type)>)
		{
			...
			::ufio::freestanding::my_memcpy(__builtin_addressof(value), currptr, objsz);
			ibuffer_set_curr(instmref, currptr + objsz);
		}
		else
		{
			::ufio::operations::decay::read_all_bytes_decay(
