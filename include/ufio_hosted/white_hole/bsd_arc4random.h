...
{
	::ufio::noexcept_call(arc4random_buf, first, static_cast<::std::size_t>(last - first));
}
...
{
	::ufio::details::bsd_arc4random_read_all_bytes_define_impl(first, last);
}
...
{
	constexpr auto *glibc_arc4random_bufptr{::ufio::details::glibc_arc4random_buf};
	if (glibc_arc4random_bufptr == nullptr)
	{
		return ::ufio::details::linux_getrandom_read_some_bytes_define_impl(0, first, last);
	}
	else
	{
...
inline ::std::byte *read_some_bytes_underflow_define(basic_bsd_arc4random<char_type>, ::std::byte *first,
													 ::std::byte *last)
{
	return ::ufio::details::bsd_arc4random_read_some_bytes_define_impl(first, last);
}
