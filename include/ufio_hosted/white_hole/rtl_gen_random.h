...
inline ::std::byte *rtl_gen_random_some_impl(::std::byte *first, ::std::byte *last)
{
	if constexpr (sizeof(::std::size_t) <= sizeof(::std::uint_least32_t))
	{
		...
		if (!::ufio::win32::SystemFunction036(first, static_cast<::std::uint_least32_t>(sz)))
		{
			throw_win32_error();
		}
		return last;
	...
	else
	{
		...
		while (first != last)
		{
			...
			if (!::ufio::win32::SystemFunction036(first, static_cast<::std::uint_least32_t>(toreadthisround)))
			{
				return first;
			}
...
inline ::std::byte *read_some_bytes_underflow_define(basic_rtl_gen_random<char_type>, ::std::byte *first,
													 ::std::byte *last)
{
	return ::ufio::win32::details::rtl_gen_random_some_impl(first, last);
}

template <::std::integral char_type>
inline void read_all_bytes_underflow_define(basic_rtl_gen_random<char_type>, ::std::byte *first, ::std::byte *last)
{
	::ufio::win32::details::rtl_gen_random_all_impl(first, last);
}
