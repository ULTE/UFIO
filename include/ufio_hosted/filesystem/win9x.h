...
{
	void *file_struct{};
	::ufio::u8string find_path{};

	::ufio::win9x_dir_handle d_handle{};
	file_type d_type{};
	[[maybe_unused]] ::std::uint_least64_t d_ino{};
	::ufio::u8string filename{};
...
inline bool set_win9x_dirent(win9x_dirent &entry, bool start)
{
	...
	if (start)
	{
		entry.find_path = ::ufio::u8concat_ufio(::ufio::mnp::code_cvt(entry.d_handle.path), u8"\\*");
		entry.file_struct = ::ufio::win32::FindFirstFileA(reinterpret_cast<char const *>(entry.find_path.c_str()), __builtin_addressof(wfda));
	}
	else
	{
		if (::ufio::win32::FindNextFileA(entry.file_struct, __builtin_addressof(wfda)) == 0) [[unlikely]]
		{
			return false;
		}
	}

	entry.filename = ::ufio::u8string{::ufio::u8concat_ufio(::ufio::mnp::code_cvt_os_c_str(wfda.cFileName))};
...
inline constexpr ::ufio::manipulators::basic_os_c_str_with_known_size<char8_t>
...
inline ::ufio::manipulators::basic_os_c_str_with_known_size<char8_t>
...
struct basic_win9x_directory_generator
{
	...
	explicit constexpr basic_win9x_directory_generator(::ufio::win9x_dir_handle directory_handle) noexcept
	{
...
struct basic_win9x_recursive_directory_iterator
{
	...
	::ufio::win9x_dir_handle root_handle{};
	...
	explicit constexpr basic_win9x_recursive_directory_iterator(::ufio::win9x_dir_handle rh, win9x_dirent *dp)
...
struct basic_win9x_recursive_directory_generator
{
	...
	explicit constexpr basic_win9x_recursive_directory_generator(::ufio::win9x_dir_handle rhd)
...
using win9x_recursive_directory_generator =
	basic_win9x_recursive_directory_generator<::ufio::containers::vector<win9x_dir_file_stack_type, ::ufio::win32_family_api_encoding_converter<win32_family::ansi_9x>::allocator_type>>;
		...
		if (prdit.entry->d_type == file_type::directory)
		{
			...
			prdit.stack.emplace_back(
				win9x_dir{win9x_at_entry{prdit.stack.empty() ? prdit.root_handle : prdit.stack.back().fd.handle},
						  ::ufio::manipulators::basic_os_c_str_with_known_size<char8_t>{native_d_name_ptr, native_d_namlen},
						  open_mode::directory},
				nullptr);
		}
		return prdit;
...
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<false, char8_t, char8_t>(et.filename.c_str(), et.filename.size());
}

inline auto native_stem(win9x_directory_entry ent) noexcept
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<true, char8_t, char8_t>(et.filename.c_str(), et.filename.size());
}
