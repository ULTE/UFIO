...
inline nt_dirent *set_nt_dirent(nt_dirent *entry, bool start)
{
	...
	::ufio::win32::nt::dir_information d_info{buffer};
	...
	::ufio::freestanding::nonoverlapped_bytes_copy_n(reinterpret_cast<::std::byte const *>(id_ful_dir_info->FileName),
	...
	entry->u8d_namlen = static_cast<::std::size_t>(
		::ufio::details::codecvt::general_code_cvt_full(enstart, enend, entry->u8d_name) - entry->u8d_name);
...
inline constexpr ::ufio::manipulators::basic_os_c_str_with_known_size<char16_t>
...
struct basic_nt_family_directory_generator
{
	...
	explicit constexpr basic_nt_family_directory_generator(void *directory_handle) noexcept
		: entry(::ufio::win32::nt::details::new_nt_dirent<Allocator>())
	{
	...
	{
		::ufio::win32::nt::details::delete_nt_dirent<Allocator>(entry);
		entry = other.entry;
		other.entry = nullptr;
		return *this;
	}
...
struct basic_nt_family_recursive_directory_generator
{
	...
	explicit constexpr basic_nt_family_recursive_directory_generator(void *rhd)
		: root_handle(rhd), entry(::ufio::win32::nt::details::new_nt_dirent<Allocator>())
	{
	}
	...
	{
		::ufio::win32::nt::details::delete_nt_dirent<Allocator>(this->entry);
	...
	for (;;)
	{
		...
		if (prdit.entry->d_type == file_type::directory)
		{
			...
			prdit.stack.emplace_back(
				...
				::ufio::manipulators::basic_os_c_str_with_known_size<char16_t>{native_d_name_ptr, native_d_namlen},
				open_mode::directory);
		}
		return prdit;
	}
...
using nt_family_recursive_directory_generator = basic_nt_family_recursive_directory_generator<
	...
	::ufio::containers::vector<basic_nt_family_file<family, char>, typename nt_directory_generator::allocator_type>>;
...
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<false, char16_t, char16_t>(et.native_d_name, et.native_d_namlen);
}

inline auto native_stem(nt_directory_entry ent) noexcept
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<true, char16_t, char16_t>(et.native_d_name, et.native_d_namlen);
}

inline auto u8extension(nt_directory_entry ent) noexcept
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<false, char8_t, char8_t>(et.u8d_name, et.u8d_namlen);
}

inline auto u8stem(nt_directory_entry ent) noexcept
{
	auto &et{*ent.entry};
	return ::ufio::details::find_dot_and_sep<true, char8_t, char8_t>(et.u8d_name, et.u8d_namlen);
}
...
inline constexpr auto status_io_print_forward(io_alias_type_t<char_type>, nt_directory_entry ent) noexcept
{
	...
	if constexpr (sizeof(char_type) == 1 ||
				  (::std::same_as<char_type, char> &&
				   ::ufio::execution_charset_encoding_scheme<char>() == ::ufio::encoding_scheme::utf))
	{
		return status_io_print_forward(io_alias_type<char_type>,
									   ::ufio::cross_code_cvt_t<char8_t>{{et.u8d_name, et.u8d_namlen}});
	}
	else
	{
		return status_io_print_forward(io_alias_type<char_type>,
									   ::ufio::cross_code_cvt_t<char16_t>{{et.native_d_name, et.native_d_namlen}});
	}
}
