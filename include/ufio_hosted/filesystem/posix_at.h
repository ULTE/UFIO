...
#else
	if ((::ufio::posix::libc_fstatat(dirfd, pathname, __builtin_addressof(buf), flags)) < 0)
	{
		throw_posix_error();
	}
#endif
...
#else
	struct stat buf;
	system_call_throw_error(::ufio::posix::libc_fstatat(dirfd, pathname, __builtin_addressof(buf), flags));
#endif
	return ::ufio::details::struct_stat_to_posix_file_status(buf);
}
...
template <posix_api_22 dsp, ::ufio::constructible_to_os_c_str old_path_type,
...
template <posix_api_12 dsp, ::ufio::constructible_to_os_c_str old_path_type,
		  ::ufio::constructible_to_os_c_str new_path_type>
...
template <posix_api_1x dsp, ::ufio::constructible_to_os_c_str path_type, typename... Args>
inline auto posix_deal_with1x(int dirfd, path_type const &path, Args... args)
{
	return ufio::posix_api_common(path, [&](char const *path_c_str) { return posix1x_api_dispatcher<dsp>(dirfd, path_c_str, args...); });
}
...
template <::ufio::constructible_to_os_c_str new_path_type>
inline void posix_renameat(posix_fs_dirent fs_dirent, posix_at_entry newent, new_path_type const &newpath, [[maybe_unused]] posix_at_flags flags = {})
{
	details::posix_deal_with22<details::posix_api_22::renameat>(
		fs_dirent.fd, ::ufio::manipulators::os_c_str(fs_dirent.filename), newent.fd, newpath);
}
...
template<::ufio::constructible_to_os_c_str path_type,::std::integral char_type>
