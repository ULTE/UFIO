...
{
	return my_dos_opendir(::ufio::noexcept_call(::__get_fd_name, fd));
}
} // namespace details
...
inline constexpr ::ufio::manipulators::basic_os_c_str_with_known_size<char> native_filename(dos_directory_entry pioe) noexcept
{
	return {pioe.entry->d_name, pioe.d_namlen};
}

inline ::ufio::manipulators::basic_os_c_str_with_known_size<char8_t> u8filename(dos_directory_entry pioe) noexcept
{
...
using posix_recursive_directory_generator =
	basic_posix_recursive_directory_generator<::ufio::containers::vector<dos_directory_file, ::ufio::posix_api_encoding_converter::allocator_type>>;
	...
	for (;;)
	{
		...
		)
		{
			...
			prdit.stack.emplace_back(
				::ufio::posix_file(::ufio::posix_fs_dirent{details::dirp_to_fd(prdit.stack.empty() ? prdit.dirp : prdit.stack.back().dirp), name},
									  ::ufio::open_mode::directory));
		}
...
{
	return ::ufio::details::find_dot_and_sep<false, char, char>(ent.entry->d_name, ent.d_namlen);
}

inline auto native_stem(dos_directory_entry ent) noexcept
{
	return ::ufio::details::find_dot_and_sep<true, char, char>(ent.entry->d_name, ent.d_namlen);
}

inline auto u8extension(dos_directory_entry ent) noexcept
{
	return ::ufio::details::find_dot_and_sep<false, char8_t, char>(ent.entry->d_name, ent.d_namlen);
}

inline auto u8stem(dos_directory_entry ent) noexcept
{
	return ::ufio::details::find_dot_and_sep<true, char8_t, char>(ent.entry->d_name, ent.d_namlen);
}
