...
inline ::ufio::tlc::string my_dos_concat_path(int dirfd, char const *pathname)
{
	if (dirfd == -100)
	{
		return ::ufio::tlc::string{::ufio::mnp::os_c_str(pathname)};
	}
	else
	{
		...
		// concat
		return ::ufio::tlc::concat_ufio_tlc(::ufio::mnp::os_c_str(pathname_cstr), "\\", para_pathname);
	}
}
...
{
	return ufio::posix_api_common(path, [&](char const *path_c_str) { return dos1x_api_dispatcher<dsp>(dirfd, path_c_str, args...); });
}
