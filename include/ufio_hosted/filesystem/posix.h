...
inline constexpr ::ufio::manipulators::basic_os_c_str_with_known_size<char>
native_filename(posix_directory_entry pioe) noexcept
{
	return {pioe.entry->d_name, pioe.d_namlen};
}

inline ::ufio::manipulators::basic_os_c_str_with_known_size<char8_t> u8filename(posix_directory_entry pioe) noexcept
{
...
using posix_recursive_directory_generator = basic_posix_recursive_directory_generator<
	::ufio::containers::vector<posix_directory_file, ::ufio::posix_api_encoding_converter::allocator_type>>;
...
operator++(basic_posix_recursive_directory_iterator<StackType> &prdit)
{
	for (;;)
	{
		...
		)
		{
			...
			prdit.stack.emplace_back(::ufio::posix_file(
				::ufio::posix_fs_dirent{
