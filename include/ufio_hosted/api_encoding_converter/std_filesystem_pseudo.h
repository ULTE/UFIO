													...
													::std::size_t filename_strvw_len, Func callback)
{
	::std::size_t const filename_strvw_len_p1{
		::ufio::details::intrinsics::add_or_overflow_die(filename_strvw_len, 1)};
	::ufio::details::buffer_alloc_arr_ptr<os_char_type, false, allocator_type> temp(filename_strvw_len_p1);
	auto ptr{temp.ptr};
	non_overlapped_copy_n(filename_strvw_str, filename_strvw_len, ptr);
	ptr[filename_strvw_len] = 0;
	if constexpr (::ufio::details::api_common_has_size_overload<os_char_type, Func>)
	{
