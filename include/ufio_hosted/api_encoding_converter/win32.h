										...
										Func callback)
{
	...
	if constexpr (::ufio::details::api_common_has_size_overload<char16_t, Func>)
	{
...
inline auto nt_api_common(T const &t, Func callback)
{
...
#endif
		= char16_t const *;
	constexpr bool has_size_overload{::ufio::details::api_common_has_size_overload<char16_t, Func>};
	if constexpr (::std::is_array_v<T>)
	{
		...
		if constexpr (sizeof(cstr_char_type) == sizeof(char16_t))
		{
			...
			{
				return callback(reinterpret_cast<char16_const_may_alias_ptr>(p), ::ufio::details::cal_array_size(t));
			}
			else
			{
		...
		{
			return ::ufio::details::nt_api_common_code_cvt_impl<allocator_type>(
				p, ::ufio::details::cal_array_size(t), callback);
		}
	}
	else if constexpr (type_has_c_str_method<T>)
	{
		...
		if constexpr (sizeof(cstr_char_type) == sizeof(char16_t))
		{
			if constexpr (has_size_overload)
			{
				...
				else if constexpr (::ufio::details::cxx_std_filesystem_pseudo_concept<::std::remove_cvref_t<T>>)
				{
				...
				{
					auto ptr{t.c_str()};
					return callback(reinterpret_cast<char16_const_may_alias_ptr>(ptr), ::ufio::cstr_len(ptr));
				}
			}
			else
			{
		...
		{
			return ::ufio::details::nt_api_common_code_cvt_impl<allocator_type>(::std::ranges::data(t),
																				   ::std::ranges::size(t), callback);
		}
		else if constexpr (::ufio::details::cxx_std_filesystem_pseudo_concept<::std::remove_cvref_t<T>>)
		{
			auto const &native{t.native()};
			return ::ufio::details::nt_api_common_code_cvt_impl<allocator_type>(native.c_str(), native.size(),
																				   callback);
		}
		else
		{
			auto ptr{t.c_str()};
			return ::ufio::details::nt_api_common_code_cvt_impl<allocator_type>(ptr, ::ufio::cstr_len(ptr),
																				   callback);
		}
	}
	else // types like ::std::basic_string_view, we must allocate new space to hold that type
	{
		...
		{
			return ::ufio::details::api_common_copy_append0_strvw_case_impl<allocator_type, char16_t>(
				reinterpret_cast<char16_const_may_alias_ptr>(t.data()), t.length(), callback);
		}
		else
		{
			return ::ufio::details::nt_api_common_code_cvt_impl<allocator_type>(t.data(), t.length(), callback);
		}
	}
}
...
{
	if constexpr (::ufio::win32_family::native == win32_family::ansi_9x)
	{
		return posix_api_common<allocator_type>(t, callback);
	}
	else
