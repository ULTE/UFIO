	...
	constexpr
		...
		: buffer_capacity(details::cal_decorated_reserve_size<sizeof(from_char_type), sizeof(char_type)>(sz)),
		  buffer_data(details::allocate_iobuf_space<char_type, allocator_type>(
			  ::ufio::details::intrinsics::add_or_overflow_die(buffer_capacity, 1))),
...
using posix_api_encoding_converter =
	basic_posix_api_encoding_converter<generic_allocator_adapter<::ufio::c_malloc_allocator>>;
										  ...
										  Func callback)
{
	...
	if constexpr (::ufio::details::api_common_has_size_overload<char, Func>)
	{
...
{
	constexpr bool has_size_overload{::ufio::details::api_common_has_size_overload<char, Func>};
	if constexpr (::std::is_array_v<T>)
	{
		...
		if constexpr (sizeof(cstr_char_type) == sizeof(char))
		{
			...
			{
				return callback(reinterpret_cast<char const *>(p), ::ufio::details::cal_array_size(t));
			}
			else
			{
		...
		{
			return ::ufio::details::posix_api_common_codecvt_impl<allocator_type>(
	...
	else if constexpr (type_has_c_str_method<T>)
	{
		...
		if constexpr (sizeof(cstr_char_type) == sizeof(char))
		{
			if constexpr (has_size_overload)
			{
				...
				{
					auto ptr{t.c_str()};
					return callback(reinterpret_cast<char const *>(ptr), ::ufio::cstr_len(ptr));
				}
			}
			else
			{
		...
		else
		{
			...
			{
				auto const &native{t.native()};
				return ::ufio::details::posix_api_common_codecvt_impl<allocator_type>(native.c_str(), native.size(),
																						 callback);
			}
			else
			{
				auto ptr{t.c_str()};
				return ::ufio::details::posix_api_common_codecvt_impl<allocator_type>(ptr, ::ufio::cstr_len(ptr),
																						 callback);
			}
		}
	}
	else // types like ::std::basic_string_view, we must allocate new space to hold that type
	{
		...
		{
			return ::ufio::details::api_common_copy_append0_strvw_case_impl<allocator_type, char>(
				reinterpret_cast<char const *>(t.data()), t.length(), callback);
		}
		else
		{
			return ::ufio::details::posix_api_common_codecvt_impl<allocator_type>(t.data(), t.length(), callback);
		}
	}
}
} // namespace ufio
