...
{
	::ufio::freestanding::array<char8_t, calculate_nt_dbg_fmt_string_size<iswide, n>()> fmt_arr;
							   ...
							   Args const *...args) noexcept
{
	if constexpr (n == 0)
	{
		...
		::ufio::win32::nt::DbgPrintEx(carr.component_id, carr.level, reinterpret_cast<char const *>(fmt), args...);
#else
		::ufio::win32::nt::DbgPrint(reinterpret_cast<char const *>(fmt), args...);
#endif
	}
	else
	{
		...
		::ufio::win32::nt::ansi_string astr{u16argnlen, u16argnlen,
...
inline void nt_dbg_write_impl([[maybe_unused]] nt_dbg_carrier carr, char const *first, char const *last) noexcept
{
	...
	::std::uint_least16_t const u16argnlen{static_cast<::std::uint_least16_t>(arg_n_len)};
	::ufio::win32::nt::ansi_string astr{u16argnlen, u16argnlen, const_cast<char *>(first)};
#if _WIN32_WINNT >= 0x0501
	::ufio::win32::nt::DbgPrintEx(carr.component_id, carr.level,
									 reinterpret_cast<char const *>(nt_fmt_cache<iswide, 1>.element),
									 __builtin_addressof(astr));
#else
	::ufio::win32::nt::DbgPrint(reinterpret_cast<char const *>(nt_fmt_cache<iswide, 1>.element),
								   __builtin_addressof(astr));
#endif
}
...
inline constexpr void write_all_bytes_overflow_define(basic_nt_dbg<ch_type> d, ::std::byte const* first, ::std::byte const* last) noexcept
{
    ...
    ::ufio::details::nt_dbg_write_impl<is_wide>(details::nt_dbg_carrier{d.component_id, d.level},
...
inline void write(basic_nt_dbg<ch_type> d, Iter first, Iter last) noexcept
{
	...
	::ufio::details::nt_dbg_write_impl<is_wide>({d.component_id, d.level},
...
{
	constexpr bool is_wide{sizeof(ch_type) == 2};
	::ufio::details::nt_dbg_scatter_constant_write_impl<is_wide, n>({d.component_id, d.level}, pscatters);
}
#endif
