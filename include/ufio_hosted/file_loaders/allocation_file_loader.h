...
{
	::std::size_t filesize{::ufio::details::posix_loader_get_file_size(fd)};
	...
	::ufio::operations::decay::read_all_bytes_decay(piob, reinterpret_cast<::std::byte *>(addr),
...
{
#if defined(_WIN32) && !defined(__CYGWIN__) && !defined(__WINE__) && !defined(__BIONIC__)
	auto seekret = ::ufio::noexcept_call(_lseeki64, fd, 0, 0);
#else
#if defined(_LARGEFILE64_SOURCE)
	auto seekret = ::ufio::noexcept_call(::lseek64, fd, 0, 0);
#else
	auto seekret = ::ufio::noexcept_call(::lseek, fd, 0, 0);
#endif
#endif
	if (seekret == -1)
	{
...
{
	::ufio::posix_file pf(::ufio::freestanding::forward<Args>(args)...);
...
{
	::ufio::posix_file pf(::ufio::io_dup, ::ufio::posix_io_observer{fd});
	rewind_allocation_file_loader(pf.fd);
	auto ret{allocation_load_address_impl(pf.fd)};
	if (writeback)
	{
...
public:
	...
	{
		auto ret{::ufio::details::allocation_load_file_fd_impl(false, pate.fd)};
	...
	{
		auto ret{::ufio::details::allocation_load_file_impl(false, fsdirent, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
	}
	...
	inline explicit allocation_file_loader(T const &filename, open_mode om = open_mode::in,
										   perms pm = static_cast<perms>(436))
	{
		auto ret{::ufio::details::allocation_load_file_impl(false, filename, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
	}
	...
	inline explicit allocation_file_loader(native_at_entry ent, T const &filename, open_mode om = open_mode::in,
										   perms pm = static_cast<perms>(436))
	{
		auto ret{::ufio::details::allocation_load_file_impl(false, ent, filename, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
	}
	...
	{
		auto ret{::ufio::details::allocation_load_file_fd_impl(options.write_back, pate.fd)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
		fd = ret.fd;
	...
	{
		auto ret{::ufio::details::allocation_load_file_impl(options.write_back, fsdirent, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
		fd = ret.fd;
	...
	inline explicit allocation_file_loader(allocation_mmap_options options, T const &filename,
										   open_mode om = open_mode::in, perms pm = static_cast<perms>(436))
	{
		auto ret{::ufio::details::allocation_load_file_impl(options.write_back, filename, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
		fd = ret.fd;
	...
	inline explicit allocation_file_loader(allocation_mmap_options options, native_at_entry ent, T const &filename,
										   open_mode om = open_mode::in, perms pm = static_cast<perms>(436))
	{
		auto ret{::ufio::details::allocation_load_file_impl(options.write_back, ent, filename, om, pm)};
		address_begin = ret.address_begin;
		address_end = ret.address_end;
		address_capacity = ret.address_capacity;
		fd = ret.fd;
	...
	inline void close()
	{
		::ufio::details::close_allocation_file_loader_impl(fd, address_begin, address_end);
