...
{
	system_call_throw_error(::ufio::posix::libc_faccessat(dirfd, cstr, X_OK, AT_SYMLINK_NOFOLLOW));
...
{
	return ::ufio::posix_api_common(csv, [&](char const *cstr) { return posix_fork_execveat_common_impl(dirfd, cstr, args, envp, pio); });
}
...
inline void execveat_inside_vfork(int dirfd, char const *cstr, char const *const *args, char const *const *envp, int volatile &t_errno) noexcept
{
	...
	if (::ufio::linux_system_call_fails(ret))
	{
		t_errno = -ret;
	}
	else
...
{
	return ::ufio::posix_api_common(csv, [&](char const *cstr) { return vfork_execveat_common_impl(dirfd, cstr, args, envp, pio); });
}
...
public:
	...
	posix_process(path_type const &filename, posix_process_args const &args, posix_process_envs const &envp,
				  posix_process_io const &pio)
		: posix_process_observer{::ufio::details::vfork_execve_impl(filename, args.get(), envp.get(), pio)}
	{
	}

	posix_process(::ufio::posix_fs_dirent ent, posix_process_args const &args, posix_process_envs const &envp,
				  posix_process_io const &pio)
		: posix_process_observer{
			  ::ufio::details::vfork_execveat_common_impl(ent.fd, ent.filename, args.get(), envp.get(), pio)}
	{
	}
	...
	posix_process &operator=(posix_process &&__restrict other) noexcept
	{
		...
		::ufio::details::posix_waitpid_noexcept(this->pid);
		this->pid = other.pid;
		other.pid = -1;
		return *this;
	}
