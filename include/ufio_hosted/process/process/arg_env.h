...
inline constexpr void construct_win32_process_args_decay_singal(
	::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator> &str, T t)
{
	constexpr bool type_error{::ufio::operations::defines::print_freestanding_okay<::ufio::details::dummy_buffer_output_stream<replace_char_type>, T>};

	if constexpr (type_error)
	{
		replace_char_type buf[32767];
		::ufio::basic_obuffer_view<replace_char_type> obf{buf, buf + 32767};
		::ufio::operations::decay::print_freestanding_decay<false>(::ufio::operations::output_stream_ref(obf), t);
		str.reserve(str.size() + 3 + obf.size() * 2);
		str.push_back_unchecked(::ufio::char_literal_v<u8'\"', replace_char_type>);
		for (auto const c : obf)
		{
			if (c == ::ufio::char_literal_v<u8'\"', replace_char_type>)
			{
				str.push_back_unchecked(::ufio::char_literal_v<u8'\\', replace_char_type>);
				str.push_back_unchecked(::ufio::char_literal_v<u8'\"', replace_char_type>);
			}
	...
	else if constexpr (requires { ::ufio::mnp::code_cvt(t); })
	{
		replace_char_type buf[32767];
		::ufio::basic_obuffer_view<replace_char_type> obf{buf, buf + 32767};
		// need decay
		::ufio::operations::print_freestanding<false>(obf, ::ufio::mnp::code_cvt(t));
		str.reserve(str.size() + 3 + obf.size() * 2);
		str.push_back_unchecked(::ufio::char_literal_v<u8'\"', replace_char_type>);
		for (auto const c : obf)
		{
...
inline constexpr void construct_win32_process_args_decay(
	::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator> &str, T t, Args... args)
{
	...
	::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator> &str, T t)
{
	...
	if constexpr (type_error)
	{
		::ufio::io_strlike_reference_wrapper<replace_char_type, ::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator>> wrapper{__builtin_addressof(str)};
		::ufio::operations::print_freestanding<false>(wrapper, t, ::ufio::mnp::chvw(::ufio::char_literal_v<u8'\0', replace_char_type>));
	}
	else if constexpr (requires { ::ufio::mnp::code_cvt(t); })
	{
		::ufio::io_strlike_reference_wrapper<replace_char_type, ::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator>> wrapper{__builtin_addressof(str)};
		::ufio::operations::print_freestanding<false>(wrapper, ::ufio::mnp::code_cvt(t), ::ufio::mnp::chvw(::ufio::char_literal_v<u8'\0', replace_char_type>));
	}
	else
	{
...
{
	inline static constexpr bool is_nt{family == ::ufio::win32_family::wide_nt};
	using char_type = ::std::conditional_t<is_nt, char16_t, char>;
	using replace_char_type = ::std::conditional_t<is_nt, char16_t, char8_t>;
	using storage_type = ::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator>;
	...
	{
		args = storage_type{::ufio::mnp::os_c_str(oscstr)};
	}
	...
	inline constexpr win32_process_args(T &&t, Args &&...as)
	{
		details::construct_win32_process_args_decay(args,
													::ufio::io_print_forward<replace_char_type>(::ufio::io_print_alias(t)),
													::ufio::io_print_forward<replace_char_type>(::ufio::io_print_alias(as))...);
	}
...
using nt_process_args = ::ufio::win32_process_args<::ufio::win32_family::wide_nt>;
using nt_process_envs = ::ufio::win32_process_envs<::ufio::win32_family::wide_nt>;
	...
	::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator> &str, T t)
{
	...
	::ufio::freestanding::my_memcpy(reinterpret_cast<ptrdiff_t_may_alias_ptr>(str.begin()) + N, __builtin_addressof(sz), sizeof(sz));
...
struct posix_process_args
{
	...
	inline constexpr posix_process_args(default_args_t, char const *const *args) noexcept
	{
		...
		::ufio::freestanding::my_memcpy(arg_envs.begin(), __builtin_addressof(args), sizeof(args));
	}
	...
	inline constexpr posix_process_args(T &&t, Args &&...as)
	{
		...
		arg_envs = ::ufio::containers::basic_string<replace_char_type, ::ufio::native_global_allocator>{sizeofptr * sizeof(::std::ptrdiff_t) / sizeof(replace_char_type)}; // calloc
		...
		{
			::std::ptrdiff_t sz{};
			::ufio::freestanding::my_memcpy(__builtin_addressof(sz), curr, sizeof(sz));
			auto const ptr{arg_envs.cbegin() + sz};
			::ufio::freestanding::my_memcpy(curr, __builtin_addressof(ptr), sizeof(ptr));
		}
	...
	inline constexpr char const *const *get() const noexcept
	{
		...
		else if (szb == sizeof(::std::ptrdiff_t))
		{
			...
			::ufio::freestanding::my_memcpy(__builtin_addressof(ptr), arg_envs.cbegin(), sizeof(ptr));
			return ptr;
		}
		else
		{
