...
inline void close_win32_user_process_information_not_null(win32_user_process_information hnt_user_process_info) noexcept
{
	...
	{
		::ufio::win32::CloseHandle(hnt_user_process_info.hthread);
		hnt_user_process_info.hthread = nullptr;
	}
	if (hnt_user_process_info.hprocess) [[likely]]
	{
		::ufio::win32::CloseHandle(hnt_user_process_info.hprocess);
...
{
	return ::ufio::win32::WaitForSingleObject(hprocess_thread, 0xFFFFFFFF);
}
																...
																win32_process_io const &__restrict processio)
{
	...
	if constexpr (is_nt)
	{
		...
		void *hFileMap{::ufio::win32::CreateFileMappingW(fhandle, nullptr, 0x02, 0, 1, nullptr)};
		...
		void *pMem{::ufio::win32::MapViewOfFile(hFileMap, 0x04, 0, 0, 1)};
		if (!pMem) [[unlikely]]
		{
			throw_win32_error();
		}
		...
		if (!
			...
			::ufio::win32::K32GetMappedFileNameW(
#else
			::ufio::win32::GetMappedFileNameW(
#endif
		...
		if (pszFilename[0] == u'\\')
		{
			...
			{
				DosDevice[0] = i;
				if (::ufio::win32::QueryDosDeviceW(DosDevice, NtPath, 64))
				{
					NtPathLen = ::ufio::cstr_len(NtPath);

					if (::ufio::freestanding::my_memcmp(pszFilename, NtPath, NtPathLen * sizeof(char16_t)) == 0) [[unlikely]]
					{
						goto next;
					}
				}
		...
		// create process
		::ufio::win32::startupinfow si{sizeof(si)};
		...
		if (!processio.in.is_dev_null)
		{
			if (processio.in.win32_pipe_in_handle)
			{
				...
				if (!::ufio::win32::SetHandleInformation(processio.in.win32_pipe_out_handle, 0x00000001 /*HANDLE_FLAG_INHERIT*/, 0)) [[unlikely]]
				{
					throw_win32_error();
				}
			}
			...
			{
				si.hStdInput = ::ufio::win32_stdin().handle;
			}
		}
		else
		{
		...
		if (!processio.out.is_dev_null)
		{
			if (processio.out.win32_pipe_out_handle)
			{
				...
				if (!::ufio::win32::SetHandleInformation(processio.out.win32_pipe_in_handle, 0x00000001 /*HANDLE_FLAG_INHERIT*/, 0)) [[unlikely]]
				{
					throw_win32_error();
				}
			}
			...
			{
				si.hStdOutput = ::ufio::win32_stdout().handle;
			}
		}
		else
		{
		...
		if (!processio.err.is_dev_null)
		{
			if (processio.err.win32_pipe_out_handle)
			{
				...
				if (!::ufio::win32::SetHandleInformation(processio.err.win32_pipe_in_handle, 0x00000001 /*HANDLE_FLAG_INHERIT*/, 0)) [[unlikely]]
				{
					throw_win32_error();
				}
			}
			...
			{
				si.hStdError = ::ufio::win32_stderr().handle;
			}
		}
		else
		{
		...
		::ufio::win32::process_information pi{};
		if (!::ufio::win32::CreateProcessW(address_begin, const_cast<char16_t *>(args), nullptr, nullptr, 1,
	...
	else
	{
		...
		void *hFileMap{::ufio::win32::CreateFileMappingA(fhandle, nullptr, 0x02, 0, 1, nullptr)};
		if (!hFileMap) [[unlikely]]
		{
			throw_win32_error();
		}
		...
		if (!
#if !defined(_WIN32_WINNT) || _WIN32_WINNT >= 0x601
			::ufio::win32::K32GetMappedFileNameA(
#else
			::ufio::win32::GetMappedFileNameA(
#endif
				reinterpret_cast<void *>(static_cast<::std::ptrdiff_t>(-1)),
				pMem,
				pszFilename,
		...
		// change nt path to dos path
		auto k32_module{::ufio::win32::GetModuleHandleA(reinterpret_cast<char const *>(u8"Kernel32.dll"))};
		if (k32_module)
		{
			...
			auto QueryDosDeviceA_p{reinterpret_cast<QueryDosDeviceA_t>(::ufio::win32::GetProcAddress(k32_module, reinterpret_cast<char const *>(u8"QueryDosDeviceA")))};
				...
				if (pszFilename[0] == u8'\\')
				{
					...
					for (char i{65}; i < static_cast<char>(26 + 65); i++)
					{
						...
						if (QueryDosDeviceA_p(DosDevice, NtPath, 64))
						{
							...
							if (::ufio::freestanding::my_memcmp(pszFilename, NtPath, NtPathLen * sizeof(char)) == 0) [[unlikely]]
							{
								goto next2;
							}
						}
		...
		// create process
		::ufio::win32::startupinfoa si{sizeof(si)};
		...
		::ufio::win32::process_information pi{};
		if (!::ufio::win32::CreateProcessA(address_begin, const_cast<char *>(args), nullptr, nullptr, 1, 0, (void *)envs, nullptr, __builtin_addressof(si), __builtin_addressof(pi)))
		{
			throw_win32_error();
		}
...
template <win32_family family, typename path_type>
inline win32_user_process_information win32_create_process_overloads(::ufio::nt_fs_dirent ent, win32_process_args<family> const &args,
																	 win32_process_envs<family> const &envs,
																	 win32_process_io const &processio)
{
...
inline win32_wait_status wait(win32_family_process_observer<family> ppob) noexcept(!throw_eh)
{
	...
	if (!::ufio::win32::GetExitCodeProcess(ppob.hnt_user_process_info.hprocess, __builtin_addressof(exit_code)))
	{
		throw_win32_error(status);
	}
...
public:
	...
	explicit win32_family_process(::ufio::nt_fs_dirent ent, win32_process_args<family> const &args, win32_process_envs<family> const &envs,
								  win32_process_io const &processio)
		: win32_family_process_observer<family>{
