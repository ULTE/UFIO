...
inline constexpr ::ufio::ip_address posix_to_ip_address_with_ai_addr_impl(int ai_family,
...
inline constexpr ::ufio::ip posix_to_ip_with_ai_addr_impl(int ai_family, posix_sockaddr const *ai_addr,
															 ::std::uint_least16_t port) noexcept
{
	return ::ufio::ip{posix_to_ip_address_with_ai_addr_impl(ai_family, ai_addr), port};
}
...
struct posix_dns_open_parameter
{
	...
	{
		return ::ufio::details::posix_dns_open_internal_impl(node_name_c_str);
	}
};
...
{
	return ::ufio::posix_api_common(t, posix_dns_open_parameter{});
}

} // namespace details

inline constexpr ::ufio::ip to_ip(posix_dns_io_observer d, ::std::uint_least16_t port) noexcept
{
	return ufio::details::posix_to_ip_with_ai_addr_impl(d.res->ai_family, d.res->ai_addr, port);
}

inline constexpr ::ufio::ip_address to_ip_address(posix_dns_io_observer d) noexcept
{
	return ufio::details::posix_to_ip_address_with_ai_addr_impl(d.res->ai_family, d.res->ai_addr);
}
...
public:
	...
	explicit posix_dns_file(T const &t)
		: posix_dns_io_observer{::ufio::details::posix_dns_open_impl(t)}
	{
	}
