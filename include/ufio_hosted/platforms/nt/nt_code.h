...
[[noreturn]] inline void throw_nt_error([[maybe_unused]] ::std::uint_least32_t err)
{
	...
	fast_terminate();
#else
	throw ::ufio::error{nt_domain_value, static_cast<::std::size_t>(err)};
#endif
#else
	fast_terminate();
#endif
...
namespace details
{
inline constexpr ::ufio::manipulators::scalar_flags nt_errorflags{
	.base = 16, .full = true, .floating = ::ufio::manipulators::floating_format::fixed};
...
inline constexpr char_type *print_reserve_nt_error_impl(char_type *iter, ::std::uint_least32_t ntstatus) noexcept
{
	...
	using namespace ::ufio::manipulators;
	iter = print_reserve_define(
		io_reserve_type<char_type, scalar_manip_t<::ufio::details::nt_errorflags, ::std::uint_least32_t>>, iter,
		{ntstatus});
...
inline constexpr ::std::size_t print_reserve_size(io_reserve_type_t<char_type, nt_code>) noexcept
{
	...
	constexpr ::std::size_t full_size(
		print_reserve_size(
			io_reserve_type<char_type, scalar_manip_t<::ufio::details::nt_errorflags, ::std::uint_least32_t>>) +
		print_reserve_size(io_reserve_type<char_type, nt_code>));
	if constexpr (::std::same_as<char_type, char>)
	{
		return full_size + ::ufio::details::string_literal_size("[nt:0x]");
	}
	else if constexpr (::std::same_as<char_type, wchar_t>)
	{
		return full_size + ::ufio::details::string_literal_size(L"[nt:0x]");
	}
	else
	{
		return full_size + ::ufio::details::string_literal_size(u8"[nt:0x]");
	}
}
...
inline constexpr char_type *print_reserve_define(io_reserve_type_t<char_type, nt_code>, char_type *iter,
												 nt_code e) noexcept
{
	return ::ufio::details::print_reserve_nt_error_impl(iter, e.ntstatus);
}
