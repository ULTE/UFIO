...
inline ::std::uint_least64_t zero_copy_transmit_define64_impl(int out_fd, int in_fd, ::std::uint_least64_t count)
{
	...
	for (;;)
	{
		...
		{
			return total_bytes +
				   ::ufio::raw_transmit64_decay(posix_io_observer{out_fd}, posix_io_observer{in_fd}, count);
		}
		else if (diff == 0) [[unlikely]]
		{
			break;
														 ...
														 ::std::uint_least64_t characters)
{
	...
	if constexpr (input_char_type_size != 1)
	{
		...
		for (::std::size_t i{}; i != input_char_type_size; ++i)
		{
			...
			chars += ::ufio::details::zero_copy_transmit_define64_impl(io_alias, outs.fd, ins.fd, characters,
																		  copy_this_round) /
	...
	{
		return ::ufio::details::zero_copy_transmit_define64_impl(outs.fd, ins.fd, characters);
	}
}
