...
inline bool nt_code_equivalent_impl(::std::size_t domain, ::std::size_t code, ::std::uint_least32_t e) noexcept
{
	...
	{
		auto lasterr{::ufio::win32::nt::rtl_nt_status_to_dos_error(e)};
...
inline bool win32_code_equivalent_impl(::std::size_t domain, ::std::size_t code, ::std::uint_least32_t e) noexcept
{
	...
	else if (nt_domain_value == domain)
	{
		...
		return false;
#else
		return ::ufio::win32::nt::rtl_nt_status_to_dos_error(static_cast<::std::uint_least32_t>(code)) == e;
#endif
	}
...
inline bool posix_code_equivalent_impl(::std::size_t domain, ::std::size_t code, int e) noexcept
{
	...
	else if (win32_domain_value == domain || nt_domain_value == domain)
	{
		...
		if (nt_domain_value == domain)
		{
			...
			win32cd = 0x00000057;
#else
			win32cd = ::ufio::win32::nt::rtl_nt_status_to_dos_error(static_cast<::std::uint_least32_t>(code));
#endif
		}
		else
		{
...
inline constexpr ::std::uint_least32_t to_win32_code_impl(::std::size_t domain, ::std::size_t code) noexcept
{
	...
	else if (nt_domain_value == domain)
	{
		...
		return 0x00000057;
#else
		return ::ufio::win32::nt::rtl_nt_status_to_dos_error(static_cast<::std::uint_least32_t>(code));
#endif
	}
	else if (posix_domain_value == domain)
	{
...
{
	return ::ufio::details::win32_code_equivalent_impl(e.domain, e.code, we.ec);
}

inline bool equivalent_define(error_type_t<nt_code>, error e, nt_code we) noexcept
{
	return ::ufio::details::nt_code_equivalent_impl(e.domain, e.code, we.ntstatus);
}

inline bool equivalent_define(error_type_t<::ufio::freestanding::errc>, error e,
							  ::ufio::freestanding::errc we) noexcept
{
	return ::ufio::details::posix_code_equivalent_impl(e.domain, e.code, static_cast<int>(we));
}

inline ::ufio::freestanding::errc to_code_define(error_type_t<::ufio::freestanding::errc>, error e) noexcept
{
...
{
	return win32_code{::ufio::details::to_win32_code_impl(e.domain, e.code)};
}

using native_code = win32_code;

inline native_code print_alias_define(::ufio::io_alias_t, ::ufio::error e) noexcept
{
