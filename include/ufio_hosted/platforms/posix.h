...
template <::ufio::posix_family family, ::std::integral ch_type>
...
{
	return {static_cast<::ufio::uintfpos_t>(st.st_dev),
			static_cast<::ufio::uintfpos_t>(st.st_ino),
			st_mode_to_perms(st.st_mode),
			st_mode_to_file_type(st.st_mode),
			static_cast<::ufio::uintfpos_t>(st.st_nlink),
			static_cast<::ufio::uintfpos_t>(st.st_uid),
			static_cast<::ufio::uintfpos_t>(st.st_gid),
			static_cast<::ufio::uintfpos_t>(st.st_rdev),
			static_cast<::ufio::uintfpos_t>(st.st_size),
#if ((defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) && !defined(__CYGWIN__)) || defined(__MSDOS__)
			131072,
			static_cast<::ufio::uintfpos_t>(st.st_size / 512),
			{st.st_atime, {}},
			{st.st_mtime, {}},
			{st.st_ctime, {}},
			{0, 0},
	...
	!defined(__linux__)
			static_cast<::ufio::uintfpos_t>(st.st_blksize),
			static_cast<::ufio::uintfpos_t>(st.st_blocks),
...
{
	int fd{::ufio::noexcept_call(_open_osfhandle, reinterpret_cast<::std::ptrdiff_t>(handle),
...
inline int my_posix_openat(int dirfd, char const *pathname, int flags, mode_t mode)
{
	...
	{
		auto pathname_cstr{::ufio::noexcept_call(::__get_fd_name, dirfd)};
		...
		// check vaildity
		::ufio::cstring_view para_pathname{::ufio::mnp::os_c_str(pathname)};
		...
		// concat
		::ufio::tlc::string pn{::ufio::tlc::concat_ufio_tlc(::ufio::mnp::os_c_str(pathname_cstr), "\\", para_pathname)};
...
{
	return ::ufio::posix_api_common(
		t,
		my_posix_at_open_paramter{dirfd, ::ufio::details::calculate_posix_open_mode(om), static_cast<mode_t>(pm)});
}
#endif
...
#if defined(__MSDOS__) || (defined(__NEWLIB__) && !defined(AT_FDCWD)) || defined(_PICOLIBC__)
	return ::ufio::posix_api_common(
		t, my_posix_open_paramter{::ufio::details::calculate_posix_open_mode(om), static_cast<mode_t>(pm)});
#else
	return ::ufio::posix_api_common(t, my_posix_at_open_paramter{AT_FDCWD,
																	::ufio::details::calculate_posix_open_mode(om),
...
{
#if (defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) && !defined(__CYGWIN__)
	::ufio::basic_win32_file<char> wf(::ufio::io_temp);
	int fd{::ufio::noexcept_call(_open_osfhandle, reinterpret_cast<::std::ptrdiff_t>(wf.handle), _O_BINARY)};
	if (fd == -1)
	{
		throw_posix_error();
	}
...
public:
	...
	basic_posix_family_file(sock_family d, sock_type t, open_mode m, sock_protocol p)
		: basic_posix_family_io_observer<family, char_type>{::ufio::details::open_socket_impl(d, t, m, p)}
	{
	}
#endif
	...
	explicit basic_posix_family_file(T const &filename, open_mode om, perms pm = static_cast<perms>(436))
		: basic_posix_family_io_observer<family, char_type>{::ufio::details::posix_open_file_impl(filename, om, pm)}
	{
	}

	template <::ufio::constructible_to_os_c_str T>
	explicit basic_posix_family_file(posix_at_entry pate, T const &filename, open_mode om, perms pm = static_cast<perms>(436))
		: basic_posix_family_io_observer<family, char_type>{::ufio::details::posix_openat_file_impl(pate.fd, filename, om, pm)}
	{
	}

#endif
	basic_posix_family_file(io_temp_t)
		: basic_posix_family_io_observer<family, char_type>{::ufio::details::my_open_posix_fd_temp_file()}
	{
	}
...
inline void posix_truncate_impl(int fd, ::ufio::uintfpos_t size)
{
#if (defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) && !defined(__CYGWIN__)
#if (!defined(__MINGW32__) || __has_include(<_mingw_stat64.h>))
...
#endif
inline void truncate(basic_posix_family_io_observer<family, ch_type> h, ::ufio::uintfpos_t size)
{
...
using basic_posix_io_observer = basic_posix_family_io_observer<::ufio::posix_family::api, chartype>;
template <::std::integral chartype>
using basic_posix_file = basic_posix_family_file<::ufio::posix_family::api, chartype>;
template <::std::integral chartype>
using basic_posix_pipe = basic_posix_family_pipe<::ufio::posix_family::api, chartype>;
...
template <::std::integral chartype>
using basic_dos_io_observer = basic_posix_family_io_observer<::ufio::posix_family::dos, chartype>;
template <::std::integral chartype>
using basic_dos_file = basic_posix_family_file<::ufio::posix_family::dos, chartype>;
template <::std::integral chartype>
using basic_dos_pipe = basic_posix_family_pipe<::ufio::posix_family::dos, chartype>;
