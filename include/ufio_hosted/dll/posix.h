...
{
	errno = {};
	auto hd{noexcept_call(dlopen, filename, ::ufio::dll_mode_to_posix_rtld_mode(mode))};
	if (hd == nullptr)
	{
		::ufio::details::throw_posix_rtld_error(
#if defined(ENOENT)
			ENOENT
#endif
		);
...
{
	return ::ufio::posix_api_common(t, posix_rtld_impl_context{mode});
}
...
public:
	...
	explicit posix_dll_file(T const &t, dll_mode mode)
		: posix_dll_io_observer{::ufio::details::create_posix_rtld_impl(t, mode)}
	{
	}
...
{
	return ::ufio::posix_api_common(symbol, ::ufio::details::posix_dll_load_impl_context{pdliob.rtld_handle});
}

template <::ufio::constructible_to_os_c_str T, ::ufio::constructible_to_os_c_str T1>
inline void *dll_load_versioned_symbol(posix_dll_io_observer pdliob, T const &symbol, [[maybe_unused]] T1 const &vers)
{
	...
	return ::ufio::posix_api_common(
		...
		[&](char const *ver) {
			return ::ufio::posix_api_common(
				symbol, ::ufio::details::posix_dll_load_versioned_symbol_impl_context{pdliob.rtld_handle, ver});
		});
#else
	return ::ufio::posix_api_common(symbol, ::ufio::details::posix_dll_load_impl_context{pdliob.rtld_handle});
#endif
