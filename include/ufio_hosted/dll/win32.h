...
{
	auto hmodule{::ufio::win32::LoadLibraryA(filename)};
	if (hmodule == nullptr)
	{
		throw_win32_error();
	}
...
inline void *create_win32_dll_ntw(char16_t const *filename, [[maybe_unused]] dll_mode mode)
{
	auto hmodule{
		...
		::ufio::win32::LoadLibraryW(filename)
#else
		::ufio::win32::LoadLibraryExW(filename, nullptr, dll_mode_to_win32_ex_flags(mode))
#endif
	};
	if (hmodule == nullptr)
	{
...
{
	return ::ufio::win32_family_api_common<family>(t, win32_family_win32_dll_parameter<family>{mode});
}
...
public:
	...
	explicit win32_family_dll_file(T const &t, dll_mode mode)
		: win32_family_dll_io_observer<family>{::ufio::details::create_win32_family_dll_impl<family>(t, mode)}
	{
	}
	...
	win32_family_dll_file &operator=(win32_family_dll_file &&__restrict other) noexcept
	{
		...
		{
			::ufio::win32::FreeLibrary(this->hmodule);
		}
	...
	void close()
	{
		if (this->hmodule) [[likely]]
		{
			auto ret{::ufio::win32::FreeLibrary(this->hmodule)};
...
{
	auto ptr{::ufio::win32::GetProcAddress(hmodule, symbol)};
...
template <win32_family family, ::ufio::constructible_to_os_c_str T>
inline void *dll_load_symbol(win32_family_dll_io_observer<family> pdliob, T const &symbol)
{
	return ::ufio::posix_api_common(symbol, ::ufio::details::win32_dll_load_impl_context{pdliob.hmodule});
}

template <win32_family family, ::ufio::constructible_to_os_c_str T, ::ufio::constructible_to_os_c_str T1>
