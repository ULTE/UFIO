...
template <::std::integral char_type, typename T1, typename T2>
	requires(::ufio::details::cond_ok_lc_scatter_printable_impl<char_type, T1, T2> ||
			 ::ufio::details::cond_ok_lc_scatter_printable_impl<char_type, T2, T1>)
...
template <::std::integral char_type, typename T1, typename T2>
	requires(::ufio::details::cond_ok_lc_dynamic_reserve_printable_impl<char_type, T1, T2> ||
			 ::ufio::details::cond_ok_lc_dynamic_reserve_printable_impl<char_type, T2, T1>)
inline constexpr ::std::size_t print_reserve_size(basic_lc_all<char_type> const *__restrict all,
												  ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
		return ::ufio::details::cond_lc_print_reserve_size_impl<char_type, T1>(all, c.t1);
	}
	else
	{
		return ::ufio::details::cond_lc_print_reserve_size_impl<char_type, T2>(all, c.t2);
	}
}
												 ...
												 ::ufio::manipulators::condition<T1, T2> c)
{
	if (c.pred)
	{
		return ::ufio::details::cond_lc_print_reserve_define_impl<char_type, T1>(all, iter, c.t1);
	}
	else
	{
		return ::ufio::details::cond_lc_print_reserve_define_impl<char_type, T2>(all, iter, c.t2);
	}
}

// template <::std::integral char_type, typename T1, typename T2, ::ufio::buffer_output_stream bop>
//	requires((::ufio::details::cond_ok_lc_printable_impl<char_type, T1> ||
//			  ::ufio::details::cond_ok_lc_printable_impl<char_type, T2>) &&
//			 (!(::ufio::details::cond_ok_lc_dynamic_rsv_printable_impl<char_type, T1>) ||
//			  ::ufio::details::cond_ok_lc_dynamic_rsv_printable_impl<char_type, T2>))
