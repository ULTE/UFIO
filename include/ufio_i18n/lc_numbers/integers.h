...
inline constexpr char_type to_char_single_digit(T t) noexcept
{
	...
	{
		return ::ufio::char_literal_add<char_type>(t);
	}
	else
	{
		...
		else
		{
			...
			{
				return ::ufio::char_literal_add<char_type, (uppercase ? u8'A' : u8'a')>(t - ten);
			}
		}
	}
}
												  ...
												  manipulators::scalar_manip_t<flags, T>) noexcept
{
	...
	{
		return ::ufio::details::lc_print_reserve_size_int_cal<char_type, flags, char8_t>(all);
	}
	else
	{
		return ::ufio::details::lc_print_reserve_size_int_cal<char_type, flags, ::std::remove_cv_t<T>>(all);
	}
}
...
template <::std::integral char_type, ::ufio::manipulators::scalar_flags flags, ::std::int_least64_t off_to_epoch>
inline constexpr ::std::size_t print_define_internal_shift(basic_lc_all<char_type> const *__restrict,
