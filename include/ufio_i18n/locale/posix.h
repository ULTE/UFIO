...
inline void *posix_load_l10n_common_impl(char8_t const *cstr, ::std::size_t n, lc_locale &loc)
{
	...
	constexpr ::std::size_t total_size{::ufio::details::intrinsics::add_or_overflow_die_chain(
		sizeof(u8"ufio_i18n.locale."), size_restriction, encoding_size_restriction, extension_size)};
	...
	if (found_dot == cstr_end)
	{
		it = ::ufio::details::copy_string_literal(u8".UTF-8", it);
	}
	...
	{
		it = ::ufio::details::copy_string_literal(u8".so", it);
	}
	*it = 0;
	auto p{buffer};
	posix_dll_file dllfile(::ufio::mnp::os_c_str(p), ::ufio::dll_mode::posix_rtld_global |
															::ufio::dll_mode::posix_rtld_now |
															::ufio::dll_mode::posix_rtld_nodelete);
...
{
	return ::ufio::posix_api_common(p,
...
public:
	...
	{
		this->rtld_handle = ::ufio::details::posix_load_l10n_impl(p, loc);
	}
