...
inline void *win32_family_load_l10n_common_impl(
	::std::conditional_t<family == ::ufio::win32_family::wide_nt, char16_t, char8_t> const *cstr, ::std::size_t n,
	lc_locale &loc)
{
	constexpr ::std::size_t size_restriction{256u};
	constexpr ::std::size_t encoding_size_restriction{128u};
	using native_char_type = ::std::conditional_t<family == ::ufio::win32_family::wide_nt, char16_t, char8_t>;
	...
	else if (n == 0)
	{
		...
		if constexpr (family == ::ufio::win32_family::ansi_9x)
		{
			...
			{
				::std::uint_least32_t env_size{::ufio::win32::GetEnvironmentVariableA(
		...
		else
		{
			...
			for (auto i{candidates}, ed{i + sz}; i != ed; ++i)
			{
				::std::uint_least32_t env_size{
					::ufio::win32::GetEnvironmentVariableW(*i, msys2_encoding, msys2_encoding_size_restriction)};
				if (env_size < 2u)
				{
					continue;
				}
	...
	// ufio_i18n.locale.{localename}.{dll/so}
	constexpr ::std::size_t total_size{::ufio::details::intrinsics::add_or_overflow_die_chain(
		sizeof(u8"ufio_i18n.locale."), size_restriction, encoding_size_restriction, sizeof(u8".dll"))};
	native_char_type buffer[total_size];
	native_char_type *it{buffer};
	if constexpr (family == ::ufio::win32_family::wide_nt)
	{
		it = ::ufio::details::copy_string_literal(u"ufio_i18n.locale.", it);
	}
	else
	{
		it = ::ufio::details::copy_string_literal(u8"ufio_i18n.locale.", it);
	}
	if (n == 0)
	{
		constexpr bool use_get_user_default_locale_name{
#if !defined(_WIN32_WINDOWS) && (!defined(_WIN32_WINNT) || _WIN32_WINNT >= 0x0600)
			family == ::ufio::win32_family::wide_nt
#endif
		};
...
#endif
				= char16_t *;
			int ret{::ufio::win32::GetUserDefaultLocaleName(reinterpret_cast<char16_may_alias_ptr>(it),
															   locale_name_max_len)};
			if (ret)
			{
				--ret;
		...
		{
			it = win32_get_locale_name_from_lcid(::ufio::win32::GetUserDefaultLCID(), it);
		}
	}
	else
	{
		it = ::ufio::details::non_overlapped_copy_n(cstr, n, it);
	}
	if (found_dot == cstr_end)
	{
		it = win32_get_locale_encoding_from_code_page(::ufio::win32::GetACP(), it);
	}
	if constexpr (family == ::ufio::win32_family::wide_nt)
	{
		it = ::ufio::details::copy_string_literal(u".dll", it);
	}
	else
	{
		it = ::ufio::details::copy_string_literal(u8".dll", it);
	}
	*it = 0;
	auto p{buffer};
	win32_family_dll_file<family> dllfile(::ufio::mnp::os_c_str(p), ::ufio::dll_mode::none);
...
template <::ufio::win32_family family, ::ufio::constructible_to_os_c_str path_type>
inline void *win32_family_load_l10n_impl(path_type const &p, lc_locale &loc)
{
	return ::ufio::win32_family_api_common<family>(
		p,
		[&loc](auto const *cstr_ptr, ::std::size_t n) {
			using native_char_type =
				::std::conditional_t<family == ::ufio::win32_family::wide_nt, char16_t, char8_t>;
...
public:
	...
	{
		this->hmodule = ::ufio::details::win32_family_load_l10n_impl<family>(p, loc);
	}
	...
	void close() noexcept
	{
		if (hmodule) [[likely]]
		{
			ufio::win32::FreeLibrary(hmodule);
			hmodule = nullptr;
		}
	}
...
template <::std::integral char_type, ::ufio::win32_family family>
inline constexpr ::ufio::parameter<basic_lc_all<char_type> const &>
...
template <::ufio::win32_family family, typename stm>
...
using win32_l10n_9xa = win32_family_l10n<::ufio::win32_family::ansi_9x>;
using win32_l10n_ntw = win32_family_l10n<::ufio::win32_family::wide_nt>;
using win32_l10n = win32_family_l10n<::ufio::win32_family::native>;
