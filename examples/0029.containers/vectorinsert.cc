...
inline constexpr void print_define(ufio::io_reserve_type_t<char_type, test_t>, out_type out,
								   test_t const &t) noexcept
{
	ufio::operations::print_freestanding<false>(out, t.content);
}
...
int main()
{
	...
	std::istringstream s;
	std::vector v1{3, 4, 5};
	ufio::vector v2{3, 4, 5};
	println("-----");

	v1.insert(v1.begin(), 3);
	v2.insert(v2.begin(), 3);
	println(ufio::mnp::rgvw(v1, " "));
	println(ufio::mnp::rgvw(v2, " "));
	println("-----");
	...
	ufio::vector v4{test_t{3}, test_t{4}, test_t{5}};
	println("-----");

	v3.insert(v3.begin(), 3);
	v4.insert(v4.begin(), 3);
	println(ufio::mnp::rgvw(v3, " "));
	println(ufio::mnp::rgvw(v4, " "));
	println("-----");
