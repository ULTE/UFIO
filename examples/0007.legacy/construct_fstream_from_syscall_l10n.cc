...
*/
	...
	*fout.rdbuf() = std::move(*fbf.fb);
	...
	ufio::native_l10n l10n(u8"");
	println(imbue(l10n, ufio::c_stdout()), "Unix Timestamp:", unix_ts,
			"\n"
			"Universe Timestamp:",
			static_cast<ufio::universe_timestamp>(unix_ts),
			"\n"
			"UTC iso8601:",
			utc(unix_ts), "\n", "UTC date_fmt:", ufio::mnp::date_fmt(utc(unix_ts)),
			"\n"
			"UTC era_d_t_fmt:",
			ufio::mnp::era_d_t_fmt(utc(unix_ts)),
			"\n"
			"Local iso8601:",
			local(unix_ts),
			"\n"
			"Local date_fmt:",
			ufio::mnp::date_fmt(local(unix_ts)),
			"\n"
			"Local era_d_t_fmt:",
			ufio::mnp::era_d_t_fmt(local(unix_ts)),
			"\n"
			"Local Timezone:",
			ufio::timezone_name(),
			"\n"
...
D:\hg\ufio\examples\0007.legacy>g++ -o construct_fstream_from_syscall_l10n construct_fstream_from_syscall_l10n.cc
-Ofast -std=c++23 -s -flto -march=native -I../../include -lntdll

D:\hg\ufio\examples\0007.legacy>set L10N=ja_JP.UTF-8

D:\hg\ufio\examples\0007.legacy>construct_fstream_from_syscall_l10n
